...
allowed-tools: Bash(git status:*), Bash(git diff:*), Bash(git log:*), Bash(git add:*), Bash(git commit:*), Bash(git push:*), Bash(vercel ls:*), Bash(vercel inspect:*), Bash(npm run smoke:prod:*), Bash(npm run mark:good:*)
---
...
6. **스모크 (2층 관문)**: `npm run smoke:prod`.
...
7. **known-good 태그**: 스모크가 통과했고 **사용자가 화면을 눈으로 확인**했으면 `npm run mark:good`.
   - 사용자 확인 없이 임의로 태그하지 않는다. 이 태그는 다음 사고 때 되돌아갈 좌표라, 확인되지 않은 시점을 찍으면 의미가 없어진다. 큰 작업이 끝난 배포에서만 제안한다.
8. **결과 보고**: 커밋 해시, 푸시된 브랜치, 최종 배포 URL과 상태(Ready/Error), 프로덕션 별칭, 스모크 결과를 간단히 요약해서 알려준다.

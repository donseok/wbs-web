...
띄웠다면 그 작업이 끝나 결과를 확인하기 전에는 턴을 끝내지 않는다." Build 서브에이전트(팀원 @TSK-03-01-build)가
...
5. **서브에이전트가 끝났는데(finished) 이 오케스트레이터가 게이트를 아직 직접 돌리지 않았다면** — 보고에
   ...
   오케스트레이터가 포그라운드에서 그 프로세스가 끝날 때까지 직접 기다린 뒤(예: PID 를 `wait` 하거나 로그
   파일을 짧은 간격으로 폴링) 게이트를 돌린다. 이미 끝나 있고 남은 작업이 없으면 위 「게이트 집행 원칙」대로
   게이트를 오케스트레이터가 바로 직접 돌린다. 오지 않을 알림을 기다리며 입력 대기로 멈추지 않는다.

...
(2026-08-20-wbs-autonomous-runner-design.md L0/L1). 규율이 두 곳에서 따로 진화하면 품질 기준이 갈라진다 — **수정은 이
폴더(`dflow-dev/references/`)에서만** 하고 소비자는 참조만 한다.

**규율은 읽는 쪽마다 파일이 나뉜다.** 이 파일은 오케스트레이터(Phase 를 띄우고 게이트를 집행하는 쪽)의 몫과, 다른 스킬이
절 이름으로 가리키는 정본을 담는다. Phase 서브에이전트의 규율은 Phase 파일에 있다.
...
- **동시 측정**: 같은 키를 둘이 동시에 재려 하면 잠금을 잡은 쪽만 재고 다른 쪽(`BASELINE_WAITING`)은 결과를 기다렸다
  ...
  `DFLOW_BASELINE_WAIT`(기본 240초)를 넘기면 재지 않고 `BASELINE_BUSY`(exit 75)로 끝난다 — 같은 명령을 다시 호출한다(실패가 아니다, heartbeat 가 끊기지 않게 대기를 짧게 끊는다). 측정은 `baseline.sh` 가 `heavy.sh` 로 감싸 돌리므로 `--` 뒤 명령에 `heavy.sh` 를 붙이지 않는다. 슬롯이 차 있으면 역시 `BASELINE_BUSY` 다. **다른 워커의 측정을 기다리는 시간과 안쪽 `heavy.sh` 슬롯을 기다리는 시간은 마감 하나(`DFLOW_BASELINE_WAIT`)를 나눠 쓴다** — 한 호출의 총 대기는 240초 + 측정 시간 이하다. 결과 게시는 원자적이며 먼저 쓴 쪽이 남는다.
...
Phase 서브에이전트는 `references/phase-prompt.md` 템플릿으로 띄우고, 각자 자기 Phase 파일만 읽는다. 오케스트레이터는 아래
「오케스트레이터가 알 것」 만 보면 된다. 띄우기·게이트·회수 절차는 `/dflow-dev` SKILL.md 「Phase 02~05」 다.
...
Build 를 서브에이전트 하나에 맡기면 호출이 200회를 넘기도 하고, 컨텍스트 누적이 호출 수의 제곱으로 는다. 그래서 design.md
`## 구현 단위` 표(phase-design.md 「구현 단위 표」)의 단위마다 새 서브에이전트에 맡기고, 단위마다 상한을 둔다(phase-build.md
「구현 단위」). 표가 없으면 단위 하나(B1)이며 종전 Build 와 같다.

## Phase 05 — Refactor (선택)

- 무인 모드에서는 이 Phase 를 실행하지 않는다 — 자율 러너와 `/dflow-team` 팀원(`/dflow-dev` 「--worker」 I) 모두다.
  검증 수단이 테스트뿐이라 위험 대비 이득이 작고, 전체 스위트를 두 번(서브에이전트·게이트) 더 돌린다.
...
위 Build·Verify·Refactor 개정과 「무거운 명령 줄 세우기」 의 감쌀 범위 확대가 이것을 줄인다. 아래 수는 **추정**이다(불변
규칙 3~5개, 워커 기준).
...
| Verify | **처음부터 sonnet** | haiku 는 쓰지 않는다(게이트 명령만 다시 돌리는 좁은 확인은 예외). 실측: haiku Verify 의 54% 가 변이 검증·E2E 를 건너뛴 부실 PASS 로 sonnet 재시도가 됐다 |
...
- **감쌀 명령**: 전체 테스트(게이트·Refactor 재실행, `baseline.sh` 를 쓰지 않는 해소 워커의 기준선·게이트), 빌드(`gradlew build`·`npm run build` 등),
  ...
  - 예외 — **JS 러너(vitest·jest 등)의 단일 테스트 파일 실행과 린트**만 짧고 가벼워 감싸지 않는다. Gradle·Maven 은 테스트
    하나도 데몬 JVM + 테스트 JVM 2~3개, 약 2.3GB 를 쓰므로 예외가 아니다(점검 실측: `heavy.sh` 밖의 Gradle 실행 때문에
    JVM 이 최대 10개까지 동시에 떴다).
...
- **`HEAVY_BUSY` 면 같은 명령을 그대로 다시 호출한다. 이것은 실패가 아니다.** 슬롯을 4분(`DFLOW_HEAVY_WAIT`) 안에
  ...
  오래 기다리면 팀장이 무응답(약 5분)으로 오판한다. 슬롯을 얻은 뒤의 명령 실행 시간에는 상한이 없다.
...
- 토큰 낭비(2026-09-24 토큰 실측):
  ...
  - Phase 서브에이전트가 이 문서 전체를 읽기 — 프롬프트에 인용된 절만 읽는다(필요하면 그 절 제목으로 grep 해 그 범위만).

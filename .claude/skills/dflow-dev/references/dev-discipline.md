...
- 구현이 크면 `## 구현 단위` 표를 더한다(「구현 단위」). 작은 작업은 생략한다 — 단위 하나(B1)로 본다.
- 탐색에서 얻은 코드베이스 지식(관례·함정·기존 유틸)은 design.md 에 적는다 —
  다음 Phase 는 이 문서가 컨텍스트의 전부다.
- design.md 에는 설계만 쓴다. 구현 중 기록은 뒤 Phase 가 build-log.md 에 쓴다(「읽기 규율」).

## 읽기 규율 (Build·Verify·Refactor)

뒤 Phase 가 design.md 와 소스를 되풀이해 통째로 읽으면 호출마다 컨텍스트가 쌓이고, 누적은 호출 수의 제곱으로 는다.

- **design.md 전체는 Phase 마다 처음 한 번만 Read 한다.** 그 뒤에는 `grep -n '^## ' <TASKS>/<TSK>/design.md`
  로 절 위치를 찾아 필요한 절만 `sed -n '<시작>,<끝>p' <TASKS>/<TSK>/design.md` 로 읽는다.
- **design.md 에 구현 중 기록을 덧붙이지 않는다.** 변이 검증 기록·설계 이탈·구현 단위 인계는 `<TASKS>/<TSK>/build-log.md`
  에 쓴다(없으면 만든다. Task 문서라 커밋한다). design.md 를 고치는 것은 설계 자체가 바뀔 때뿐이다. 예외는 다른 스킬이
  design.md 에서 읽는 두 절 `## 담당자 확인 필요 결정`·`## 도커 금지로 생략한 검증` 이다.
- **소스는 필요한 범위만 읽는다.** `grep -n '<심볼>' <파일>`(여러 파일이면 `grep -rn`)로 위치를 찾고 Read 의 `offset`·`limit`
  으로 그 주변만 읽는다. 통째 Read 는 300줄 이하 파일이거나 파일 전체 구조를 바꿀 때만 한다. 이미 읽은 범위는 다시 읽지
  않는다 — Edit 뒤 확인도 바뀐 줄 주변만 본다.

## 구현 단위 (Build 나누기)

Build 를 서브에이전트 하나에 맡기면 호출이 200회를 넘기도 하고, 컨텍스트 누적이 호출 수의 제곱으로 는다. 층(BE·FE)만
나눠서는 줄지 않았다(단위마다 200회 넘게 호출). 그래서 Build 를 구현 단위(B1~Bn)로 나눠 단위마다 새 서브에이전트에
맡기고, 단위마다 상한을 둔다. 오케스트레이터의 띄우기·회수 절차는 `/dflow-dev` SKILL.md 「Phase 02~05」 다.

- **Design 이 단위를 정한다.** design.md `## 구현 단위` 표: `단위 | 범위(파일·기능) | 새 테스트 | 담당 불변 규칙`. 한 단위가
  도구 호출 약 80회 안에 끝날 크기로, 단위끼리 같은 파일을 고치지 않게 나눈다(층이 갈리면 층마다, 한 층이 크면 기능
  묶음마다). 단위는 표 순서대로 하나씩 돈다. **작은 작업은 표를 생략한다 — 단위 하나(B1)이며 종전 Build 와 같다.**
...
- **단위 완료**: 그 단위의 새 테스트와 관련 테스트가 초록이면(「Phase 03」 완료 조건) 커밋하고 보고 첫 줄을
  `UNIT_DONE <단위>` 로 쓴다.
- **단위 상한**: 도구 호출이 약 80회를 넘었거나, 읽은 파일·출력 합이 컨텍스트 250K 토큰(추정)을 넘었다고 판단되면 커밋할
  ...
  커밋한 뒤(테스트가 빨개도 된다. 게이트는 마지막 단위 뒤다) 보고 첫 줄을 `UNIT_HANDOFF <단위>` 로 쓴다. 이어 받는
...
- **변이 검증(mutation check)**: design.md 「불변 규칙」 각 항목마다 **일부러 틀린 구현을 넣어
  ...
  research/docs 특례 작업(「research/docs 작업 특례」)은 변이할 코드가 없으므로 변이 검증과 아래 기록 표 대신 문서 검증
  체크리스트를 쓴다. 구현 단위가 여럿이면 규칙마다 담당 단위가 돈다(「구현 단위」).
  ...
  - **한 번에, `heavy.sh` 안에서**: 변이 여럿은 가능하면 스크립트 하나(변이 넣기 → 대상 테스트 → 되돌리기를 규칙마다
    ...
    호출 수와 슬롯 대기가 준다. 스크립트도 「포그라운드 실행」 의 10분 상한을 지킨다 — 길면 나눈다.
...
- **Build 게이트 실패는 1회 재시도한다.** 오케스트레이터가 Build 게이트(전체 스위트)에서 신규 실패를 보면 곧바로
  실패로 끝내지 않고, 같은 Build 서브에이전트에 실패 목록을 넘겨 고치게 한 뒤 게이트를 다시 돈다. 수정은 이 절의
  규율 그대로(테스트 삭제·skip 금지). 두 번째 실패는 중단하고 사람에게 보고한다 — Verify 재시도와 같은 구조다.
...
2. **변이 검증 기록 감사**: build-log.md 「변이 검증 기록」 표의 행마다 표의 변이를 다시 넣고 표의 대상 테스트가
   빨강을 내는지 확인한다(Build 의 fail-fast·되돌리기·`heavy.sh` 규칙 그대로). 「불변 규칙」 에 있는데 표에 없는 규칙,
   표와 결과가 다른 행, `안 잡힘` 인데 보고가 없는 행을 찾아 보고한다. E2E 스위트 전체가 대상인 행도 같다.
   끝나면 넣은 변이를 모두 되돌려 `git status --porcelain` 이 Task 문서 밖에서 비어 있게 한다. research/docs 특례
   작업은 표 대신 문서 검증 체크리스트를 순회한다.
3. **화면 작업이면 E2E 를 돌린다**(「화면 작업의 브라우저 E2E」 스모크 넷, 스크린샷 포함). 서버는 「무거운 명령 줄
   세우기」 의 E2E 서버 규칙대로 슬롯을 붙잡고 띄우며, 끝나면 끄고 푼다.
4. design.md·build-log.md·소스는 「읽기 규율」 대로 읽는다. 감사 결과는 보고로 돌려주고 design.md 에 적지 않는다.

- 실패 시 수정은 **Build 규율로 회귀**한다: 테스트를 삭제·skip 해 초록을 만드는 것 금지,
  테스트 총수 감소 = 게이트 실패. 수정 커밋은 Build 커밋과 분리.
- **Verify 게이트는 Verify 가 코드를 고쳤을 때만 전체 스위트를 돈다.** 오케스트레이터가
  `git diff --name-only <Build 게이트 sha>..HEAD` 를 보고, 바뀐 파일이 Task 문서(`<TASKS>/<TSK>/` 아래)와 `*.md` 뿐이고
  `git status --porcelain` 도 Task 문서 밖에서 비어 있으면(되돌리지 못한 변이가 커밋 밖에 남아 Phase 06 에서 섞이지
  않게) 재실행을 생략하고 Build 게이트 결과를 그대로 쓴다.
...
- 동작 변경 금지 — 중복·네이밍·구조만. design.md·소스는 「읽기 규율」 대로 읽는다. 끝나면 전체 스위트 재실행, 기준선 회귀 = 실패

...
| 공통 규칙 | 정본 |
|---|---|
| 1 커밋·트레일러 | 이 파일(서브에이전트 커밋). 오케스트레이터 커밋은 SKILL.md 「Phase 02~05」 커밋 규칙 문단 |
| 2 읽기 | 이 파일 |
...
`--trailer` 는 `git commit` 옵션이다 — 머지 커밋(`git merge`)은 이 옵션을 모르므로 `/dflow-merge` 「트레일러 고정」 의
방법(둘째 `-m`)을 따른다. 트레일러는 `/dflow-dev` 「--worker」 행 G 의 기본 브랜치 반영 확인이 보는 증거 중 하나라,
빠뜨리면 이 작업이 기본 브랜치에 머지된 뒤에도 후속 워커가 `skipped 선행 승인 대기` 로 오판한다.

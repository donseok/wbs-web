...
from _wbs_md import _fenced_ranges, _in_ranges, line_start_offsets  # noqa: E402
...
Examples:
  ...
  wbs-parse.py docs/wbs.md --export
...
def extract_task_block(wbs_text: str, tsk_id: str) -> str:
    ...
    The ending boundary scan ignores fence-internal WP/ACT-level headings, so
    a fenced example inside the Task body (e.g. a ```markdown block showing
    `## WP-99:`/`### ACT-99-01:` syntax) does not end the block early and
    silently drop the real acceptance/spec_sections that follow it. Task
    headings (`TSK-...`) still close the block regardless of fence state —
    same carve-out as wbs-validate.py's `_split_tasks`/merge-wbs-status.py's
    `parse_status_lines` — so a fenced example TSK heading can't let this
    block swallow the next real Task's metadata.
    """
    fences = _fenced_ranges(wbs_text)
    lines = wbs_text.splitlines()
    offsets = line_start_offsets(wbs_text)
    result = []
    found = False
    level = 0
    ...
    for idx, line in enumerate(lines):
        ...
        # End at same or higher level heading. Task headings always count as
        # boundaries; non-Task headings inside a fence don't.
        is_task_heading = bool(_TSK_HEADING_RE.match(line))
        if (found and hl >= 2 and hl <= level and f"{tsk_id}:" not in line
                and (is_task_heading or not _in_ranges(offsets[idx], fences))):
            break
...
def parse_nodes(wbs_text: str, docs_dir: str) -> list:
    ...
    WP/ACT 헤딩 인식은 펜스 코드 블록 내부의 줄을 무시한다 — 그러지 않으면
    Task 본문의 ```markdown 예시에 든 `## WP-99:`/`### ACT-99-01:` 같은
    줄이 parent 가 없는 phantom WP/ACT 노드를 만들고, 그 뒤의 진짜 Task 를
    엉뚱한 부모 아래로 편입시켜 계층을 깨뜨린다. TASK 헤딩 인식은 의도적으로
    펜스 필터를 적용하지 않는다 — 과다 탐지(눈에 보임)가 과소 탐지(silent
    loss)보다 안전하다는 wbs-validate.py 의 채택 정책을 그대로 따른다.
    """
    lines = wbs_text.splitlines()
    fences = _fenced_ranges(wbs_text)
    offsets = line_start_offsets(wbs_text)
    ...
    for idx, line in enumerate(lines):
        in_fence = _in_ranges(offsets[idx], fences)

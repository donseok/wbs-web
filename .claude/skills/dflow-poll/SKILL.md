...
- list 응답에는 tags 가 없어 poll.sh 가 후보별 show 로 조회한다. 태그 없어 떨어진 후보는
  `--tag-cache-cycles`(기본 3)주기 동안 다시 show 하지 않으므로, 새로 단 태그는 그만큼 늦게 잡힌다.
  서버 list 응답에 tags 를 포함시키는 개선은 러너 설계 개정 묶음의 후보다.
...
2. **종료 알림 분기** (exit code — 산문 파싱 금지):
   ...
   - **10 = 반려 감지(재작업 대상)**: stdout 각 줄이 `TSK<TAB>order-id<TAB>review_note` —
     ...
     **반려는 order.status 를 rejected 로 만들지 않는다 — claimed 로 롤백될 뿐이라 일반 claimed 와
     ...
     `/dflow-dev <id8>` 재작업 사이클을 실행한 뒤 재기동한다(dflow-dev Phase 01 의 반려 재작업 경로).

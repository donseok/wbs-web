...
팀원 안의 격리 가드는 Bash 명령 문자열을 정적으로 검사해 네 형태를 거부한다: bare `git`(rtk 훅이 `rtk git` 으로
재작성해 걸린다), `$(command -v git)`·변수처럼 git 절대경로를 치환·변수로 넣는 형태, git 을 감싼 명령 치환
(`x=$(... git ...)`), 워크트리 밖 경로를 가리키는 `-C`. `set -a; . 파일; set +a; <명령>` 같은 `.` 소싱 접두도
같은 이유로 거부된다. 허용되는 형태는 `command -v git` 단독 실행, 그 절대경로를 글자 그대로 적은 git 호출,
접두 없는 `.claude/skills/dflow-work/scripts/dflow.sh <cmd>` 호출, `DFLOW_ENV_FILE=<절대경로> dflow.sh <cmd>`
같은 인라인 환경변수 대입이다. `dflow.sh` 내부의 git 호출은 Bash 명령 문자열이 아니라 스크립트 안의 코드라서
이 가드를 거치지 않는다.

- 팀원은 백그라운드로 돈다. 끝나면 완료 알림이 오고, 변경이 남았으면 워크트리 경로·브랜치가 함께 온다.
  완료 알림은 팀원이 자기 서브에이전트를 기다리며 턴을 끝낼 때도 오므로 그 자체로는 완료의 증거가 아니다.
...
- 회수: 결과 줄을 처리한 직후(status 와 무관하며 `blocked` 도 포함한다) `TaskStop(w<slot>-<id8>)`. 이름 붙은
  에이전트는 일을 마쳐도 idle 로 남는다. 팀원이 띄운 서브에이전트는 팀원이 끝나도 독립적으로 살아남아 팀장의
  ListAgents 목록에 자기 이름으로 나타난다. TaskStop 은 이름 단위이며, 그 안에서 돌던 Bash 자식 프로세스까지
  함께 멈춘다.
- `blocked` 워크트리: 팀원이 커밋·push 하고 끝나므로, 결과 처리 직후 「고아 정리 규칙」 2번을 맞추면(HEAD 가
  ...
  표시를 가져 재구성이 충돌한다.
- 격리 워크트리는 팀원이 턴을 끝낼 때 작업트리가 깨끗하면(미추적·미무시 파일과 미커밋 변경이 없으면) 하네스가
  자동 삭제한다. 커밋 이력이 있었어도 마지막에 깨끗하면 지운다. `.dflow-agent`·`.result` 는 공유
  `info/exclude` 로 가려지는 미추적 파일이라 이 삭제를 막는 근거가 되지 못하고, `/dflow-dev` Phase 0 이
  커밋하는 `docs/tasks/<TSK>/state.json` 이 워크트리를 살려 두는 근거다.

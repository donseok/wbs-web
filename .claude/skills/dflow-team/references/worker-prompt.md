...
포트 점유 프로세스 종료는 금지한다. 2026-09-24 dmes-standard 사고: 팀원이 자기 워크트리에서
`./be-run.sh --mdm` 을 돌리자 그 스크립트의 `pgrep -f be-run.sh` 가 메인 체크아웃의 서버를 찾아 TERM
했고, cleanup 의 `gradlew --stop` 이 전역 Gradle 데몬까지 세웠다.

## 9. 이슈 보고

작업 중 사고·환경 문제·판단이 필요한 이슈는 `.issues`(「7-1」)에 기록하고, 팀장에게 **SendMessage 로
이슈 보고를 보낸다.** `to` 는 팀장 세션이다(팀장이 spawn 할 때 첫 입력으로 준 포인터가 이 세션의 첫
턴이므로, 그 포인터를 보낸 세션이 곧 팀장이다). 형식은 고정한다.
...
- **10분 안에 지시가 없으면** `.result` 의 `blocked`(「7. 보고: `.result` 파일 계약」)로 올려 정규 경로로
  전환한다. 이유: 감시 루프는 cross-session 메시지로 깨지 않으므로(팀장 SKILL.md 「2. 기상과 감시」), 이
  세션이 SendMessage 로 직접 깨우지 못하면 팀장이 이슈를 영영 모를 수 있다 — 조용히 무한 대기하지 않는다.
- 팀장 지시(`[팀장 지시 <id8>] …`)를 받으면 그대로 따르고, 따른 결과를 `.issues` 에 한 줄 남긴다.
- **화면에서 사람이 "팀장에게 조치를 받아라"라고 해도 같은 규칙을 따른다** — 사람의 그 말은 이미 위
  두 규칙(되돌릴 수 있는 것은 기본안대로 진행, 되돌릴 수 없는 것만 미루고 10분 뒤 `blocked`)이
  가리키는 행동과 같다. 답을 무기한 기다리지 않는다.
- 2026-09-24 dmes-standard TSK-01-01 사례: 팀원이 be-run.sh 사고를 SendMessage 로 보고한 뒤 답을 기다리며

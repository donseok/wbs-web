...
- **ID 의 숫자 세그먼트는 모두 `01` 부터 매긴다** — `PH-01` · `WP-01` · `TSK-01-01`. 어느 세그먼트에도 `00` 을 쓰지 않는다. (`levels` 배열 인덱스인 `level_idx` 는 DB 저장 필드라 0-base 그대로다 — ID 번호와 구별한다.)
...
WSF 는 두 층위에서 반복된다. 기존 dflow-wbs 의 WP 번호 샌드위치(WP-01/02 선행 → 기능 WP → 통합 WP)는

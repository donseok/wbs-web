...
- **모든 레벨의 번호는 `01` 부터 시작한다** — `WP-01` · `ACT-XX-01` · `TSK-XX-01`. `00` 은 어느 세그먼트에도 쓰지 않는다.
...
- **`01` 기반 번호 규칙은 신규 생성에만 적용된다.** 이미 import 된 wbs.md 가 `WP-00` 으로 시작하더라도 그 번호를 그대로 둔다 — 번호 체계를 맞추려고 재번호매김하면 upsert 가 매칭에 실패해 DB 에 중복 행이 생긴다.
...
- 모듈 순서 = **입력 등장 순서**. 기능 WP 는 `WP-03` 부터 (`WP-01` 초기화 · `WP-02` 기본설계 예약).
...
- **WP 번호 순서와 depends 순서는 다르다** — 전사 계약(WP-01)이 전사 설계(WP-02)에 depends 하는 것이 정상이다. WP 번호는 성격별 묶음이고 실행 순서는 depends 가 정한다.

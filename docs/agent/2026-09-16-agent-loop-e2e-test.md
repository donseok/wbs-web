...
| 코드 | 내용 | 나타난 단계 |
|---|---|---|
| N23 | **중단된 팀원을 팀장이 이어받지 못하고, 자동으로 못 잇는 것을 멈춤으로 표시하지도 않는다.** `TSK-02-10` 의 워커가 Build 도중 죽었다. 워크트리(`dflow-443b8ffe`)에는 커밋 3개와 **미커밋 6파일 129줄, 미추적 6파일**이 그대로 남았고 서버도 `status=claimed`·`stage=ip` 로 점유가 살아 있어 재개에 필요한 것이 전부 갖춰져 있었는데, **팀장을 다시 띄워도 그 워크트리를 이어받지 않는다.** 「1. 시작」 2번이 「자동 재착수 없음 — 수동 세션이 진행 중인 작업일 수도 있다」로 못박고 있고, 고아 스캔은 미커밋이 있으면 `.dflow-agent` 를 `parked` 로 바꾸고 「재개 필요」 한 줄만 남긴다. 그 한 줄에는 **어떻게 재개하는지가 없다.** 함께 드러난 둘: (가) `dflow.sh heartbeat` 는 `.dflow-agent` 값이 `*/parked` 면 exit 2 로 거부하므로, **사람이 parked 워크트리에서 손으로 재개하면 그 세션은 좌석표에 진척을 하나도 알리지 못한다**(`dflow.sh:286`). 보존 표시가 곧 재시작 방해다. (나) **생존 신호는 DB 에 있는데 `show` 가 내주지 않는다.** `agent_work_orders` 에는 `last_heartbeat_at`·`heartbeat_phase` 가 실제로 쌓이고 좌석표는 그것으로 침묵 5분을 `STALE`, 30분을 `OFFLINE` 로 그린다(`seatState.ts`). 그런데 `show` 응답 키에는 둘 다 없고, 대신 `stale`(= `claimed_at` 으로부터 24시간 경과 여부, `AGENT_CLAIM_STALE_HOURS = 24`, `agentWork.ts:37`)만 있다. **그래서 화면은 멈춘 워커를 아는데 팀장은 모른다.** 필드 둘을 `show` 에 더하면 팀장의 자동 재개를 이 PC 밖으로 넓힐 수 있다. 2026-09-18 에 정본 스킬을 고쳤다: 고아 스캔을 **정리 가능·재개 가능·멈춤** 셋으로 가르고, 이 PC·이 신원의 워크트리가 남아 있으면(`claimed_by` 소문자 대조 + `mine=true`) 「5-1. 재개 spawn」 이 claim 없이 그 자리에서 이어받으며, 워크트리가 없거나 다른 PC 가 claim 한 갈래는 자동으로 오지 않고 `--resume <id8>` 로만 온다. 자동으로 못 잇는 것은 전부 **「멈춤」 표**(id8·TSK·경로·브랜치·미커밋 수·사유·**재시작 명령**)로 시작·마감 보고에 낸다. 재시도 상한(3)은 새 파일을 만들지 않고 `events.jsonl` 의 마지막 `team.result` 이후 **재개분(`spawn_kind:"resume"`)** 개수로 센다. 같은 날 화면 쪽도 닫았다 — 좌석표의 멈춘 좌석에 「이어서 시작」 버튼이 생겼고(`STALE`·`OFFLINE` 에만), 그 요청이 `watch` 응답의 `resume_requests` 로 팀장에게 간다. **여기서 갭이 하나 더 드러났다**: `dflow.sh watch` 가 응답에서 `.expires_at` 한 줄만 뽑고 본문을 버려, 서버가 요청을 실어 보내도 팀장에게 닿지 않았다. `--json` 플래그를 더해 해소했다. 서버 계약을 늘릴 때 **그것을 읽는 클라이언트 스크립트가 같이 늘어야 한다**는 것이 이 갭의 일반형이다. **발견이 늦어진 경위가 더 볼 만하다**: 서버를 만든 세션은 검증 때 `dflow.sh watch` 를 먼저 돌려 `expires_at` 한 줄만 나오는 것을 보았고, 원시 응답을 보려고 curl 로 넘어갔다. **그 자리에서 이미 「스크립트가 본문을 버린다」를 보고 있었으면서 자기 검증을 통과시키는 데만 쓰고 갭으로 올리지 않았다.** 막힌 길을 우회해 검증에 성공하면 그 길이 막혔다는 사실이 성공에 가려진다. 에이전트가 자기 검증 경로를 실제 운영 경로와 다르게 잡을 때 나타나는 형태이고, 서버와 클라이언트를 다른 세션이 맡으면 그 사이를 아무도 보지 않는다. 종류를 가르지 않으면 팀장을 다시 띄울 때마다 도는 「이어받은 슬롯 재기록」이 재시도로 잡혀 상한에 금방 닿는다 | 16 |
| N22 | **같은 서버 응답을 두 워커가 반대로 판정한다.** `TSK-02-06` 과 `TSK-02-10` 의 선행 조건은 완전히 같았다(선행 `TSK-00-03` 이 `stage:im`·`order_approved:false`·`head_sha:null`·**`reached:true`**). 그런데 전자는 `skipped 선행 승인 대기` 로 끝냈고 후자는 정상 착수했다. `/dflow-dev` 는 「`'reached' in d` 면 **그 값이 판정이며 축을 다시 조합하지 않는다**」(v2.3)고 못박으므로 `skipped` 쪽이 규칙 위반이다. **판정이 확률적이라는 뜻이고, 승인 버튼을 누르지 않는 이번 운영에서는 이 오판 하나가 화면 계열 Task 전체를 막는다.** N12 와 같은 자리에서 나는데, N12 는 규칙 자체가 막는 경우이고 이것은 규칙을 지키지 않는 경우다 | 16 |
| N21 | **워커가 기점을 스스로 과거로 되돌린다.** 오케스트레이터가 `origin/main` 에서 워크트리를 만들어 주었는데도 `TSK-02-11`·`TSK-02-05` 두 워커가 claim 전 기점 이동에서 「--worker」 B 규칙을 오적용해, 이미 main 에 반영된 선행의 `head_sha`(`11192be`)로 되돌아가 스택 브랜치를 만들었다. 둘 다 `risk: "선행 main 미반영(팀장 머지 대기)"` 를 기록했으나 `git merge-base --is-ancestor 11192be origin/main` 이 **참**이다. B 규칙은 「선행이 approved 인데 main 미반영이면」이 전제인데 그 전제를 확인하지 않았다. 결과는 **기준선 오염**이다(되돌아간 기점의 테스트 수가 현재 main 보다 적어 게이트가 헐거워진다). `TSK-02-11` 은 `blocked` 로 손을 들어 팀장이 알아챘지만 **`TSK-02-05` 는 묻지 않고 그대로 완주했다 — 조용히 끝난 쪽이 더 위험하다** | 16 |
| N20 | **횡단 관심사 작업이 이미 머지된 작업들의 요청 계약을 소급해서 깨뜨린다.** `TSK-00-03`(권한 가드)이 `/api` 전체에 `requireRole` 을 걸자, 앞서 머지된 네 Task 의 시험이 전부 401 을 받았다. 가드를 API 라우터 **뒤**에 두면 라우터가 먼저 응답해 가드가 무력해지므로 앞에 둘 수밖에 없고, 그러면 기존 시험의 `fetch` 호출이 전부 인가 헤더를 실어야 한다. 두 번의 머지에서 **시험 37곳**을 손으로 고쳤다(domains 9·rules 1·units 15·systems·dict-systems·terms). `depends: TSK-00-02` 가 선언돼 있어도 막지 못한다. **선행 관계는 「그 작업이 끝났는가」만 보고 「그 작업이 내 계약을 바꾸는가」는 보지 않기 때문이다.** N19 가 동시에 도는 작업 사이의 충돌이라면 이것은 **이미 끝난 작업을 거슬러 깨는** 형태이고, `/dflow-dev`·`/dflow-team` 어디에도 해소 경로가 없다. 근본 대응은 WBS 단계에서 횡단 관심사 Task 를 **모든 API Task 의 선행으로 앞세우거나**, 계약 Task 로 분리해 시험 규약(인가 헤더 포함)을 먼저 동결하는 것이다 | 16 |
| N19 | **병렬 팀 개발이 만든 계약 충돌을 `depends` 가 막지 못하고, 절차서에도 그 해소 경로가 없다.** `TSK-02-08`(도메인 상속 검사 API)과 `TSK-02-09`(도메인 유효 검증식 조립 API)는 서로 선행 관계가 없어 둘 다 `origin/main` 에서 브랜치를 따 동시에 돌았는데, 같은 모듈 셋(`src/routes/domains.js`·`src/server.js`·`tests/routes/domains.test.js`)을 각자 **전부** 정의했다. 텍스트 충돌이 아니라 **계약 충돌**이다: 한쪽은 `createApp({ db } = {})`, 다른 쪽은 `createApp(db)` 로 같은 함수의 시그니처를 다르게 정했고, 라우터 export 도 default 와 named 로 갈렸다. 먼저 머지된 `TSK-02-09` 의 코드 주석은 「나중에 `TSK-02-08` 이 이 파일에 `GET·PUT·DELETE` 를 추가할 때는 ADR-0011 을 지켜야 한다」고 **미래형으로** 적고 있었는데, 실제로 그 작업은 나중이 아니라 **동시에** 돌고 있었다. 해소는 오케스트레이터가 손으로 두 라우트 집합을 합치고 시그니처를 하나로 정하고 이미 머지된 작업의 시험 세 줄을 고치는 것이었다(`mdm-dict` `b3c9957`). **`/dflow-dev` 와 `/dflow-team` 어디에도 이 경로가 없다** — 두 스킬 모두 머지를 fast-forward 나 자동 머지로 전제하고, 충돌은 「중단·보고」로만 다룬다. WBS 생성 단계에서 **같은 파일을 건드릴 작업들 사이에 선행을 걸거나 계약 Task 로 분리**하는 것이 근본 대응이다 | 16 |
| N18 | **`review_note` 가 비어 있거나 무내용이어도 반려가 성립하고, 그러면 재작업에 요구사항 입력이 없다.** `TSK-02-07` 의 반려 사유는 **「다시해」** 세 글자다. `/dflow-dev` 의 반려 재작업 경로는 「`review_note` 를 **요구사항 입력**으로 삼아 설계부터 다시 판단한다」고 규정하는데, 이 입력에는 무엇이 문제였는지가 없다. 그대로 Design 을 재실행하면 같은 산출물을 다시 만들게 되고, 통과하든 또 반려되든 그 결과가 무엇을 뜻하는지 알 수 없다. **서버에도 스킬에도 사유의 최소 길이나 내용을 요구하는 검사가 없다.** 이 주행에서는 실행 세션이 사용자에게 사유를 되물어 막았다. 재작업을 자동으로 도는 운영에서는 되물을 상대가 없다 | 15 |
| N17 | **`/dflow-dev` 의 반려 판정 술어가 `ready` 배선을 못 잡고, 그 오판이 산출물을 치워 버리는 경로로 이어진다.** 스킬은 반려를 「로컬 `phase=reported` + 서버 `status=claimed`」로 감지한다(N16 의 1형). 2형(`status=ready`)에서는 이 술어가 거짓이라 스킬이 **신규 claim 경로**로 분기하고, 그 경로의 「재claim 시 이전 시도의 잔재 격리」 규칙이 `docs/tasks/<TSK>/` 를 `docs/tasks/<TSK>.prev-<날짜>/` 로 옮긴다. `TSK-02-07` 의 경우 46.7K 짜리 `design.md` 와 반려 이력이 통째로 빠진다. **스킬 본문이 「반려 재작업은 예외 — 옮기지 않는다」고 명시한 바로 그 사고가, 판정이 빗나가서 일어난다.** 게다가 로컬 `state.json` 이 없는 새 워크트리에서는 서버가 `ready` 이므로 워커가 이것을 **평범한 미착수 작업으로 잡아** 설계부터 새로 쓴다. 해소 방향은 판정 술어를 `status` 가 아니라 **마지막 `kind=completion` 리포트의 `review_action=reject`** 하나로 바꾸는 것이다. 그 필드는 두 배선 모두에서 동일하게 남는다 | 15, 16 |
| N16 | **반려 배선이 한 가지가 아니다.** 같은 주행에서 두 형태를 실측했다. 1형(`TSK-01-02`, 2026-09-17): `status` `reported`→`claimed`, `stage` `im`→`ip`, `claimed_by` 유지, 실적 80→50(크레딧 표의 RW 값). 2형(`TSK-02-07`, 같은 날): `status` `reported`→**`ready`**, `stage` `im`→**`as`**, `claimed_by`→**`null`**, 실적 **100 그대로**. 2형은 점유가 풀려 **다시 claim 해야** 재작업할 수 있고, 1형은 claim 이 유지되어 그대로 이어 간다. 계약 문서(`api-contract.md`)는 반려를 사람 전용 전이로 규정할 뿐 어느 형태로 되돌리는지 적지 않는다. **2형에는 그 자체로 내부 불일치가 있다 — `stage=as`(미착수)인데 `actual_pct=100`(완료)이 남아 있다.** 두 사례의 차이가 웹 UI 의 선택지 때문인지, claim 주체(오케스트레이터 직접 / 팀원 워커)의 차이인지, 사람이 반려 뒤 단계를 따로 만졌기 때문인지는 확정하지 못했다. **이 갈림이 N17 의 원인이다** | 15, 16 |

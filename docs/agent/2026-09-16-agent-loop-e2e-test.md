...
`<PAT_SU>` 발급(이름 `e2e-su`, PREFIX `b5ghryuO0ACc`, 스코프 `work:read`·`work:claim`,
프로젝트 MDM 사전, 만료 2026-12-15). **`<PAT_MEM>` 은 아직 발급하지 못했다** — 그 계정으로 직접
로그인해야 하는데 이 세션이 비밀번호를 갖고 있지 않다.
...
**결과**: [ ] 통과  [ ] 실패  **[x] 절반** 비고: 2026-09-16 주행. `<PAT_SU>` 만 `.env` 에 넣은
상태로 `doctor` 와 `me` 를 확인했다. 세 번째 명령(`--as yoo7032@gmail.com me`)은 `<PAT_MEM>` 이
없어 아직 못 돌렸다.

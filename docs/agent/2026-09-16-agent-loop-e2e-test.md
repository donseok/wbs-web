...
| 코드 | 내용 | 나타난 단계 |
|---|---|---|
| N10 | **담당자 재배정이 변경 이력에 남지 않는다.** 항목 상세의 「변경 이력」은 실적%와 단계 전이만 적고 담당자 변경은 한 줄도 남기지 않는다. 이번 주행에서 `<SU>` 담당 7건 중 최소 다섯이 `<MEM>` 으로 재배정됐는데(`TSK-00-03`·`TSK-02-01` 은 **한 번도 claim 하지 않았는데도** 담당자가 붙어 있다), **누가 언제 몇 건을 바꿨는지 화면으로 확인할 방법이 없다.** 사용자 본인도 건수를 기억하지 못했고 이력에도 근거가 없어 끝내 확정하지 못했다. 배정은 사람 전용 전이(`api-contract.md:168`)인데 그 전이만 감사 기록에서 빠져 있다 | 9, 11, 15 |

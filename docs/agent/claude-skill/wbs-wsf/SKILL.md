...
> 플러그인 `dev:wbs-wsf` 의 파생 스킬. 플러그인 원본은 수정하지 않는다.
> **이 파일의 정본은 wbs-web 리포다** — `docs/agent/claude-skill/wbs-wsf/SKILL.md`.
> `~/.claude/skills/wbs-wsf` 는 그 디렉터리로의 심링크다.
...
```bash
python3 ~/.claude/plugins/cache/dev-tools/dev/1.7.1/scripts/prd-validate.py validate --target {DOCS_DIR}/PRD.md
python3 ~/.claude/plugins/cache/dev-tools/dev/1.7.1/scripts/prd-validate.py validate --target {DOCS_DIR}/TRD.md
```
...
```bash
python3 ~/.claude/plugins/cache/dev-tools/dev/1.7.1/scripts/wbs-parse.py {DOCS_DIR}/wbs.md - --dev-config > /tmp/dev-config.json
python3 ~/.claude/plugins/cache/dev-tools/dev/1.7.1/scripts/wbs-validate.py validate --wbs {DOCS_DIR}/wbs.md --dev-config-json "$(cat /tmp/dev-config.json)"
```
...
10. 의존 그래프 검증:
    ```bash
    python3 ~/.claude/plugins/cache/dev-tools/dev/1.7.1/scripts/wbs-parse.py {DOCS_DIR}/wbs.md --tasks-all > {scratchpad}/tasks.json
    python3 ~/.claude/plugins/cache/dev-tools/dev/1.7.1/scripts/dep-analysis.py {scratchpad}/tasks.json --graph-stats
    ```
...
>
> **가져오지 않는 것 (원본이 낡았다):**
> - **상태 어휘** — 원본 491행은 5상태(`[ ]`/`[dd]`/`[im]`/`[ts]`/`[xx]`)를 서술한다. 이 스킬은 상태를 서술하지 않는다 — 생성은 항상 `[ ]`, 전이는 D'Flow 가 정본이다(`## 상태`).
> - **category 값 이름** — 원본은 `infrastructure`(494행)·`development`(536행)·`integration-test`(579행)를 쓴다. 이 스킬은 7종 약어(`## Task category`)를 쓴다: `infra`·`dev`·`itest`.

...
> - `ddobak-folder-path-worklist-2026-07-27.md` — 또박또박 측 작업지시(ddobak-W1~W19). **(내부 문서 · 미첨부 — 필요한 내용은 본문 §0.1·§11에 인라인)** (본문에서 「또박또박 작업지시 §…」로 인용)
> - `docs/design/dflow-minutes-upload-api-spec.md` — **기존 API 계약(정본은 wbs-web 레포 사본, v2.2)**. ⚠️ 이 계약서는 아직 `folder_path`를 모르고, 이번 변경과 **정면 충돌하는 문장 3개를 살아 있는 채로 담고 있다.** 또박또박 측 사본과도 이미 갈라져 있다 → **§4 「⚠️ W8」 블록을 착수 전에 읽을 것.** (✅ 그 개정은 **완료** — 현재 정본 **v2.3**, 또박또박 송부본은 **v2.4**. §4 W8 행 참조)
> - **`dflow-decisions-final-2026-07-27.md` — 결정 정본. 이 지시서와 어긋나면 그 문서가 이긴다.**

---

> ## ⚠️ 2026-07-27 개정 — 결정 정본이 이긴다 · 구현은 이미 끝났다
>
> 이 지시서는 **결정 이전에** 작성됐다. `dflow-decisions-final-2026-07-27.md`(이하 「결정」)가 확정한 사항과 어긋나는 문장은 **결정이 이기며**, 아래 절은 결정에 맞춰 개정했다:
>
> | 절 | 개정 내용 | 근거 |
> |---|---|---|
> | §3.3 | 미분류 폴백 응답 값 **확정**(`folder_path: null`) + `folder_path_status` 신설 | 결정 §2-D·§2-C |
> | §4 | 작업표에 **`W24`(`include_archived`)·`W25`(플래그)** 등재 | 결정 §2-B·§2-A |
> | §8.2 | 건별 `failed` 경계 · `pmo_admin` 게이트 · 판정 전 폴더 미생성 · `from` nullable · `folder_path_status` | 결정 §2-H·§2-C·§2-E ⑥⑦ |
> | §8.3 | 판정 기준을 **조상 규칙**으로 교체(「⚠️ 결정 필요」 해소) | 결정 §2-J |
> | §8.5 | 제목 접두 역파싱 대안 **강등** | 결정 §1-② |
> | §9.7 | **(a) 채택** — `include_archived`·`archived` 구현 완료(「⚠️ 결정 필요」 해소) | 결정 §2-B |
> | §10 | 조상 규칙·플래그·건별 failed 반영 | 위와 동일 |
> | §11.1·§11.2·§11.3 | 표제 `~W19`, **차수표를 결정 §3 순서로 교체**, `ddobak-W8` 확정·`ddobak-W14` 문구 정정 | 결정 §3·§2-B·§2-C |
> | §12 | 미해결 표 갱신(해결 2건 · 정본 확정 3건 · 착수 조건 2건) | 결정 §6 |
>
> **구현은 끝났다**(작업 브랜치 `c769ee5`·`afc1943`·`4387576`, `main` 미머지). 이 문서는 이제 "무엇을 만들 것인가"가 아니라 **"무엇이 만들어졌는가"**로 읽는다 — 남은 것은 배포 순서(§11.2)와 운영 절차(결정 §4)다.
...
>
> ✅ **개정(결정 §2-A)**: 그 상호 의존을 **서버 플래그 `MINUTES_FOLDER_PATH_ENABLED`(`W25`)** 로 끊었다. D'Flow는 `W1`~`W6`·`W24`를 **한 배포(R1)로 내되 플래그를 `false`로 둔다** — 그동안 `POST /minutes`는 `folder_path`를 **키 부재와 완전히 동일하게** 취급하므로(검증 400조차 없음) 오늘 동작을 1비트도 바꾸지 않고, 배치(`W6`)와 보관 상태 노출(`W24`)만 새로 열린다. 즉 **또박또박 1차는 언제 내도 무해**하고, 3차(재편철)를 2차(전송 전환)보다 먼저 돌릴 수 있다. 상세 = §4 `W25` · §11.2.
...
| C7 ★신설 | **폴더명 NFC 정규화** | macOS에서 만든 한글 폴더명은 **NFD**로 들어올 수 있다. 저장·비교 경로 전부가 `normalizeFolderName()`(`src/lib/domain/minutes.ts`)을 통과해야 한다 — 외부 API(`parseFolderPathValue`)·UI 생성(`createMinuteFolder`)·개명(`renameMinuteFolder`) 3곳 모두 적용 완료 | **눈에 같은 이름의 폴더가 두 개** 생긴다. 부분 유니크 인덱스도 바이트가 달라 막지 못한다 |
...
> ✅ **W25 플래그 게이트(결정 §2-A)** — 아래 검증·편철은 **`MINUTES_FOLDER_PATH_ENABLED=true`일 때만** 적용된다. 플래그가 꺼져 있으면 `folder_path`는 **키 부재와 완전히 동일**하게 취급돼 **검증 400조차 나지 않는다**(61자 폴더명이 섞인 회의도 오늘처럼 정상 전송된다). 구현: `parseMinutePayload`의 `folderPathProvided = folderPathEnabled() && b.folder_path !== undefined`(`src/lib/minutes/externalApi.ts`). 배치(§8)와 `include_archived`(§9.7)는 **플래그와 무관하게 항상 활성**이다.
...
{ "ok": true, "id": "...", "action": "created",
  ...
  "folder_path_status": "exact",                         // ★ 신규(결정 §2-C). 편철 품질 신호
  "title": "...", "date": "...", "team": "MES", "url": "...", ... }
```

또박또박이 “어디에 들어갔는지”를 전송 다이얼로그에 표시한다. `meta`에 폴더 목록을 노출하지 않는 대신(§5 · **D5** — §0.1) 이 에코가 유일한 피드백 경로다.

> ⚠️ 에코는 **요청한 경로가 아니라 실제 편철 결과**다. 플래그가 `false`인 R1 구간에서도 응답은 정직하다 — `folder_path`를 무시하고 팀 루트에 편철했으면 `["MES"]`를 돌려준다(요청 경로를 되돌려주지 않는다).

#### `folder_path_status` — 편철 품질 신호 (★ 결정 §2-C로 신설)

경로가 짧아지는 원인이 **3개**라 “보낸 경로와 받은 경로를 비교”로는 구분되지 않는다. 또박또박이 스스로 판정하려면 §3.2 정규화 규칙 ①②③과 활성 팀코드 목록을 Ruby에 재구현해야 하는데(게다가 ② 판정은 서버 TTL 캐시에 의존해 클라이언트가 재현할 수 없다), 그건 이 문서가 서버 측에 금지한 “두 번째 정규화 구현”을 클라이언트에 강요하는 셈이다. → **서버가 판정해 실어 준다.**

| 값 | 의미 |
|---|---|
| `exact` | 정규화 결과 그대로 편철(**한 칸 내림 포함** — 정상 동작) |
| `truncated` | 깊이 5 초과로 **절단**(§3.2-4). 사용자가 실제로 만든 조상 폴더에 들어가므로 해상도 저하이지 위조는 아니다 |
| `partial` | 중간 폴더 **생성 실패**로 조상까지만 편철. 종전에는 200 성공 응답에 아무 신호도 없이 **완전히 침묵**하던 경로다 |
| `unclassified` | 시드 팀 루트 부재로 **미분류**(§3.2-5) |

- **심각한 쪽부터 판정한다**: `unclassified` > `partial` > `truncated` > `exact`.
- `POST /minutes` 응답과 배치 `results[]`(§8.2) **양쪽**에 실린다. 절단 신호는 등록보다 **배치에 더 급하다** — 절단본은 APPLY 후 재실행하면 목표 폴더가 실재하므로 **영구히 `already_correct`**가 되어 자기 복구가 불가능하다(결정 §2-C).

#### 미분류 폴백 시 응답 값 — ✅ **확정**(결정 §2-D · E1)

§3.2-5의 `null` 폴백이 발동하면 `folder_id`는 `null`이 되는데, `folder_path`를 `[]`로 두면 또박또박이 §3.1 3값 규약(`[]` = 팀 루트)대로 **“팀 루트에 편철됨”이라는 정반대 안내**를 한다.

**확정**: `folder_id: null` + **`folder_path: null`** + `folder_path_status: "unclassified"` — `[]`(팀 루트 편철 **성공**)와 구분한다. 또박또박 측 응답 타입 nullable(`ddobak-W9`)과 “미분류로 들어갔습니다” 문구를 요청한다(§11.3 ⑥).

> ★ **배치 응답의 `from`도 nullable이다**(`string[] | null`, `null` = 이동 전 미분류 — §8.2). 계약 예시가 전부 배열이라 또박또박이 `Array`로 타입을 고정하면 깨진다. `ddobak-W9`의 nullable 처리에 **이 필드도 포함**할 것(결정 §2-D).
...
| **W4** | `src/app/api/v1/minutes/route.ts:44-57` | `respondMinute`에 `folder_id`·`folder_path` 필드 추가. **두 필드 모두 nullable**(미분류 폴백 시 둘 다 `null` — §3.3 ✅ 확정). **＋ `folder_path_status`**(`exact`\|`truncated`\|`partial`\|`unclassified` — 결정 §2-C) | ✅ |
| **W5** | `src/app/api/v1/minutes/route.ts:86-95` | `handleExisting`의 `metadata`에 `folder_id` 추가. **`folderPathProvided`일 때만** 갱신, 부재면 기존 값 유지(§3.1 3값 표). **DB 변경 불필요** — `update_minute_metadata_with_wiki_retraction`의 allowlist에 `folder_id`가 이미 있고(`0045:1167`) 적용 로직도 있다(`0045:1206-1208, 1321`).<br>⚠️ **`W25` 플래그가 `false`면 이 갱신은 일어나지 않는다** — 결정이 “진짜 위험”으로 지목한 경로다(§4 W25). ＋ **키 부재 + team만 바뀐 재전송은 새 팀 루트로 이동**(§12) | ⚠️ D1 |
| **W6** | `src/app/api/v1/minutes/folder/route.ts` **(신규)** | **일괄 마이그레이션 엔드포인트** — 기존 회의록 재편철용. 상세 §8. **`W25` 플래그와 무관하게 항상 활성**이며 **`pmo_admin` 전용**(§8.2-14) | ✅ |
| **W7 → W19·W20a** | — | 트리 피커 교체 권고는 **§6 폴더 중심 재편에 흡수**됐다(담당·하위 구분 삭제 + D&D). 아래 W18~W23 참조 | — |
| **W8** | `docs/design/dflow-minutes-upload-api-spec.md` | 계약 개정 — 범위 **§3·§8·§9**(§3만 아님). 또박또박 측 사본과 동기화. 아래 ⚠️ W8 블록 참조.<br>✅ **완료** — v2.3 커밋(`4387576`). 결정 §2-E의 9건을 얹은 **v2.4**를 또박또박에 한 번에 송부한다(중간 개정 이력을 만들지 않는다) | ✅ **착수 전 선행** |
...
| **W24** ★신설 | `src/app/api/v1/minutes/route.ts` | **보관 상태 구분 노출** — `GET /minutes`에 `include_archived`(기본 `false` = 종전 동작) + 응답 `items[]`에 `archived: boolean` **상시** 포함. §9.7 (a) 채택분(결정 §2-B).<br>⚠️ **`W10`이 아니라 `R1`에 포함된다** — `ddobak-W14`를 4차까지 기다릴 이유가 없다 | ✅ |
| **W25** ★신설 | `src/lib/minutes/externalApi.ts` (`folderPathEnabled()`) | **서버 플래그 `MINUTES_FOLDER_PATH_ENABLED`**(기본 `false`) — `POST /minutes`의 `folder_path` 편철 전환 스위치(결정 §2-A). 아래 ⚠️ W25 블록 | ✅ |

> **`W24`·`W25`는 「신규 엔드포인트는 1개뿐」(§5) 방침과 충돌하지 않는다** — `W24`는 **기존 라우트의 쿼리 파라미터·응답 필드**, `W25`는 **env 플래그**다. 라우트는 여전히 `POST /api/v1/minutes/folder` 하나만 늘었다.

### ⚠️ W25 — 서버 플래그 `MINUTES_FOLDER_PATH_ENABLED` (결정 §2-A)

전환 시점을 **D'Flow가 쥐기 위한** 스위치다. 코드는 5줄이지만 배포 순서 전체가 여기에 걸려 있다.

| 대상 | R1 (플래그 `false` · 기본) | R2 (플래그 `true`) |
|---|---|---|
| `POST /minutes`의 `folder_path` | **완전히 무시** — 키 부재와 **동일**. **검증 400조차 나지 않는다** | §3.1·§3.2대로 편철 |
| 응답 `folder_id`·`folder_path`·`folder_path_status` | 실제 편철 위치(=팀 루트)를 **정직하게 에코** | 실제 편철 결과 |
| 재전송(`replace`)의 폴더 갱신(`W5`) | **일어나지 않음** | 일어남 |
| 배치 `POST /minutes/folder`(`W6`) | **활성** | 활성 |
| `include_archived`·`archived`(`W24`) | **활성** | 활성 |

- **왜 필요한가 — 진짜 위험은 `W3`가 아니라 `W5`다.** `folder_path`가 실려 오기만 하면 재전송 1건마다 D'Flow 위치가 덮이고, 또박또박에서 폴더에 안 들어 있는 회의는 `[]`를 보내므로 **사람이 정리한 편철이 팀 루트로 평평화**된다(실측 17건 — 결정 §1-②). `overwrite_manual`은 **배치에만** 걸리는 플래그라 이 경로를 못 막는다.
- **`W1`만 먼저 내면 오히려 회귀가 생긴다.** `W1`의 검증은 편철과 무관하게 파싱 시점에 400을 낸다(폴더명 61자 이상·타 팀 팀코드 루트). 또박또박의 사전 차단(`ddobak-W4`)은 2차로 옮겨졌고 또박또박은 폴더명을 100자까지 허용하므로, **오늘 정상 전송되는 회의가 400으로 실패하는 창**이 열린다. 플래그가 그 창을 닫는다.
- **부수 이득**: `ddobak-W1`(payload에 `folder_path` 조립)이 이미 프로덕션에 나갔는지 D'Flow는 **관측할 수 없다**(구버전 파서가 미지 키를 조용히 무시했으므로 로그·DB에 흔적이 0). 플래그가 있으면 **그 답을 몰라도 안전하다.**
- **`true`로 돌리는 조건(셋 다)**: ① 재편철 1회차 APPLY 완료 ② 19건 대조표 정리 완료(결정 §4) ③ 또박또박 2차 배포 준비 완료 통보.
- ⚠️ **또박또박 2차(제목 접두 제거)는 반드시 플래그 `true` 이후**다. 뒤집히면 그 구간 전송분이 “접두도 없고 폴더도 안 잡힌” 상태가 된다(§11.3 ① ⓐ가 경고한 그 상태).
...
- 신규 엔드포인트는 **`POST /api/v1/minutes/folder` 1개뿐**(§8). 그 외 추가 없음
  - ⚠️ **각주(결정 이후에도 이 문장은 참이다)**: 뒤에 신설된 `W24`·`W25`는 라우트가 아니다 — `W24`는 **기존 `GET /api/v1/minutes`의 쿼리 파라미터(`include_archived`)와 응답 필드(`archived`)**, `W25`는 **env 플래그(`MINUTES_FOLDER_PATH_ENABLED`)**다. 늘어난 라우트는 여전히 배치 1개다
- **`GET /minutes`의 기본 동작을 바꾸지 않는다** — `include_archived`는 기본 `false`라 또박또박이 아무것도 고치지 않아도 오늘과 동일하게 동작한다(하위호환). 대신 그 무수정 상태에서는 §9.7의 오진도 그대로 남는다(§9.7 「실효는 또박또박 호출부 3건에 달려 있다」)
...
- [ ] 회의록 D&D 다른 팀으로 이동 → `team_code` 갱신 **&&** **검색 인덱스(`ai_documents`) 재적재** 발생
  > **정정(2026-07-28, Q2)** — 원문은 「위키 철회·재빌드 발생」이었으나 **현재 스키마로 달성 불가**라 정정했다.
  > `wiki_rebuild_required`는 `old_project_id` 변경 또는 시간축 변경으로만 서고 `team_code`는 그 조건에 없다.
  > `team_code`는 `v_index_content_changed`에만 들어가 **검색 인덱스 재적재**를 일으킨다(운영 DB 실측).
  > 팀이 바뀌어도 **본문과 프로젝트가 그대로면 위키 근거는 변하지 않으므로** 재빌드가 옳은 동작도 아니다.
  > 구현은 raw update 가 아니라 **RPC 경유**로 되어 있다 — raw 면 `ai_documents`가 옛 팀으로 남아 검색이 어긋난다.
...
> ✅ **전건 해소(2026-07-27).** ① 0043 = **적용 확인 완료**(D'Flow 개발 측이 운영 DB에 읽기 전용 SELECT 실행 — 시드 루트 `PMO(0)·ERP(1)·MES(2)·가공(3)·MDM(4)`, `created_by` 전부 `null`. **미분류 회의록 0건** · 사용자 루트 폴더 0건 = §6.3 불변식 이미 성립. 결정 §5). ② `W8` 계약 개정 = 완료(v2.3, 송부본 v2.4). ③ **⚠️ 결정 필요 3건 전부 확정** — §3.3(`folder_path: null`) · §8.3(**조상 규칙**) · §9.7(**(a) 채택**). 아래 원문은 이력으로 남긴다.
...
- ~~**§8.3의 ⚠️ 결정 필요 · §9.7의 ⚠️ 결정 필요 · §3.3의 ⚠️ 확정 필요** 3건은 구현 중 자연히 결론이 나지 않는다. **팀장 판단이 필요하므로 착수 시점에 함께 올릴 것**~~ → ✅ **올렸고 확정됐다**: §3.3 = `folder_path: null`(＋`folder_path_status` 신설) · §8.3 = **조상 규칙**(결정 §2-J) · §9.7 = **(a) `include_archived` 노출**(`W24`)
- **또박또박 측 조치 요청 7건(§11.3)** 도 팀장 조율 대상이다. 특히 ①·⑦(차수 재배치)·⑥(응답 타입 nullable)은 이 문서의 계약과 직접 맞물린다
  - ~~②(ddobak-W8 필수 승격)~~ → ✅ **확정됨**(§11.3 ②). 조율 대상이 아니다 — v2.4 §4.3 `folder_path_status` 배지로 필수화됐다
...
기존 `/api/v1/minutes*` 관례를 그대로 따른다: `gateMinutesApi`(env 2단 + Bearer 시크릿) → `parseUserEmail` → `resolveUserByEmail`(403 `unknown_user`) → **`resolveUserRole` = `pmo_admin` 게이트(403 `forbidden_role`)** → `createAdminClient`.

> ★ **`pmo_admin` 게이트는 결정 §2-H로 신설됐다.** 다른 라우트의 계정 게이트는 `auth.users` **실재만** 확인하고 role을 보지 않는다 — 즉 `ACTOR_EMAIL`에 오타가 나 **실재하는 다른 직원**을 가리키면 배치는 정상 성공하고 그 사람이 생성 폴더 트리의 **유일한 관리자**가 된다(되돌리려면 DB 직접 수정). `items: []` 프로브(요건 9)도 이 게이트를 통과해야 하므로 **오설정이 첫 호출에서 드러난다.** role 조회 실패는 **fail-closed**(403) — 보안 가드이므로 “모르면 통과”가 아니다.
>
> 이 라우트는 `POST /minutes`의 **W25 플래그와 무관하게 항상 활성**이다(결정 §2-A). R1에서 새로 열리는 것은 이 배치와 `include_archived` 둘뿐이다.
...
// 응답
{
  ...
  "results": [
    { "external_id": "ddobak:018f…", "status": "moved",
      ...
      "folder_path_status": "exact" },                 // ★ 신규(§3.3 · 결정 §2-C)
    { "external_id": "ddobak:018f…", "status": "moved",
      "from": null, "to": ["MES","품질"], "folder_id": "9f3c…",   // ★ from 은 nullable — null = 이동 전 미분류
...
> ★ **`from`은 `string[] | null`이다**(결정 §2-D). `null` = 이동 전 미분류(`folder_id is null`). 예시가 전부 배열이라 또박또박이 `Array`로 타입을 고정하면 런타임에서 깨진다 → `ddobak-W9`의 nullable 처리 대상에 **이 필드도 포함**할 것.
>
> ★ **`folder_path_status`는 `moved` 결과에 실린다**(값 집합·판정 순서는 §3.3). **dry run에서 특히 중요하다** — 절단(`truncated`)을 APPLY 전에 보여야 또박또박이 경로를 줄일 기회가 있다. APPLY 후에는 절단본이 실재 폴더가 되어 **영구히 `already_correct`**로 굳는다(결정 §2-C).
...
11. **`status` 값 집합을 응답 예시가 아니라 계약으로 못박는다** — 구현자가 예시에서 유추하지 말 것:
    ...
    | `failed` | `validation_failed: <사유>` | `folder_path` 타입·§3.2 ③·`items[].team` 형식 등. ★ **요청 전체 400이 아니라 건별 `failed`다**(요건 12) |
    | **`failed`** | **`no_team_root`** | ★ **신규.** 정규화 후 `path[0]`의 시드 루트가 없어 §3.2-5 `null`(미분류) 폴백이 발동한 경우 |
    | **`failed`** | **`folder_error: <경로> 생성 실패`** | ★ **신규.** APPLY 중 목표 경로의 중간 폴더를 만들지 못함. **조상에 떨구지 않는다** — 리포트(`to`)와 실제 트리가 어긋나면 안 되므로 이동 자체를 하지 않는다 |
    | **`failed`** | **`update_failed: <사유>`** | ★ **신규.** 판정·폴더 확보까지 성공했는데 `minutes` 갱신이 실패 |
...
12. ★ **건별 검증 실패는 요청 전체를 400으로 떨구지 않는다** (결정 §2-E ⑥ — 종전 구현의 결함 수정). 200건 중 1건의 `folder_path` 오타로 나머지 199건이 함께 죽으면 “부분 실패는 전체를 롤백하지 않는다”(요건 7)와 어긋난다.

    | 층 | 무엇이 걸리나 | 결과 |
    |---|---|---|
    | **봉투(envelope) 400** | `items`가 배열이 아님 · **200건 초과** · 원소가 객체가 아님 · `items[].external_id` 부재/공백/128자 초과 · `dry_run`·`overwrite_manual` 타입 오류 | 요청 전체 **400** |
    | **건별 `failed`** | `items[].folder_path` 타입·원소 타입·빈 이름·**60자 초과** · §3.2 ③(타 팀 루트) · `items[].team` 형식 오류 | 그 건만 `failed(validation_failed: …)` / `failed(folder_name_too_long: …)`, `from`은 채워 보고 |

    `external_id`가 **봉투 오류인 이유**: 그것이 건별 보고의 **키**다 — 없으면 “어느 건이 실패했는지” 말할 수 없어 건별 보고가 성립하지 않는다.

13. ★ **판정 전에 폴더를 만들지 않는다.** 판정 단계의 경로 해석은 `create: false`로 돈다 — APPLY라고 여기서 만들면 **뒤이어 `skipped(manual_placement)`로 건너뛸 건의 목표 트리까지 실제로 생성**돼, 아무 회의록도 들어가지 않는 **빈 폴더가 ACTOR 명의로 트리에 남는다.** 생성은 **“이동한다”가 확정된 뒤**에만 일어난다.
    - 판정에 생성이 필요 없는 이유: **조상 규칙(§8.3)은 이미 존재하는 조상 체인만** 보면 되기 때문이다.
    - **파생 규약 — dry run의 `moved`**: 목표 폴더가 아직 없으면 `folder_id: null` + `to: <목표 경로>`로 보고한다. **`folder_id: null`을 “미분류”로 읽지 말 것** — dry run은 폴더를 만들지 않으므로 id가 아직 없을 뿐이다(미분류는 `status: failed(no_team_root)`로 따로 나온다).

14. ★ **`pmo_admin` 게이트**(§8.2 머리말 · 결정 §2-H) — role 미달이면 **403 `forbidden_role`**, role 조회 실패도 **fail-closed 403**. 이 게이트는 페이로드 검증보다 **먼저**이므로 `items: []` 프로브(요건 9)에도 그대로 걸린다.

### 8.3 이미 수동으로 옮긴 회의록 (`overwrite_manual`) — 판정은 **조상 규칙**이다

> ✅ **결정 §2-J로 확정(2026-07-27).** 아래 「⚠️ 결정 필요」는 **해소**됐다. 이지선다로 올렸던 (a)/(b) 중 어느 쪽도 단독 채택이 아니고, **(c) 조상 규칙 + (b) 순서 교체(취지) + `W25` 플래그** 셋을 함께 채택했다. 경위는 이 절 끝 「해소 경위」.
...
**판정 축이 “현재 위치가 팀 루트냐”에서 “현재 위치가 목표 경로의 조상이냐”로 바뀌었다.**

| 현재 `folder_id` | 해석 | `overwrite_manual: false`(기본) |
|---|---|---|
| 목표 위치와 **동일** | 이미 정답 | `already_correct` — **표 전체보다 먼저** 판정한다(§8.2-10) |
| `null`(미분류) | 아직 편철 안 됨 | **이동** |
| 목표 경로의 **조상** (팀 루트는 그 **특수 케이스**일 뿐이다) | 사람이 옮긴 것과 아직 덜 편철된 것이 동시에 참 — **더 깊게 넣는 것은 사람의 정리를 훼손하지 않는다** | **이동** |
| 그 외 — 형제·자손·무관한 가지 | 사람이 **다른 가지로** 옮겼다 | **skip** (`reason: "manual_placement"`) |

구현: `ancestorIdsOf(snap, 목표 folderId).has(현재 folder_id)` (`src/lib/minutes/folders.ts`) — **이미 존재하는 조상 체인만** 본다. 판정에 폴더 생성이 필요 없다는 것이 §8.2 요건 13(판정 전 미생성)의 전제다.

| 현재 위치 | 목표 경로 | 판정 |
|---|---|---|
| `MES` (팀 루트) | `MES/물류/2026-07` | **이동** — 팀 루트도 조상이다(종전 2행과 결과 동일) |
| `MES/물류` | `MES/물류/2026-07` | **이동** — 실측 17건의 상당수가 이 형태다(결정 §2-J) |
| `MES/조업및표준화` | `MES/기타/…` | **skip** — 다른 가지. 사람의 정리를 보호한다 |
| `MES/물류/주간` | `MES/물류` | **skip** — 얕은 쪽으로 되돌리는 이동도 **조상이 아니다.** 배치가 트리를 평평하게 만들지 않는다 |
| `MES/물류` | `MES/물류` | `already_correct` |

`overwrite_manual: true`면 전부 이동. **dry run으로 `manual_placement` 건수를 먼저 확인한 뒤 결정**할 것.

> ⚠️ **조상 규칙 적용 후에도 몇 건은 정상적으로 남는다** — 사람이 다른 가지로 옮긴 건이다. 그 건수를 **장애로 읽지 말 것이며, `OVERWRITE_MANUAL`을 켜는 근거로도 쓰지 말 것**(켜면 사람이 정리한 17건을 덮는다). 판단은 **19건 사람 대조**(결정 §4-③)로만 한다.

#### 해소 경위 — 왜 (a)도 (b)도 단독 채택이 아닌가 (종전 「⚠️ 결정 필요」 블록을 대체)

**문제 인식 자체는 유효하다.** 종전 2행 “팀 루트 = 외부 API가 자동 편철한 자리”는 `insertNew`가 `resolveTeamRootFolderId`로 팀 루트에만 떨구던 세계의 명제이고, **W3이 그것을 `resolveFolderPath`로 바꾸면** 신규 전송이 하위 폴더에 직접 편철된다 → 그 순간부터 “**하위 폴더에 있지만 사람이 옮긴 게 아닌**” 회의록이 존재해 3행의 “사람이 옮겼을 가능성” 추론이 무너진다. 구체 시나리오: `MES/품질/주간정례`에 자동 편철된 회의를 또박또박에서 `MES/품질/월간`으로 옮기고 **재전송은 안 한 채** 재편철을 실행 → 하위 폴더라서 skip → D1=B(또박또박이 SSOT)가 그 건에만 적용되지 않는다.

결정이 바꾼 것:

| 종전 선택지 | 결정 |
|---|---|
| **(a)** 판정 기준 교체 — “마지막 전송 이후 D'Flow 쪽 이동이 있었는지” | **미채택.** 이동 시각·주체 흔적이 없어 스키마 또는 이력 테이블이 필요하다 |
| **(b)** 순서 교체 — 재편철(3차)을 전송 전환(2차)보다 앞세움 | **취지 채택**(결정 §2-A). 단 “D'Flow `W6`를 `W1`~`W5`보다 먼저 낸다”는 **미채택** — `W1`~`W6`·`W24`를 한 배포로 내고 **`W25` 플래그**로 전환 시점을 쥔다(§4 W25) |
| **(c)** 조상 규칙 — 원안에 **없던 선택지** | ★ **채택**(결정 §2-J). **스키마 변경 0 · 이력 테이블 불필요 · 순서 교체 불필요**이면서 (a)가 노린 것을 대부분 얻는다 |

**왜 (b)만으로는 부족한가 — 전제가 실측상 거짓이다.** (b)의 논거는 “3차를 앞세우면 기존 전송분이 전부 팀 루트에 평평 → 판정표 2행 전제가 참 → **오분류 0**”이었다. 그러나 또박또박 연동 **19건 중 17건이 이미 2단 하위 폴더**에 있다 — **D'Flow 사용자가 직접 정리한 결과**다(결정 §1-②·§5). 순서 교체가 막는 것은 “2차 이후 새로 생길 자동 편철분의 오분류”뿐이고, **지금 존재하는 89%(17/19)는 순서와 무관하게 남는다.** 그 89%를 처리하는 것이 조상 규칙 + 19건 사람 대조(결정 §4)다.

**진짜 위험은 `W3`가 아니라 `W5`였다.** `overwrite_manual`은 **배치에만** 걸리는 플래그라 재전송 경로를 못 막는다 → 그 스위치를 D'Flow 손에 두는 것이 `W25`다(§4). 그래서 **19건 대조와 APPLY는 플래그 `true`(R2) 전에 끝내야 한다.**

> §11.3 ⑤(‘`ddobak-W3` 선배포 금지’ 근거 정정)가 지목한 “같은 결정”이 바로 이것이다 → **§11.2 차수표 개정 완료.** 두 절을 별개의 미결 항목으로 읽지 말 것.
...
### 8.5 ~~대안 — 또박또박 없이 D'Flow 단독 SQL~~ → **강등(실행하지 않는다)**

> ⛔ **결정 §1-② 실측으로 이 대안은 폐기 수준으로 강등한다.** 아래는 이력으로 남긴다 — **실행 후보로 읽지 말 것.**
>
> **왜 불필요한가**: 이 대안은 “기존 전송분이 팀 루트에 평평하게 쌓여 있고 제목 접두만이 유일한 계층 단서”라는 세계를 가정한다. **실측은 반대다** — 또박또박 연동 19건 중 **17건이 이미 하위 폴더에 정리돼 있고**(팀 루트 잔류는 2건), 그 편철 위치는 **제목 접두와 일치하지도 않는다**(`기타-표준화(우봉우)…` → 실제 `조업및표준화`, `APS-형강인터뷰` → 실제 `생산계획`). 즉 접두를 역파싱하면 **사람이 정리해 둔 위치를 접두 기준으로 되돌려 놓는다** — 복구가 아니라 훼손이다.
>
> **왜 위험만 남는가**: (i) 접두는 2단째 하나뿐이라 3단 이상 복원 불가 (ii) 제목에 원래 `-`가 있으면 오파싱 (iii) 접두와 실제 위치의 불일치를 판별할 방법이 없다 (iv) `ddobak-W6`(접두 제거) 이후 전송분은 **제목에 단서가 0**이라 애초에 적용 불가.
>
> **정본은 §8.2 배치다.** 또박또박이 응답하지 않는 상황이라도 이 SQL로 우회하지 말고, **19건 규모에서는 사람 대조**(결정 §4)가 더 싸고 정확하다.
...
> 파급 1건: §11.3 ①이 “그 구간 분은 §8.5의 D'Flow 단독 복구책이 원리적으로 적용 불가해진다”를 `ddobak-W6` 차수 이동 논거의 **보조**로 들었는데, §8.5가 강등됐으므로 **그 보조 논거는 무효**다. ①의 주논거(“접두도 없고 폴더도 안 잡힌 상태”)는 그대로 유효하다.
...
### 9.7 `exists_on_dflow: false`의 archived 오진 — ✅ **(a) 채택 · `W24`로 구현 완료**
...
**~~⚠️ 결정 필요~~ → ✅ (a) 채택 (결정 §2-B) — 해소 선택지 2개는 아래 이력:**

| 안 | 내용 | 비용·성격 | 결정 |
|---|---|---|---|
| **(a) D'Flow가 보관 상태를 구분 노출** | `GET /minutes`에 `include_archived=true`(또는 `archived=only`) 파라미터를 추가하거나, `external_id` 정확 조회에서는 archived 필터를 걸지 않고 응답에 `archived: true`를 실어 준다. 또박또박은 “초기화됨” 대신 **“D'Flow에서 보관됨 — 복원 후 다시 시도”**를 안내할 수 있다 | D'Flow 코드 변경 필요(§5의 “신규 엔드포인트 1개뿐” 방침과 충돌하지 않음 — 기존 라우트 파라미터 추가). **원인을 없애는 안** | ✅ **채택** |
| **(b) 또박또박 안내 문구 완화** | `exists_on_dflow: false`를 **“초기화됨”으로 단정하지 않고** “D'Flow에서 확인되지 않습니다(초기화·보관·삭제 중 하나)”로 바꾸고, 복구 갈래에 **“D'Flow에서 보관 해제 확인”**을 추가 | D'Flow 변경 0, 문구만. **오진을 못 없애고 사용자에게 전가하는 안** | 문구 부분만 `ddobak-W14`로 **병행**(§11.1) |

~~권고: **(a)**~~ → **채택: (a).** (b)만 하면 자동 링크의 오분류(위 부수 영향)는 남는다.

#### 구현 결과 (`W24` · §4 표)

- `GET /api/v1/minutes`에 **`include_archived`** — 기본 `false` = **종전 동작 그대로**(하위호환). `true`면 보관분 포함. 값이 `true`/`false`가 아니면 400.
- 응답 `items[]`에 **`archived: boolean` 상시 포함** — `include_archived` 값과 **무관하게 항상 실린다**(기본 조회에서는 늘 `false`). 클라이언트가 분기 필드의 **유무**를 조건부로 다루지 않게 하기 위해서다.
- 범위 초과 페이지의 `PGRST103` 폴백 카운트 쿼리에도 **같은 필터**를 적용한다(한쪽만 빠지면 total이 어긋난다).
- ⚠️ **`W10`이 아니라 `R1`에 포함된다** — 즉 `ddobak-W14`를 4차까지 기다릴 이유가 없다(§11.2).

#### ⚠️ 그러나 실효는 **또박또박 호출부 수정 3건**에 달려 있다 — 없으면 죽은 기능이다

`include_archived`는 기본 `false`이므로, 또박또박이 **아무것도 고치지 않으면 오진은 그대로 남는다**(문구만 고친 (b)안과 결과가 같아진다).

| # | 또박또박 조치 | 없으면 |
|---|---|---|
| ① | `dflow/status`의 존재 확인(`list_minutes(external_id:)`) 호출에 **`include_archived=true`** | `exists_on_dflow: false` 오진이 **그대로 남는다** |
| ② | 자동 링크 감지의 **`GET /minutes?linked=true` 페이지 순회에도 `include_archived=true`** + `archived: true` 행은 “존재함(보관)”으로 처리해 **차집합에서 제외** | `RELINK_RESET=1` 경로의 보관분 오분류가 남는다. “보관분을 후보에서 제외”는 **보이게 하는 단계가 먼저** 있어야 성립한다 — 기본값에서는 보관분이 목록에 아예 없어 **제외할 대상으로 인식되지도 않는다** |
...
> **예비안(또박또박이 호출부를 못 고칠 때만)**: `external_id` **정확 조회에 한해** archived 필터 기본 해제. 그러면 또박또박 무수정으로 `exists_on_dflow: true`가 되고, [전송] 시 D'Flow가 409 `archived` + “보관된 회의록입니다. 복원 후 다시 시도하세요.”로 이미 정확히 안내한다. **필요하면 회신하라** — 계약 개정으로 공지하고 반영한다(결정 §2-B).
...
- [ ] **(§3.3 ✅ 확정)** 시드 루트 부재로 미분류 폴백 → 응답이 `folder_id: null` **＋ `folder_path: null`**(`[]` 아님) **＋ `folder_path_status: "unclassified"`**. 또박또박 다이얼로그가 “팀 루트”라고 말하지 않음
- [ ] **(W25 · 플래그 `false`)** `folder_path`를 실어 보내도 **키 부재와 동일** — 팀 루트에 편철되고 **400이 나지 않는다**. **61자 폴더명이 섞인 요청도 200**(검증 자체를 하지 않음). `replace`의 폴더 갱신(W5) **미발생**
- [ ] **(W25 · 플래그 `false`)** 그 상태의 응답 `folder_path`가 **실제 편철 위치**(`["MES"]`)다 — 요청 경로를 되돌려주지 않음
- [ ] **(W25 · 플래그 `true`)** 위 항목들이 §3.2대로 편철로 전환됨. 배치(§8)·`include_archived`(§9.7)는 **플래그와 무관하게 양쪽에서 동작**
- [ ] **(§3.3 `folder_path_status`)** 6단 경로 → `truncated` / 정상 한 칸 내림 → `exact` / 시드 루트 부재 → `unclassified`. 심각한 쪽 우선 판정
- [ ] **(W24)** `GET /minutes?external_id=…` 기본 호출은 보관분 **0건**, `include_archived=true`면 **1건 + `archived: true`**. `archived` 필드는 **기본 조회에도 항상 실린다**
- [ ] **(결정 §6)** 비활성 팀 회의록 재전송 → **400 `team_inactive`** + 명시 사유 (500 아님)
- [ ] **(결정 §6)** `folder_path` **키 부재** + `team`만 바뀐 재전송 → 폴더가 **새 팀 루트로 이동**(팀은 ERP인데 폴더는 MES 서브트리인 상태를 남기지 않음). 새 팀 루트가 없으면 폴더 유지 + 로그
- [ ] **(C7 · NFC)** NFD 한글 폴더명으로 전송 → 기존 NFC 폴더에 **편철되고 동명 폴더가 새로 생기지 않음**. UI 생성·개명도 동일
- [ ] **(결정 §6 폴더 삭제 가드)** 회의록·하위 폴더가 든 폴더 삭제 → 부모로 **승격 후** 삭제(미분류 강등 없음, cascade 아님). 상위에 동명이 있으면 **중단 + 안내**
...
- [ ] **(조상 규칙 §8.3)** `overwrite_manual: false`에서 **목표 경로의 조상이 아닌** 하위 폴더에 있던 회의록은 `skipped(manual_placement)` (목표 위치와 같으면 `already_correct` — §8.2-10)
- [ ] **(조상 규칙 §8.3)** 현재 `MES/물류` · 목표 `MES/물류/2026-07` → **`moved`**(조상이므로 이동)
- [ ] **(조상 규칙 §8.3)** 현재 `MES/물류/주간` · 목표 `MES/물류` → **`skipped(manual_placement)`**(얕은 쪽 되돌리기는 조상 아님 — 배치가 트리를 평평하게 만들지 않음)
- [ ] **(조상 규칙 §8.3)** 현재 `MES/조업및표준화` · 목표 `MES/기타/…` → **`skipped(manual_placement)`**(다른 가지 보호)
- [ ] **(요건 12)** 200건 중 1건만 `folder_path` 타입 오류 → 그 건만 `failed(validation_failed)`, **나머지 199건은 정상 처리**(요청 전체 400 아님)
- [ ] **(요건 12)** `items` 비배열 · 201건 · 원소가 객체 아님 · `items[].external_id` 부재 → **요청 전체 400**
- [ ] **(요건 13)** `dry_run: true` 실행 후 **`minute_folders` 행 수 불변** — skip될 건의 목표 트리가 생성되지 않음
- [ ] **(요건 13)** dry run에서 목표 폴더가 아직 없으면 `moved` + **`folder_id: null`** + `to: <목표 경로>` (이 `null`을 ‘미분류’로 읽지 않음)
- [ ] **(요건 14)** `pmo_admin`이 아닌 실재 계정 + `items: []` → **403 `forbidden_role`**(200 아님). role 조회 실패도 **403**(fail-closed)
- [ ] **(§2-D)** 미분류 회의록을 이동 → 결과의 **`from`이 `null`**(빈 배열 아님)
- [ ] **(§3.3)** `moved` 결과에 **`folder_path_status`**가 실린다(dry run 포함)
...
### 11.1 또박또박 작업 요약 (ddobak-W1~W19)

> 표제 정정: 결정이 **`ddobak-W18`(계약 사본 v2.4 동기화 · 0차)** 와 **`ddobak-W19`(2차 포함분)** 를 추가로 언급하므로 범위는 `~W17`이 아니라 **`~W19`**다(결정 §2-E·§3).
...
| **표시** `W8` | 성공 응답의 `folder_path` 표시(절단·한 칸 내림 인지). ~~현재 ‘권장’ → 필수 승격 요청~~ → ✅ **필수 확정**(결정 §2-C·§7-5). **＋ `folder_path_status` 배지 추가**(`truncated`·`partial`·`unclassified`일 때 눈에 띄게) — 사전 미리보기를 D5로 포기했으므로 **절단·부분 편철·미분류가 사용자에게 보이는 유일한 경로**다. 차수는 2차(§11.2) |
...
| **연결 해제 대응** `W14` | ~~`exists_on_dflow: false`면 “연결이 해제되었습니다” 배지~~ → ✅ **문구 정정(결정 §2-B ③)**: `exists_on_dflow: false`는 **초기화 전용 신호가 아니다** — 보관·삭제도 같은 값을 만든다. 배지는 **“D'Flow에서 확인되지 않습니다(초기화·보관·삭제 중 하나)”** 로 하고, 복구 안내를 **세 갈래**로 세분화한다: ① **D'Flow에서 보관 해제** 확인(신설 — 종전 두 갈래에 없던 진짜 해법) ② [D'Flow에서 찾기]로 재연결 ③ 새로 전송. ＋ [전송] 즉시 실행 차단은 유지.<br>⚠️ **판별은 문구가 아니라 데이터로 한다** — `include_archived=true` 조회에서 `archived: true`면 **“보관됨 — 복원 후 재시도”로 단정**할 수 있다(추측 불필요). 그 조회를 안 켜면(§9.7 ①) 문구만 완화된 채 오진이 남는다 |
| **계약 동기화** `W18` ★ | 또박또박 사본(`tasks/dflow-minutes-upload/artifacts/`, 현재 v2.1)을 **D'Flow 정본 v2.4로 교체**. **0차**(모든 차수의 참조 기준). 반대 방향 동기화 금지 — 정본은 wbs-web 사본이다(결정 §2-E) |
| **2차 포함분** `W19` ★ | 전송 전환 차수에 함께 나가는 또박또박 측 잔여 항목. 차수 배치는 §11.2(2차) |
...
> ✅ **확정(결정 §2-H·C-1)**: `ACTOR_EMAIL` = **`donseok75@gmail.com`**(실명 담당자, `auth.users` 실재 · `pmo_admin` 확인 완료). 전용 서비스 계정은 채택하지 않는다.
> ★ **실재만으로는 부족하다 — `pmo_admin`이어야 한다**(§8.2-14). 배치가 만드는 폴더의 `created_by`는 **ACTOR 명의를 유지**하므로(“이 폴더는 일괄 재편철이 만들었다”는 유일한 감사 표식), 오타로 **실재하는 다른 직원**을 가리키면 그 사람이 생성 트리의 유일한 관리자가 된다. 게이트가 그 오설정을 **첫 프로브에서** 드러낸다.

### 11.2 배포 차수와 상호 의존 — ✅ **결정 §3 순서로 교체**

> ⚠️ **개정 전 이 절은 “순서를 바꾸지 말 것”이라 적혀 있었다 — 이번 결정과 정면 충돌한다.** 결정이 이긴다. **재편철(3차)이 전송 전환(2차)보다 앞선다**(결정 §2-A). 종전 표(또박또박 원문 차수표 인라인)는 이 절 끝 「종전 차수표(이력)」로 내린다.

#### 배포 순서 정본 (결정 §3)

```
[D'Flow] R1  계약 v2.4 + W1·W1-b·W2·W3·W4·W5·W6·W24 + W25(플래그) + 조상 규칙(§8.3)
             ＋ 폴더 삭제 가드(§12) ＋ 배치 400 경계 수정(§8.2-12) ＋ NFC 정규화(C7)
   ...
   ────── [양측] 19건 대조표 정리 (결정 §4)  ← 사람이 판단하는 유일한 구간
   ────── [또박또박] 재편철 1회차 APPLY
...
| 차수 | 항목 | 전제 조건 |
|---|---|---|
| **R1** (D'Flow) | 계약 v2.4 + `W1`·`W1-b`·`W2`~`W6`·`W24`·`W25` + 조상 규칙 + 폴더 삭제 가드 + NFC | 없음. **플래그 `false`로 배포** — `POST /minutes` 동작 무변경 |
| **0차** (또박또박) | `ddobak-W18` (계약 사본 v2.4 동기화) | 계약 v2.4 송부 |
| **1차** (또박또박) | `ddobak-W1` · `W9` · `W10` · `W11` · **`W14`** · **`W17`** | 없음 — 플래그 `false` 동안 `folder_path`는 무시되므로 `ddobak-W1` 배포 여부와 무관하게 **무해**하다.<br>`W14`·`W17`은 D'Flow 의존 0(§11.3 ⑦) · `W14`는 `W24`가 R1에 들어 있어 **1차에서 완결 가능**(§9.7) |
| **3차** (또박또박) — 폴더 재편철 | `ddobak-W12` · `ddobak-W13` — **dry-run 먼저** | **R1 배포 후**(배치 엔드포인트 필요). ★ **2차보다 앞선다** |
| **대조** (양측) | 19건 `from`/`to`/`status` 대조표 → `manual_placement` 건별 정답 판정 | 3차 dry-run 결과 (결정 §4) |
| **APPLY** (또박또박) | **또박또박 위치가 정답인 건만** `items`에 담아 APPLY | 대조 완료. **반드시 R2 이전** |
| **R2** (D'Flow) | **플래그 = `true`** (코드 배포 없음, env 전환 + 재배포) | ① 재편철 1회차 APPLY 완료 ② 19건 대조 완료 ③ 또박또박 2차 준비 완료 통보 |
| **2차** (또박또박) | `ddobak-W2` · `W3` · `W4` · `W5` · `W6` · `W7` · **`W8`** · `W19` | **반드시 R2 이후.** 뒤집히면 그 구간 전송분이 “접두도 없고 폴더도 안 잡힌” 상태가 된다 |
| **재편철 2회차** (또박또박) | `items`를 **창 구간 전송분만**(`dflow_synced_at` > 1회차 실행 시각)으로 한정 | 2차 이후. 전량으로 돌리면 남은 `manual_placement` 건이 매번 재보고돼 담당자를 `OVERWRITE_MANUAL=1`로 밀어붙인다 |
| **R3** (D'Flow) | `W10` 연결 초기화 + 연동 식별자 표시 | ⚠️ [연결 초기화] 버튼은 **`ddobak-W14` 배포 확인 전까지 `pmo_admin` 한정**으로 연다 |
| **4차** (또박또박) — 자동 링크 | `ddobak-W15` · `ddobak-W16` | **R3 이후.** 착수 조건 2개는 §12(자동 claim 금지 · 초기화분 제외) |
| **R4** (D'Flow) | `W18`~`W23` 폴더 중심 UI 재편(D&D) | **재편철 1회차 완료 후**(결정 C-2) — 먼저 내면 사용자 정리분이 늘어 3차가 전부 skip한다 |

#### 지켜야 할 순서 제약은 **3개뿐**이다 (결정 §3)
...
> 그 외의 “순서 강제”는 전부 `W25` 플래그가 흡수했다. 특히 **“D'Flow `W6`를 `W1`~`W5`보다 먼저 낸다”는 전달하지 않는다** — `W3`는 또박또박이 `folder_path`를 보내기 전까지 무동작이고, 분해하면 헬퍼가 미사용 코드가 되며(lint), `W1`만 먼저 내면 오히려 400 회귀 창이 열린다(§4 W25).

#### 종전 차수표 (또박또박 원문 인라인 · **이력**)

> ⛔ **아래는 결정 이전의 표다. 실행 근거로 쓰지 말 것** — 특히 “2차 → 3차” 순서와 마지막 강조 문장(“순서를 바꾸지 말 것”)은 결정 §2-A로 **뒤집혔다.**
...
> ~~⚠️ **강조: 3차는 D'Flow `W6`가 전제다. 4차 중 `ddobak-W15`·`ddobak-W16`은 D'Flow `W10`이 전제다**(`ddobak-W14`·`ddobak-W17`은 D'Flow 의존 0 — 아래 참조). 이 두 개(`W6`·`W10`)가 또박또박 마이그레이션의 실질 전부를 잠그고 있다. **순서를 바꾸지 말 것.**~~ → **폐기.** `W6`·`W10` 의존은 유효하되(R1·R3에 각각 포함), **“순서를 바꾸지 말 것”은 이번 결정으로 뒤집혔다** — 3차가 2차보다 **앞선다.**

#### D'Flow 개발자 관점 — 내 작업이 무엇을 잠금 해제하나

> ✅ **개정**: `W1`~`W6`·`W24`가 **한 배포(R1)** 로 나가므로 아래 표의 “작업별 잠금 해제”는 이제 **차수(R1/R2/R3)** 로 읽는다. 특히 `W1`~`W5`가 또박또박 2차를 여는 시점은 **코드 배포가 아니라 `W25` 플래그 `true`(R2)** 다.

| 내 작업 (D'Flow) | 잠금 해제되는 또박또박 작업 | 그전까지 또박또박은 |
|---|---|---|
| **`W25`** (플래그 · R1은 `false`) | **1차 전체 · 3차(재편철)** — 플래그가 `false`인 동안 `POST /minutes`가 무변경이라 **먼저 정리하고 나중에 전환**할 수 있다 | (플래그가 없었다면) `ddobak-W1` 배포 즉시 `W5`가 사람이 정리한 편철을 재전송 단위로 덮기 시작한다 |
| **`W24`** (`include_archived`·`archived` · R1) | `ddobak-W14`를 **1차에서 완결** 가능 | 보관을 “연결 해제”로 오진하고, 안내한 복구 두 갈래가 **둘 다 실패**한다(§9.7) |
| **`W8`** (계약서 개정 · **착수 전 선행**) | 전 차수의 참조 기준 | 자기 레포의 계약 사본(v2.1)에서 **이번 변경과 모순된 지시**를 읽는다(§4 ⚠️ W8) |
| **`W1`~`W5`** (`folder_path` 코어 + 응답 에코 + `replace` 동기화)<br>⚠️ **실제 개방 시점 = `W25` 플래그 `true`(R2)** | **2차**: `ddobak-W2`(접두 제거) · `ddobak-W3`(자유 루트) · `ddobak-W5`<br>표시·타입: `ddobak-W8`·`ddobak-W9`가 실효를 가짐 | 접두를 못 뗀다(떼면 “접두도 없고 폴더도 안 잡힌” 상태). 폴더 이동이 재전송으로 전파되지 않는다 |
| **`W6`** (`POST /minutes/folder` 배치 · **R1에 포함 · 플래그 무관**) | **3차**: `ddobak-W12` · `ddobak-W13` — ★ **2차보다 먼저 돈다** | 기존 전송분을 정리할 수단이 없다.<br>⚠️ 종전 문구 “팀 루트에 평평하게 그대로 남는다”는 **실측상 거짓**이다 — 19건 중 17건은 이미 사용자가 하위 폴더로 정리해 뒀다(결정 §1-②) |
...
> ✅ **결정 반영 상태**(결정 §7 「또박또박에 요청하는 것」과 짝을 이룬다):
> | # | 상태 |
> |---|---|
> | ① `ddobak-W4`·`W6`·`W7`을 2차로 | **요청 유지 · 차수 반영 완료**(§11.2 2차). 단 보조 논거였던 “§8.5 복구책 적용 불가”는 §8.5 강등으로 **무효** |
> | ② `ddobak-W8` 필수 승격 | ✅ **확정** — ‘요청’이 아니라 **필수**다. ＋ **`folder_path_status` 배지** 추가(결정 §2-C) |
> | ③ `dflow_synced_at.present?` 조건 추가 | **요청 유지**(결정 §7-9). B-4 승인만으로는 안 닫힌다 |
> | ④ C2 제목 비교 두 변형 | **요청 유지 · 최우선**(결정 §7-10) — 연동 19건이 전부 접두 있는 제목이라 문언대로 구현하면 **전건 0매칭** |
> | ⑤ `ddobak-W3` 선배포 금지 근거 정정 | **정정 확정 · 조치도 확정** — “3차 재편철로 정리”를 택했고, 그에 따라 **§11.2 차수표 개정 완료**(§8.3 해소 경위) |
> | ⑥ `ddobak-W9` nullable | ✅ **성립** — D'Flow가 `folder_path: null`로 확정했다. ＋ **배치 `from`도 nullable**(결정 §2-D) |
> | ⑦ `ddobak-W14`·`W17`을 1차로 | ✅ **반영 완료**(§11.2 1차). `W24`가 R1에 있어 `W14`는 1차에서 완결된다 |
...
## 12. 미해결·별건 (착수 전 인지) — ✅ **결정 §6으로 갱신**

이 문서에 규정하지 않았던 항목이다. **결정 §6이 전건에 답을 냈다** — 아래 「결정」 열이 정본이며, `해결됨`이 아닌 항목만 여전히 임의 판단 금지 대상이다.

| 항목 | 내용 | **결정 (§6)** |
|---|---|---|
| **깊이 절단 정책** | 60자 초과는 400 거절(D3)인데 깊이 초과는 **무통보 절단**이다. `신규TF/A/B/C/D1`·`…/D2`가 한 칸 내림 후 6단이 되면 둘 다 `MES/신규TF/A/B/C`로 병합되는데 에러가 없다 — D3가 막으려는 조용한 병합이 깊이 축에서 재현된다. 이 문서는 절단(현행)을 규정하고 최소 완화(응답 에코 표시)만 §11.3 ②로 요청했다. ~~**정본 확정 필요**~~ | ✅ **정본 확정 — 절단 유지 ＋ `folder_path_status` 신호**(§3.3).<br>“D3와 일관되게 400”은 성립하지 않는다: 60자 절단은 또박또박에 **존재하지 않는 이름**의 폴더를 만드는 **위조**지만, 깊이 절단은 사용자가 **실제로 만든 조상 폴더**에 들어가는 **해상도 저하**이고 `team_code` 파생도 안 깨진다(root 세그먼트 보존).<br>⚠️ **별건 1건 유지**: 같은 앱 안에서 깊이 정책이 **3중으로 갈린다**(UI = 버튼 숨김 / 서버 액션 = 거절 / 외부 API = 절단). `W21`(폴더 D&D)에서 다시 터지므로 통일은 별건 티켓.<br>⚠️ **미측정 값 1개**: 또박또박 폴더 깊이 5단 이상 분포(결정 §7-2). 0에 가깝고 사전 경고가 어렵다면 400 전환 재검토 |
| **폴더 삭제 가드** (§6) | §6.3 불변식이 **삭제로 재파괴**된다. 외부 전송이 만든 폴더는 `parent_id not null`·`created_by=전송자`라 `isTeamRootFolder` 보호 밖 → 일반 사용자가 삭제하면 cascade + `folder_id set null` → 미분류 재발 | ✅ **해결됨 — R1에 포함(별건 아님).** 전면 금지가 아니라 **「비우기 우선」**: 삭제 시 자식 폴더·소속 회의록을 **부모로 승격**시킨 뒤 삭제한다(`deleteMinuteFolder`, `src/app/actions/minutes.ts`). 기존 UX 유지·스키마 무변경.<br>가드: 상위에 **동명이 있으면 중단**(조용히 cascade로 지우지 않는다) · 회의록 승격 시 **`updated_at` 무갱신**(0043·배치와 같은 규칙) · 권한은 **승격 전에 RLS와 같은 조건으로 선판정**(옮겨만 놓고 삭제가 막히는 상태 방지) |
| **오매칭 후 본문 복구** | “초기화 = 되돌리기 수단”이 되돌리는 것은 **연결**이지 **본문**이 아니다. 오매칭 `replace`가 남의 본문을 덮으면 원본은 `minute_versions`에 남지만 D'Flow 버전 패널에 **되돌리기 액션이 없다** | **별건 유지 · 단 4차 착수 조건에 건다** — 자동 링크 v1은 **자동 claim을 끄고 dry-run 리포트 + 사람 승인만** 허용한다(미연결 21건 규모라 비용이 거의 없다). 되돌리기 수단이 없는 상태에서 자동 claim을 켜지 않는다 |
| **배치 폴더의 `created_by`** | 재편철(§8)로 생성되는 폴더의 소유자가 `ACTOR_EMAIL` 계정이 된다. 전용 서비스 계정으로 돌리면 그 트리 전부를 일반 사용자가 개명·삭제할 수 없다. ~~**어느 계정으로 돌릴지 사전 합의 필요**~~ | ✅ **확정 — ACTOR 명의 유지 ＋ `pmo_admin` 게이트**(§8.2-14).<br>`created_by = ACTOR`는 “이 폴더는 일괄 재편철이 만들었다”는 **유일한 감사 표식**이라 item별 소유로 흩지 않는다. 심각도도 재평가: 사용자 **41명 중 28명이 `pmo_admin`**이라 손댈 수 없는 인구는 13명이고 요청 경로가 있다(결정 §1-③).<br>⚠️ **별건 티켓 등록**: 폴더 권한을 `created_by`에서 분리 |
| **구버전 `replace`의 team 불일치** | 키 부재 경로에서도 `team_code`는 metadata에 항상 실려 갱신된다 → 구버전 클라이언트가 team만 바꿔 재전송하면 `team_code=ERP` + `folder_id`는 MES 서브트리 = **§6.4가 D&D에서 금지한 상태를 외부 API가 정상 경로로 만든다.** ~~W5 처리 규정 대기~~ | ✅ **해결됨 — 새 팀 루트로 폴더를 옮긴다.** `folder_path` **키 부재 + `team_code`만 바뀐** 재전송은 `resolveTeamRootFolderId(새 팀)`로 폴더를 이동시킨다(`route.ts` `handleExisting`). 400 거절은 구버전 클라이언트의 **정상 조작(담당 정정)**을 막고, 현행 유지는 불일치 데이터를 남긴다. 새 팀 루트가 없으면 **폴더 유지 + 로그**(미분류로 빼내지 않는다) |
| **zip 그룹핑 분할** | 접두 제거의 부수영향은 “그룹 이름 변경”이 아니라 **분할**이다. `meetingBodyOf`는 `_`·공백만 토큰 구분자로 쓰므로 하이픈 접두가 살아남아 `품질-주간회의`와 `주간회의`가 다른 group이 된다 → 같은 시리즈가 두 폴더로 쪼개진다(데이터 손실 없음). D6 이후 zip 그룹핑이 `meetingBodyOf`의 **유일한 잔존 소비자**다 | **수용 · 별건 유지**(데이터 손실 없음). ⚠️ 해결책을 **“제목에서 하이픈 접두 제거”로 가면 안 된다** — 정상 하이픈 제목을 훼손한다. §6 재편 이후 **그룹핑 축을 제목에서 폴더로 교체**하는 별건 티켓 |
| **초기화분이 후보로 잔존** | 또박또박 자동 링크 가드는 ‘그 회의를 다시 붙이지 않는다’(회의 축)일 뿐 초기화된 minute을 **후보 풀에서 빼지 않는다.** 같은 (날짜·team·정규화 제목) 회의가 2건이고 하나가 초기화됐다면 나머지가 exact+유일로 판정돼 **방금 사람이 끊은 회의록을 claim**한다 | **4차 착수 조건.** **D'Flow 추가 작업 0** — 또박또박 자동 링크가 “후보의 작성자가 또박또박 전송 계정이면 **자동 claim 금지**, 사람 승인으로 강등” 규칙을 넣는다 |
| **비활성 팀** (§10 수용기준) | `team_code`가 비활성 팀인 회의록의 재전송이 실패하는데 원인이 응답에 드러나지 않았다 | ✅ **사실로 종결 ＋ 400 매핑.** 비활성 팀은 메타 갱신 RPC가 `t.active`를 요구해 **반드시 실패**하고 `insertNew`도 활성 팀 검증에서 걸린다 = **정상 설계**다. 다만 500이면 원인 불명 장애로 보이므로 **400 `team_inactive` + 명시 사유**로 매핑했다(`MINUTE_TEAM_INVALID` → 사용자 문구). ⚠️ **또박또박 통보 사항**: D'Flow에서 팀을 비활성화하면 그 팀 회의록의 **재전송이 실패한다**.<br>＋ §10의 「비활성 팀 시나리오」 수용기준 귀속을 **§8(배치)** 로 옮긴다(테스트 이미 존재) |

---

## 부록. 구현 실행 — `/loop` 프롬프트 (**이력 — 실행 완료**)

> ⛔ **이 절은 이제 이력이다. 다시 돌리지 말 것.**
>
> 이 프롬프트로 시작한 구현 루프는 **완료됐다** — §4 표(`W1`·`W1-b`·`W2`~`W10`)와 §6.6 표(`W18`~`W23`), 그리고 결정으로 신설된 **`W24`·`W25`까지 전부 DONE**이다(작업 브랜치 `c769ee5`·`afc1943`·`4387576`, `main` 미머지). 진행 원장 `docs/design/folder-path-progress.md`가 항목별 검증 결과와 커밋 해시를 갖고 있다.
>
> 특히 아래 두 줄은 **더 이상 유효하지 않다**:
> - `[순서 제약]`의 “`W8` → `W1` → … → `W6`” — 결정 §3이 **R1 일괄 배포 + `W25` 플래그**로 대체했다(§11.2).
> - `[BLOCKED 규칙]`의 “⚠️ 결정 필요 3건과 §12 미해결은 구현하지 말고 BLOCKED” — **3건 전부 확정됐고**(§3.3·§8.3·§9.7) §12도 결정 §6으로 답이 나왔다.
>
> 남은 작업은 **구현이 아니라 배포·운영**이다: R1 배포 → 또박또박 0·1·3차 → 19건 대조 → APPLY → R2(플래그 `true`) → 2차 → R3 → R4(§11.2). 아래 원문은 그대로 보존한다.

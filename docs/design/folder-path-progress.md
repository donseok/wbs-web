...
**완료 18 + R1 결정분 10 / 차단 0 / 확인 필요 0** — Q2 는 2026-07-28 종결(§6 참조)
...
~~**Q2만 남아 있다.**~~ → ✅ **종결(2026-07-28)** — 수용기준을 「검색 인덱스(`ai_documents`) 재적재 발생」으로 정정했다
(작업지시 §6.8 에 근거 주석 포함, 코드 변경 0). 아래는 판단 근거 기록이다.

결정 정본이 Q2를 직접 다루지 않았다 — §6.8 수용기준 「다른 팀으로 이동 → 위키 철회·재빌드」는
...
| 10 | **Vercel Preview 대상 env 가 0건** — Preview 는 Supabase 접근 불가라 로그인 뒤 화면 검증 불가 | 인수인계 §1 정정 + `CLAUDE.md` G2 한계 명시 |
| 11 | 결정 §8 부록 기준표가 19건(실측 20건), 위치·제목도 변동 | 2026-07-28 재실측으로 표 전량 교체 |
| 12 | 별건 티켓 4건 미등록 | `docs/design/folder-path-backlog.md` 로 박제 (등록처 결정은 미결) |
| 13 | 또박또박 송부물 미작성 | `docs/design/ddobak-notice-2026-07-28.md` 작성 (송부는 사람 몫) |

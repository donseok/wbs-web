> ⚠️ **폐기됨 (2026-08-03).** 이 문서를 구현 근거로 쓰지 마라.
> 실측 검증에서 설계 blocker 4건(개방형 RLS 전제·member 쓰기 등급·이메일 미검증 가입·
> app_role 승격)과 사실 오류(마이그레이션 순번 0055 → 실제 0065)가 확인됐다.
> **정본은 `project-invite-spec-v2.md` 이며, 기능은 그 계약으로 이미 구현됐다.**
> 이 파일은 판단 이력으로만 남긴다.

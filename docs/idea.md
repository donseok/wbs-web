...
- **D'Flow 스킬 프롬프트 압축** — 같은 효과를 내면서 토큰을 적게 쓰도록 간결하게 고친다. (미착수, 2026-09-24 제안. 주간 토큰 한도 리셋 뒤 착수)
  - 크기(글자, 09-24 staging 1d2b9b87): dflow-team/SKILL.md 119.7K(팀장 세션 내내 상주) · dflow-dev/SKILL.md 37.1K + dev-discipline.md 28.0K(팀원 6명·Phase 서브에이전트마다) · dflow-merge/SKILL.md 37.2K(스윕·해소 워커) · references 합계 약 150K(필요할 때만). 한국어는 대략 1글자 ≈ 1토큰.
  - 방법
    1. 규칙과 근거 분리: 규칙마다 붙은 "이유:" 문단·사고 이력·실측 수치를 `references/rationale.md` 로 옮기고, 판단이 애매할 때만 읽게 한다.
    2. 중복 제거: SKILL.md·backends.md·restart.md 등에 반복된 규칙은 정본 한 곳에 두고 나머지는 가리키기만 한다.
    3. 드물게 쓰는 절(마감·재시작·전환 규칙 등)은 references 로 빼서 필요한 순간에만 읽는다.
    4. 셸 블록·출력 토큰(`SPAWN_DEFERRED_CAPACITY` 등)·명령은 글자 그대로 둔다(동작 불변).
  - 동등성 보증: 압축 전 규칙 목록(금지·순서·조건 규칙, 출력 토큰, 명령)을 기계적으로 추출 → 압축 뒤 각 항목이 SKILL.md 또는 제때 읽히는 reference 에 남았는지 대조. `tests/skills` 의 문자열 단언은 "규칙이 어디로 옮겨졌는지" 를 검사하도록 고친다.
  - 결정 대기: 언어(권장: 한국어 유지 + 간결화, 추정 30\~40%↓ / 영어 전환은 토큰이 더 줄지만 사람이 고치기 불편). 순서는 dflow-team → dflow-dev → dflow-merge. 2차 묶음 ①②③(Orca 실행·팀원 전용 설정·탭 닫기) 반영 뒤에 한다.
- **D'Flow 팀 자원·토큰 절감 2차 묶음** — 09-24 중복·자원·토큰 전면 점검의 후속. 1차 묶음(A\~J)은 staging 1d2b9b87 반영.
  - 완료(2026-09-24 staging 2b933b11, main 미반영): ① Orca 팀원을 tmux 와 같은 준비 단계 + `orca terminal create --command ./.dflow-run` 으로 띄움 ② 팀원 전용 설정(켜진 플러그인 전부 끄기·`--no-chrome --strict-mcp-config`, 실측 첫 턴 65.3K→54.2K 토큰) ③ 끝난 Orca 탭 닫기·Orca 재투입 잠금 해제·머지된 워크트리 정리 조건.
    - 첫 실제 spawn 때 팀원 대화 기록(jsonl)에서 MCP 0개 확인할 것.
  - 남은 것(미착수, 주간 토큰 한도 리셋 뒤)
    - ④ 팀장 깨우기 절제: tick 스크립트가 상태가 바뀌었을 때만 팀장을 깨운다(30분 무응답 깨우기는 유지). 팀장의 빈 기상 턴을 줄인다.
    - 작은 항목(④와 병렬): heartbeat 훅의 60초 절제 판정을 docs 스캔 앞으로 옮기고 토큰 사용량은 증분으로 계산 / poll 의 태그 조회 캐시·show 결과를 docker-allow `--json` 이 재사용 / 문서만 바뀐 머지는 방언 검증을 다음 스윕으로 이월(도커가 꺼져 있어 후순위).
    - ⑤ 1단계: Build Phase 를 단위별 서브에이전트(B1\~Bn, 또는 BE·FE)로 나누고 마지막 단위가 연결 테스트를 맡는다. 2단계(팀원 1M 컨텍스트 끄기·compact 기준)는 dmes Task 1\~2건으로 A/B 측정한 뒤 사용자에게 묻는다.
    - ⑥ 게이트 sha 캐시는 효과가 작아 제외(권장).
  - 사용자 판단 대기: 상주 서버(dmes fe-run.sh 11.6GB·ddobak 2.9GB) 때문에 capacity 가 CAPACITY_LOW(메모리 압박 warn)라 새 팀원 spawn 이 미뤄진다.
- 

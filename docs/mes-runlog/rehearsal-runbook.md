...
4. ~~agent-ops 등록~~ — **불필요(2026-08-24 개정)**. task 가 있는 wbs.md 업로드·위임 체크가 프로젝트를 자동 활성하고, 늦게 활성돼도 백필이 주문을 채운다
   ...
   **에이전트 활성·주문 동작 요약** (2026-08-24 최종 개정)
   - **자동 활성**: 위임 체크·dev_workflow ON·task 있는 wbs.md 업로드 중 처음 일어나는 것이 `agent_projects` 를 만든다(등록 화면·버튼 없음)
   - **백필**: 활성 시점과 설정 › 에이전트 카드 "재개" 시 dev_workflow=true 리프 전부에 주문 보장 — 업로드↔활성 순서 무관
   - **위임 체크 = 발행**(dev_workflow 도 자동 ON). 체크 해제 = 그 항목 ready·claimed 주문 취소("회수" 버튼 없음 — 체크 해제 하나로 통합).
     완료 보고(reported)가 온 주문은 취소되지 않는다 — 아래에서 승인·반려로만 정리
   - **킬스위치**: 프로젝트 설정 › 에이전트 카드 "전체 중지"/"재개"(프로젝트 관리자). 중지된 프로젝트는 위임 체크로 되살리지 않는다(경고 노출)
   - **승인·반려**: `/agent-ops` 화면은 없다 — 그 항목 WBS 명세 패널의 **"에이전트 진행 상황"** 섹션에서 한다(보고 이력·증적 링크 포함). 프로젝트 전체를 한눈에 훑는 칸반은 없음 — 알림벨(`work.reported`)이 관리자에게 자동 통지
...
reported 되면:
1. [웹] `/p/<UUID>/wbs` → 그 항목 클릭 → 명세 패널 "에이전트 진행 상황"(2026-08-24, agent-ops 화면은 없어졌다) → 보고 링크(브랜치·SHA) 확인 → **승인**
2. [Claude]
```
[Claude]
TSK-00-01 승인했어. 머지해.
...
1. [웹] 그 항목 명세 패널 "에이전트 진행 상황" → **반려**, 사유: `가동률 분모가 24h 고정. 자정 걸친 가동과 계획정지 제외 규칙이 PRD 와 다름`
...
| 머지 스킬 "approved 아님" | 웹 승인 전 | 명세 패널 "에이전트 진행 상황"에서 상태 확인 후 재시도. 우회 금지 |

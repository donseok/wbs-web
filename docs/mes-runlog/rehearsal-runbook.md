...
4. **`/agent-ops` → 프로젝트 선택 → "등록"** ← ⚠️ **wbs.md 업로드보다 먼저.** 미등록 프로젝트는 업로드 시 주문(work order)이 0건 생성된다

- [ ] UUID: `________________________________`
- [ ] agent-ops 등록됨 (빈 칸반 보임)
...
- [ ] 미리보기 신규 수 == 3-2 노드 수
- [ ] 주문 생성 8 (0 이면 → 2-1 ④ 누락. 등록 후 `/agent-ops` "발행"으로 건별 발행)
...
reported 되면:
1. [웹] `/agent-ops` → reported 컬럼 → 보고 링크(브랜치·SHA) 확인 → **승인**
2. [Claude]
```
[Claude]
TSK-00-01 승인했어. 머지해.
...
1. [웹] `/agent-ops` → **반려**, 사유: `가동률 분모가 24h 고정. 자정 걸친 가동과 계획정지 제외 규칙이 PRD 와 다름`
...
| 적용 후 주문 생성 0 | agent-ops 미등록 | 등록 후 `/agent-ops` "발행"으로 건별 |
...
| 머지 스킬 "approved 아님" | 웹 승인 전 | `/agent-ops` 상태 확인 후 재시도. 우회 금지 |

...
마이그레이션이 코드보다 **먼저**다. 테이블이 없는 상태로 코드가 배포되면
이슈 목록을 열 때마다 `getIssues` 의 첨부 조회가 PGRST 오류를 낸다 —
목록 자체는 뜨지만 로그가 오염된다(0027 사고 교훈). 상세 모달은 '첨부 없음'이 아니라
조회 실패를 표시한다.

반대 순서(마이그레이션만 적용, 코드는 구 버전)는 **무해하다** — 아무도 쓰지 않는 빈 테이블일 뿐이다.

> **초판에는 여기 더 무거운 경고가 있었다.** `deleteIssue` 가 첨부 경로 조회 실패를 중단
> 사유로 삼아, 테이블이 없으면 첨부와 무관한 이슈까지 아무도 못 지우는 상태가 됐다.
> 배포 전 검토에서 지적돼 고쳤다 — 그 조회가 실패해도 이슈 삭제 자체는 안전하고,
> 최악의 결과는 어차피 감수하고 있는 '고아 객체'와 같기 때문이다. 지금은 로그만 남기고 진행한다.
> 그래서 **순서를 어겨도 삭제가 막히지는 않는다.** 그래도 로그 오염 때문에 순서는 지키는 편이 낫다.

...
Run: `grep -c "for update" supabase/migrations/0019_meeting_minutes.sql`
Expected: `0` — UPDATE 정책이 없어야 수정 금지가 성립한다

Run: `grep -cE "^create (unique )?index" supabase/migrations/0019_meeting_minutes.sql`
Expected: `3`. **`^create index`로 세지 말 것** — `create unique index`가 안 걸려 `2`가 나온다 (그 스펠링만이 유효하다).

Run: `grep -c "owner = auth.uid()" supabase/migrations/0019_meeting_minutes.sql`

...
**Files:**
- Create: `supabase/migrations/0020_meeting_minutes.sql`
...
-- 막히는 건 그다음 createMinutes 의 메타 기록(canCreateMinutes + filePath prefix 검사)과
-- 아래 RLS insert_minutes 두 겹이다. UPDATE 정책이 없으므로 기존 객체 덮어쓰기는 불가능하다.
-- 삭제만은 소유자로 좁힌다 — 파괴적이고, 서버 액션을 우회해 직접 호출할 수 있기 때문이다.
...
-- bucket_id 만 검사하면 로그인한 아무나 브라우저 콘솔에서 남의 회의록 파일을 지울 수 있다
-- (0008 의 알려진 구멍 — 여기선 반복하지 않는다). meeting_minutes 의 delete_minutes 와 짝을 이룬다:
...
-- 남의 파일을 자기 이름으로 등록하는 위조 행을 만들 수 있다(읽기 RLS 가 using(true) 라
-- 남의 file_path 는 누구나 조회 가능). minutesStoragePath() 의 타임스탬프 프리픽스 덕에
-- 정상 업로드는 절대 충돌하지 않는다 — 이건 백스톱이다.
...
Run: `grep -c "^create policy" supabase/migrations/0020_meeting_minutes.sql`
Expected: `6` (storage 3 + table 3). 행 시작 앵커(`^`)를 붙여야 주석 안의 문자열이 안 세어진다.

Run: `grep -c "^drop policy if exists" supabase/migrations/0020_meeting_minutes.sql`
Expected: `6` (모든 create policy 앞에 멱등 drop이 있다)

Run: `grep -c "for update" supabase/migrations/0020_meeting_minutes.sql`
Expected: `0` — UPDATE 정책이 없어야 수정 금지가 성립한다

Run: `grep -cE "^create (unique )?index" supabase/migrations/0020_meeting_minutes.sql`
Expected: `3`. **`^create index`로 세지 말 것** — `create unique index`가 안 걸려 `2`가 나온다 (그 스펠링만이 유효하다).

Run: `grep -c "owner = auth.uid()" supabase/migrations/0020_meeting_minutes.sql`
Expected: `1` — 스토리지 삭제가 소유자로 좁혀져 있어야 한다

- [ ] **Step 3: 커밋**

```bash
git add supabase/migrations/0020_meeting_minutes.sql
...
- [ ] **Step 2a: 적용 *전에* 두 가정을 검증한다 — 건너뛰면 조용히 망가진다**

`0020`의 스토리지 삭제 정책은 검증되지 않은 가정 **두 개** 위에 서 있다. 어느 쪽이 틀리든 스토리지 삭제가 **거부**되는데, `remove()`는 거부를 `200 / [] / error: null`로 돌려준다(실측). 그래서 **행은 지워지고 파일만 조용히 고아가 된다.** 코드 경로 어디에서도 알 수 없다. 사후 QA로는 못 잡는다 — 삭제 버튼은 초록불이다.

`deliverables` 버킷에는 이미 `RowDetailPanel.tsx:325-330`의 브라우저 클라이언트(사용자 JWT)로 올린 진짜 객체가 들어 있다. 회의록 모달이 복사할 바로 그 코드 경로다. 자연 실험으로 쓴다.

```sql
-- 가정 1 (본 삭제 경로): storage.objects.name 이 upload() 에 넘긴 키 전체와 같은가?
--   틀리면 정책의 EXISTS 가 항상 거짓 → 모든 삭제 거부.
...
--   0행 = 조인 키가 틀렸다 = 정책이 틀렸다. (attachments 에 행이 하나라도 있는지 먼저 볼 것.)

-- 가정 2 (롤백 경로만): owner 가 존재하고 브라우저 업로드로 채워지는가?
select column_name, data_type from information_schema.columns
where table_schema = 'storage' and table_name = 'objects' and column_name in ('owner','owner_id');

select id, name, owner, owner_id from storage.objects
where bucket_id = 'deliverables' limit 5;
```

- **가정 1이 깨지면 적용하지 말 것.** 조인 키를 실제 컬럼으로 바로잡은 뒤 적용한다.
- 가정 2에서 `owner`가 NULL이고 `owner_id`가 차 있으면 `owner = auth.uid()` → `owner_id = auth.uid()::text`.
- 두 컬럼 다 없으면 `owner` 분기를 지운다 — 업로드 직후 롤백만 잃는다(본 삭제는 EXISTS 분기로 산다).

> **왜 `remove()`의 에러 검사로는 못 잡는가:** 그 검사는 전송 실패와 비-2xx만 잡는다. RLS 거부는 200이다. 실측: anon 키(스토리지 delete 정책 없음)로 없는 키를 지워도 `200 / [] / error: null`. 거부와 부재는 `remove()`를 통해 구별 불가능하다.

- [ ] **Step 2b: 마이그레이션을 실제 DB에 적용 — 사람이 한다**

Supabase 대시보드 → SQL Editor에 `supabase/migrations/0020_meeting_minutes.sql` 전문을 붙여넣고 실행한다. `supabase db push`는 쓰지 않는다(`SUPABASE_DB_URL`이 비어 있다).
...
## 마이그레이션
`0020_meeting_minutes.sql` — **머지 전에 SQL Editor 로 적용해야 한다.** 적용 전 배포 시 `/minutes` 가 500.

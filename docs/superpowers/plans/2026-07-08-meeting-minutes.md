...
- [ ] **Step 2a: 적용 *전에* 남은 가정 하나를 검증한다 — 건너뛰면 조용히 망가진다**

`0020`의 스토리지 삭제 정책에 남은 미검증 가정은 **딱 하나**다: `mm.file_path = storage.objects.name`. 틀리면 정책의 `EXISTS`가 항상 거짓이라 **모든 스토리지 삭제가 거부**되는데, `remove()`는 거부를 `200 / [] / error: null`로 돌려준다(실측). 행은 지워지고 파일만 남는다. 코드 경로 어디에서도 알 수 없고, 사후 QA로도 못 잡는다 — 삭제 버튼은 초록불이다.

(`storage.objects.owner` 가정은 없어졌다. 업로드가 행-먼저로 바뀌어 롤백도 행이 있는 상태에서 실행되므로 `EXISTS` 분기가 인가한다. 확인할 수 없는 것에 의존하지 않는 편이, 하고 나서 검증하는 것보다 낫다.)
...
```sql
-- attachments 에 행이 하나라도 있는지 먼저 확인한다(0행이면 실험 자체가 성립하지 않는다).
select count(*) from attachments;

-- storage.objects.name 이 upload() 에 넘긴 키 전체와 같은가?
...
- 위 count가 0이 아닌데 조인이 **0행**이면 조인 키가 틀렸다 = 정책이 틀렸다. **적용하지 말 것.** 실제 컬럼으로 바로잡은 뒤 적용한다.

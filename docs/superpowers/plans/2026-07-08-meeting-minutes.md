...
- [ ] **Step 2a: 적용 *전에* `owner` 가정을 검증한다 — 이걸 건너뛰면 조용히 망가진다**

`0019`의 스토리지 삭제 정책은 `owner = auth.uid()`에 의존한다. `storage.objects.owner`가 존재하고 실제로 채워지는지는 레포에서 확인할 수 없었다(`@supabase/storage-js`는 REST 응답 필드만 설명할 뿐 테이블 스키마를 모른다). 틀리면 모든 스토리지 삭제가 거부된다.
...
-- (b) 브라우저 세션 업로드가 실제로 owner 를 채우는가?
--     deliverables 버킷은 이미 RowDetailPanel.tsx:327 의 브라우저 클라이언트(사용자 JWT)로
--     올린 진짜 객체를 갖고 있다 — 회의록 모달이 복사할 바로 그 경로다. 자연 실험.
select id, name, owner, owner_id from storage.objects
where bucket_id = 'deliverables' limit 5;
```

`owner`가 NULL로 나오면 정책을 `owner_id::uuid = auth.uid()`(또는 `coalesce(owner, owner_id::uuid) = auth.uid()`)로 바꾼 뒤 적용한다.

> **왜 사후 QA로는 못 잡는가:** `deleteMinutes`가 `remove()`의 에러를 확인하기 때문에 이제는 잡힌다. 그 검사가 없었다면 삭제가 거부돼도 행은 지워지고 UI는 성공을 표시해, 파일만 조용히 고아가 됐을 것이다.

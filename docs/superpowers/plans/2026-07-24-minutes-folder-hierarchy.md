...
## v3 (같은 날 후속): 수정 모달 하위 구분 + '미지정' 상태

- 수정 모달(MinuteMetaModal)에도 하위 구분 탭 — 현 소속 폴더로 초기화(`getMinuteDetail`에
  folder_id 추가, `fetchMinuteFoldersLite` 신설), (팀, 하위) 선택이 열림 시점과 달라졌을 때만
  `updateMinuteMeta(id, patch, folderId?)`로 이동(미전달=무접촉 — 수동 편철 존중). 미분류는
  무변경 저장 시 팀 루트로 자기 치유.
- **하위 '미지정'('') 상태 도입**(리뷰 확정 major 반영): 팀 루트 편철(백필·외부 API의 지배적
  형태)·미분류·타팀/체인 밖 폴더는 어떤 하위도 선택 표시하지 않는다 — 대표 폴백을 선택으로
  보여주면 허위 표시 + 대표 하위로의 이동이 영구 불가(no-op 판정)였다. 미지정 저장은 팀 루트.
  `teamSubOfFolder`는 `{team, sub: string | null}` 반환(루트·드리프트=sub null, 추측 금지).
- 리뷰 반영 나머지: 같은 팀 탭 재클릭 무시 + 팀 왕복 시 열림 시점 하위 복원(왕복이 편철을
  바꾸면 안 됨), 폴더 로드 경합 가드(teamRef — 응답이 사용자 팀 전환을 덮지 않게), 폴더 로드
  `.catch`(전송 실패도 '빈 목록=숨김' 계약).

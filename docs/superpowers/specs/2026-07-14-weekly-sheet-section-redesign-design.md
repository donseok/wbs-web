...
연속 동일 section의 rowSpan 병합(`spans`)은 **제거한다.** 레거시 시트에서 `ERP` 5행이 병합되면 병합 셀에 첫 행의 모듈(`SD/LE`)만 남고 나머지 4개 모듈명이 화면에서 사라진다. 병합을 없애면 신규 시트(구분당 1행)는 모양이 그대로이고, 레거시 시트는 행마다 자기 `구분 / 모듈`을 정직하게 보여준다.

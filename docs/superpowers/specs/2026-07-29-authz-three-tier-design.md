...
- **역할 세분화**(읽기 전용 관리자, 팀장 등급 등) — 3단으로 확정.
  - 단, 2026-09-15 좁은 예외 하나가 추가됐다: **서브트리 관리자** — 담당자가 지정된 비리프 노드(WBS 트리)의 담당자가, 그 하위 리프의 **에이전트 허브 개발 프로세스 op**(승인·반려/승인취소/재작업·회수·단계 조정)를 관리자와 동등하게 처리한다. 일반 역할 등급이 아니라 **항목 단위 자격**이고(판정 축은 프로젝트 역할 2축 그대로, 여기에 트리 조상-담당자 조회를 얹는다), 허브 프로세스 op(전부 서버 가드·service_role 경로)와 승인의 실적% 특권 쓰기에만 적용돼 **RLS·트리거·마이그레이션 변경이 없다**. 판정은 `src/lib/agent/assignee.ts`(`isSubtreeManager`)·`src/lib/agent/subtreeManager.ts`(`requireSubtreeManagerOrAdmin`), 설계 정본은 에이전트 허브 스펙 `2026-09-14-agent-hub-design.md` §11-4.

...
| 보고 | WBS 반영 | 귀속 |
|---|---|---|
| `progress` (0~99) | **즉시 자동 반영** — 기존 실적 기록과 같은 3종 세트(`actual_pct` 갱신 + `change_logs` insert + `recordProgressSnapshot`)를 수행해 대시보드 트렌드·스냅샷이 그대로 동작 | 변경 주체(`change_logs.user_id`) = `user_email` 계정. **에이전트 이름 귀속은 보고 행(`agent_work_reports.agent`)이 담당** — `change_logs` 에는 note 컬럼이 없고 ALTER 는 금지다 |
...
- **사이드바 메뉴는 1차 범위에서 제외한다(사용자 결정 2026-07-31).** `/agent-ops` 는 URL 직접
  접근으로 시작한다 — 공유 UI 파일(`src/components/app/*`) 접점을 0으로 만들어 1차 배포를
  "신규 파일 + 신규 테이블만"인 순수 추가 배포로 유지하기 위함(7-27 사이드바 사고 부위 +
  Preview 로 로그인 화면 검증 불가 한계). 루프가 샘플 프로젝트에서 안정 확인된 뒤
  메뉴 추가만 별도 배포(브랜치+Preview 규칙)로 낸다.
...
- **P1(가변 깊이 WBS + 임포트 마법사)은 루프 샘플 검증 직후 연속 착수** (사용자 결정 2026-07-31 —
  구축 프로젝트 실전 투입 전에 루프+자유 깊이를 둘 다 갖춘다. 루프는 리프 단위라 P1 과 재작업 충돌 없음).
- **사이드바 메뉴 추가** — 루프 안정화 후 별도 배포(§5). 1차는 URL 직접 접근.

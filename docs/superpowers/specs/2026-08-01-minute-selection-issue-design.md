...
- 유효성: `endBlockIndex >= blockIndex`, 두 해시 모두 16-hex, `text` 정규화 후 비어있지
  않음, `text` 길이 상한 20,000자(기존 TEXT_MAX)·범위 블록 수 상한 200(비정상 요청 방어).
...
2. `rawText`를 줄 단위 정규화(`normalizeDraft`와 동일 규칙: CRLF→LF, 줄 내 공백 압축,
   빈 줄 제거)한 것이 비어있지 않은가. (아니면 `empty`) — 이 정규화 결과가 발췌 후보.
3. **공백 제거 대조**: 범위 `[startIndex..endIndex]` 안 markable 블록들의 `block.text`를
   이어붙이고 모든 공백을 제거한 문자열 H, 선택 텍스트에서 모든 공백을 제거한 문자열 N을
   만들어 N이 H의 **연속 부분 문자열**인지 확인한다. 추가로 매치 위치가 실제로 시작
   블록 안에서 시작하고 끝 블록 안에서 끝나는지 확인한다(끝점이 범위를 부풀리는 요청
   거절). (아니면 `text`)
4. 통과 시 `excerpt` = 2의 정규화 텍스트.
...
- 표출 조건(전부 충족):
  - 선택이 비어있지 않고, anchor·focus의 조상 `[data-mblock]`이 **둘 다** bodyRef 안.
  - 공백 제거 5자 이상.

...
- 헤드라인: `현행 {megaName} 프로세스는 {Major 이름 상위 3개}[ 등] N개의 Major 프로세스로
  구성되며, 이슈가 확인된 Sub 프로세스는 M개임` — N = 마스터 Major 수(미지정 제외),
  M = 트리에 표시되는 구분 고유값 총수(미지정 포함). Sub 마스터가 없어 트리의 Sub는
  이슈 관찰값뿐이므로, 셈플의 "M개의 Sub로 구성됨" 표현을 쓰지 않는다(디자인 감사 #1 반영).
  Major 0 + 미지정만인 영역은 `현행 {megaName} 프로세스에서 이슈가 확인된 Sub 프로세스는
  M개임 (Major 미지정)`.

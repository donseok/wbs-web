...
스토리지 객체 정책은 경로 첫 세그먼트가 `issue_id` 라는 규약을 이용한다. 0036:20-30 이
`deliverables` 에 쓴 것과 같은 형태다.
...
`src/app/actions/issues.ts` 에 `requireIssueEditable(issueId)` 를 신설해 export 하고,
새 파일이 그것만 쓴다. 판정은 기존 `adminOrOwnerGate` + `created_by` 비교와 동일하다
(= `can_edit_issue()` SQL 헬퍼, = UI 의 `canEdit`. 세 층이 같은 정의를 갖는다).

```ts
export async function requireIssueEditable(issueId: string): Promise<
...
`revalidateIssues(projectId)` 의 인자다. 이슈 조회가 실패하면 "권한 없음"이 아니라
**중단**한다(보안 가드는 fail-closed).

기존 `updateIssue`·`deleteIssue` 는 **건드리지 않는다.** 두 액션은 같은 조회에서
`mega_code`·`source_type` 까지 함께 읽어 불변 규칙을 검사하므로 헬퍼로 완전히 대체되지 않고,
바꿔봐야 회귀 위험만 산다.
...
**FK 임베드(`issue_attachments(count)`)를 쓰지 않는다.** 이 파일의 주석(`11-17`)이 금지하는 그것이다 —
"관계 미탐지 시 부모 쿼리 전체가 죽는다". 담당자·회의록 링크가 이미 별도 조회 + Map 병합이다.
조회 실패는 로그를 남긴다(silent-empty 금지).
...
**9곳(src 1 + tests 8)이 컴파일 실패**한다. `majorId`/`majorSeq`/`majorName` 이 같은 이유로
...
기존 18개 파일의 방식을 따른다(`issue-major-processes.test.ts:1-11` 이 표준형).

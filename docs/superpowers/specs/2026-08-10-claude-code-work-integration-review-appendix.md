...
- **알림함이 먼저 개발된다(2026-08-11 사용자 결정)** — 저장 계층(0074)·`emitNotification` 헬퍼가 선행
  배포되므로, **연동 서버 Task를 구현할 때 아래 표의 지점에 emit 호출을 직접 포함한다**(서버 계획
  Global Constraints에 명시). 연동이 먼저 나가는 예외 상황에만 알림함 쪽 retrofit으로 후속 —
  어느 순서든 동작한다(먼저 배포된 쪽 지점만큼만 발행).

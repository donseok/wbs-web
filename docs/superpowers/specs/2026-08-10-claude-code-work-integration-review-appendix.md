...
작성 2026-08-10 · 상태 **검토 보고 — 구현 승인 아님**
...
코퍼스에서 사용자가 유일하게 확정한 **해석 2**(`21-multi-client-model.md:93` "서버 접속 사용자 = 에이전트 러너만. 사람은 WBS 확인과 승인만")는 그 비범위표(`:95-101`)에 "개인 배정 축 · '내 작업' 화면 · 사람용 claim"을 명시적 비범위로 못박았다. 이 문서의 제품 정의가 정확히 그것이다. **따라서 착수 조건은 해석 2의 번복 승인이다**(미결 ⓪). 번복 승인 시 코퍼스 쪽에도 기록한다.
...
- **TSK-00-01 미결 결정** (`research`, depends 없음) — 미결 ⓪(해석 2 번복)·①(발급 권한)·②(단일 테이블)을 결정하고 `decisions.md`에 기록. **acceptance: 세 결정 명문화.** 이 Task가 `[im]`이기 전에는 WP-01 이하 전부 `[ ]`.
...
- **⓪ [최우선] 08-05 확정 '해석 2'를 번복하는가.** 코퍼스에서 사용자가 유일하게 확정한 결정("서버 접속 사용자 = 에이전트 러너만. 사람은 WBS 확인과 승인만")은 "개인 배정 축·'내 작업' 화면·사람용 claim"을 명시적 비범위로 선언했다. 이 문서의 제품 정의가 정확히 그것이므로 **이 기능은 그 확정의 번복이다.** 이번 검토 자체가 사용자 요청에서 출발했으므로 번복이 자연스러우나 명문 승인이 필요하다. 승인 시 코퍼스 쪽에도 기록. **Phase 1 착수 게이트.**
- **① 토큰 발급 권한.** 제안: `kind`로 나눠 `user_pat`은 본인 세션 자율 발급 + **프로젝트 관리자 대리 발급**(웹에 로그인하지 않는 팀원 몫, `created_by` 기록 — §2.7), `runner`는 슈퍼유저 전용. 자율·대리 모두 읽기 스코프 한정 — `work:report`는 미결 ⑦이 닫히기 전까지 관리자 승인 발급만. **Phase 1 착수 게이트.**
- **② 토큰 테이블을 `agent_runners` 하나로 갈 것인가.** 코퍼스가 이미 초안을 보유하므로 `kind`만 더해 단일 테이블 권고 — 승인하면 훗날 러너 합류가 마이그레이션 없이 끝난다. **Phase 1 착수 게이트.**

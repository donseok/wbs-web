...
## 지금 결정할 것 (Phase 1 착수 게이트)

| # | 결정 | 권고 |
|---|---|---|
| ⓪ | **08-05 확정 "해석 2"(사람은 WBS 확인·승인만)를 번복하는가** — 이 기능(사람용 PAT·내 작업·claim)은 그 확정이 비범위로 못박은 것이다 | 번복 승인 (이번 요청 자체가 그 방향) |
| ① | 토큰 발급 권한 | `user_pat` 본인 자율 + 관리자 대리 발급(읽기 스코프 한정), `runner` 슈퍼유저 전용. `work:report` 자율발급 금지 |
| ② | 토큰 테이블 | 코퍼스의 `agent_runners` 단일 테이블 + `kind` 컬럼 (러너 합류가 공짜) |

...
> 구현 기록 (2026-08-13): 실제 1차 구현은 사용자가 답을 얻고 다시 기여하는 흐름을 먼저
> 닫는 **Project Memory**로 조정했다. Ask(정본 문서 + 승인된 지식 + 회의록 출처), 사람 문서
> 작성·템플릿·낙관적 잠금·revision, 검증/재검토 기한, 질문 공백, 도움됨/오래됨 피드백,
> AI 제안 검토, 모바일 메뉴, 제품 이벤트를 포함한다. 읽기 모델의 200/500 무음 절단과 철회
> 근거 노출도 함께 제거했다. 실제 파일명은 `0079_wiki_memory.sql`과
> `0079_wiki_memory_rollback.sql`이다.
>
> 아래 초안의 트리 전용 홈·시작 문서 3장 자동 생성·자동 수집 재개는 1차 배포 범위에서
> 제외했다. 트리 데이터/RPC는 준비했지만 화면 재배치는 사용 계측 후 진행한다. 특히 기존
> atomic ingest가 accepted 항목을 직접 갱신할 수 있어 **제안 전용 경로가 완전히 분리되기
> 전까지 `WIKI_SERVICE_ENABLED`와 `WIKI_WORKER_ENABLED`는 모두 끈 채 유지한다.** 이 구현
> 기록이 아래 §9·§12·§14의 과거 목표보다 우선한다.

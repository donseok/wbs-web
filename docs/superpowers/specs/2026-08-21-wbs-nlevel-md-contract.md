...
- **웹 경로(사람/PL)**: 골격 트리에서 시스템 노드 선택 → 파일 선택 → 검증 리포트 미리보기(levels 정합·owner 위반·신규/갱신/스킵·담당 미매칭·fold 건수·재업로드 diff) → 적용.
- **API 경로(자동화)**: frontmatter `attach` 가 부착점.
- **충돌 규칙**: 파일 attach ≠ 화면 선택 노드 → 거부(fail-closed). attach 는 웹 경로에선 교차검증 안전핀(파일 잘못 선택 방어), API 경로에선 필수값.

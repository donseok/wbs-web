...
3. **감시 시작** — 팀장을 깨우는 신호는 **둘**이며 둘 다 띄운다. 팀장은 절대 포그라운드로 기다리지 않는다.
   ...
   - **팀원 완료 감지 — 기상 신호는 백엔드에 따라 다르지만 `.result` 계약은 하나다.**
     - **pane 백엔드(Orca·tmux)**: 팀원이 별도 프로세스라 완료를 알려 오지 않는다. `Monitor` 를
       until-loop 로 걸어 진행 중 슬롯들의 워크트리에서 `docs/tasks/<TSK>/.result` 가 생기는지
       감시한다. 이 감시가 없으면 poll.sh 가 몇 시간 안 끝나는 사이 완료된 슬롯이 보충되지 않는다.
       **재무장 규칙**: 슬롯이 생기거나 회수될 때마다 Monitor 를 다시 띄우는데, 구 Monitor 종료와
       신규 Monitor 최초 패스 사이에 도착한 `.result` 가 유실될 수 있다. 따라서 **재무장 스크립트는
       기동 즉시 현재 전체 슬롯 경로를 1회 전수 검사한 뒤 감시에 들어간다.**
     - **에이전트 팀 백엔드**: 팀원이 끝나면 완료 알림이 자동으로 온다. **Monitor 도 재무장도 필요
       없다**(위 유실 위험 자체가 없다). 알림을 받으면 그 슬롯의 `.result` 를 읽는다.

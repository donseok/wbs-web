...
경로)을 보고한 뒤 잠금을 지우지 않고 끝낸다. 이유: 이 세션의 에이전트를 멈추는 것은 공유 상태를 건드리지
않으며, 남겨 두면 새 팀장의 에이전트와 같은 agent 브랜치에서 경합한다. 체크아웃과 이 신원의 워크트리·세대
파일은 이제 새 팀장 것이고, 새 팀장의 재구성은 같은 `agent`·`repo` 의 마지막 `team.start` 이후

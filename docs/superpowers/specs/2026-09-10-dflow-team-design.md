...
- **부트스트랩 실패 정리**: `.result` 의 branch 가 `-`(브랜치를 만들기 전에 끝남)이면
  ...
  `.env` 링크, 스킬 링크(`.claude/skills` 또는 그 안의 `dflow-*`))만 있을 때만 `--force` 로 정리한다. 그 밖의 변경이 있으면 보존하고
...
- 3건과 별도로, 그중 문서 작업을 선행으로 둔 작업 1건을 더 만든다. 리허설 작업은 승인하지 않으므로 이
  작업은 매번 `skipped` 로 끝나 일시 제외되고 30분 뒤 다시 뜬다. 같은 id8 을 다시 띄우는 경로(§4-8)와 생성
  브랜치 정리(§4-2)를 확인하기 위해서다.
- **3건 중 1건은 테스트가 있는 코드 작업이다.** mes-base 는 문서 전용이라 그대로는 기준선·Refactor
...
8. `~/.dflow/events.jsonl` 에 `team.start` → `team.spawn`×2 → `team.result` → `team.spawn`(3번째) →
   ...
   식별자(`<신원>/<host>/w1`, `<신원>/<host>/w2`)이고, 3번째 작업의 `.dflow-agent` 는 먼저 빈 슬롯의 값과
   같다.
...
4. `<id8> <답>` 으로 답한 뒤 재spawn 된 워커가 `ANSWER` 를 design.md 에 남기고 같은 agent 브랜치 위에서
   이어 간다. 빈 슬롯이 없을 때 답하면 그 작업이 대기 큐 맨 앞에서 다음 빈 슬롯을 받는다. 코드 작업이면
   재spawn 워크트리에 `node_modules` 가 설치되고(§6-3 행 H 의 재개 경로) 게이트가 127 로 끝나지 않는지 본다.

...
| 팀장 잠금 | 한 체크아웃에 팀장은 하나다. 체크아웃의 git 디렉터리에 잠금 디렉터리를 `mkdir` 로 원자 획득하고, 매 기상 갱신하는 `beat` 가 70분보다 오래되면 죽은 것으로 본다. 소유는 `owner` 의 신원과 팀장 세션 PID 로 판정하며, 매 기상 소유를 확인한 뒤에만 `beat` 를 쓴다(§4-4) | 같은 체크아웃의 팀장 둘은 슬롯 번호·세대 파일·승인 스윕을 서로 덮어쓴다. 잠금을 빼앗긴 팀장은 신원·host·리포가 새 팀장과 같아 신원만으로는 자기 잠금을 가려내지 못한다 |
...
1. **전제 검사**: SKILL.md 에 적힌 셸 블록 하나로 돌린다. 블록은 실패한 항목을 모두 출력한 뒤 0 이 아닌
   ...
   - **팀장 잠금**: 잠금은 디렉터리 `$(git rev-parse --git-path dflow-team.lock)` 이며, 나머지 검사가 모두
     ...
     디렉터리를 지우고 전제 검사를 실패로 끝낸다. 이유: `beat` 가 없는 잠금은 막 생긴 것으로 보여 이후의
     모든 시작을 막고, 방금 `mkdir` 로 만든 잠금은 다른 팀장이 건드리지 않으므로 지워도 남의 잠금이 아니다.
     팀장은 매 기상 소유를 확인한 뒤 `beat` 를 현재 시각으로 갱신한다(§4-5). `mkdir` 가 실패하면 기존
     잠금의 `beat` 를 본다. `beat` 가 없으면 방금 만들어진
     잠금으로 보고 새로운 것으로 친다. 70분보다 새로우면 살아 있는
     팀장이 있으므로 시작을 거부하고, 잠금 경로·`owner`·`beat` 시각과 함께 "그 팀장이 끝난 것이 확실하면
     잠금 디렉터리를 지우고 다시 시작하라" 를 안내한다(세션이 죽은 직후 재기동하면 `beat` 가 아직
     새롭기 때문이다). 더 오래됐으면 죽은 것으로 보고 가져온다. 가져올 때는 잠금 디렉터리를 `mv` 로 이
     팀장만 아는 이름(`<잠금>.stale.<pid>`)으로 옮기고, 옮긴 디렉터리의 `beat` 를 다시 읽어 여전히 오래됐을
     때만 지운 뒤 `mkdir` 로 다시 획득한다. 옮긴 잠금이 새로우면(그사이 다른 팀장이 가져갔다) 옮긴 경로를
     알리며 거부하고 사람이 되돌리게 한다. `mv` 나 다시 하는 `mkdir` 가 실패하면 다른 팀장이 먼저 가져간
     것이므로 거부한다. 이유: `beat` 를 다시 읽은 뒤 지우기 전에 다른 팀장이 먼저 가져가면 그 잠금까지
     지우게 되는데, 옮긴 디렉터리는 이 팀장만 보므로 확인과 삭제 사이에 끼어들 틈이 없다. 잠금은
     마감(§4-9)에서 같은 소유 판정을 통과할 때만 지운다. 이유: 한 체크아웃의 팀장 둘은 슬롯
     번호·세대 파일·승인 스윕을 서로 덮어쓴다. 생존(가져와도 되는지)은 PID 가 아니라 `beat` 로 본다.
   ...
   - 로컬 `docs/tasks/*/state.json` 중 `phase=reported` 이면서 `api_base` 가 없는 것이 있으면 시작을
     ...
     복제하므로(§3-20) 출처를 모르는 로컬 후보를 자동 스윕이 머지할 수 있고, `api_base` 가 없으면 스윕이
     그 출처를 가려낼 수 없다(§6-4).
...
**잠금 상실 마감**(§4-5): 기다림·마지막 승인 스윕·워크트리 정리·`team.*` 기록을 하지 않는다. 떠 있는 poll 을
멈추고, 집계와 남은 슬롯(TSK·id8·워크트리 경로)을 보고한 뒤 잠금을 지우지 않고 끝낸다. 이유: 체크아웃과 이
신원의 워크트리는 이제 새 팀장 것이고, 새 팀장의 재구성은 같은 `agent`·`repo` 의 마지막 `team.start` 이후

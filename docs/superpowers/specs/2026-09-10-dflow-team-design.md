# /dflow-team — 팀장 스킬 설계 (2026-09-11, 개정 4판)

D'Flow 에서 내게 배정되고 에이전트 위임(`tags:agent`) 된 ready 작업을 상시 감시하다가, 발견되는
대로 팀원에게 나눠 주어 동시에 개발시키는 팀장 스킬이다. 팀원은 **자기 서브에이전트를 띄울 수 있는
독립 세션**이며(단순 서브에이전트가 아니다) 각자 자기 워크트리에서 작업 한 건을 맡아 `/dflow-dev` 를
돌린다.
...
⚠️ 2026-09-11 저녁 2차 검토의 걸림돌 T1~T19(`docs/superpowers/specs/2026-09-11-office-team-blockers.md`)가
이 스펙에 아직 반영되지 않았다 — §4(슬롯 상태 메모리 전용, 무응답 2시간 정리), §4-3(에이전트 팀 기점 "무관"),
§5-A 행 B(스택), §6(원격 후보 조건) 등이 수정 대상이다.
...
> (dev-discipline.md). 그런데 단순 서브에이전트는 다시 서브에이전트를 띄우지 못한다. 그래서 팀원을
> 서브에이전트로 부르면 그 안에서 `/dflow-dev` 가 깨진다. 팀원은 **자기 서브에이전트를 띄울 수 있는
> 주체**여야 한다는 것이 이 설계의 제1 제약이다. 개정 2판이 폐기된 것도, 계획서가 옛 구조로
> 되돌아갔던 것도 이 근거가 문서에 없었기 때문이다.
>
> **개정 4판이 개정 3판을 고친 것** — 개정 3판은 위 제약을 "팀원 = 별도 프로세스(pane)" 로만 풀 수
> 있다고 보고, 일반 터미널을 병렬 불가로 판정해 `/dflow-poll` 안내 후 종료시켰다. 2026-09-11 실측에서
> **에이전트 팀 팀원(`isolation: worktree`)도 이 제약을 만족한다**는 것이 확인됐다(§3-1 실측 표).
> 팀원 둘을 동시에 띄워 각자 자기 링크드 워크트리를 받고, cwd 가 Bash 호출 사이에 유지되고, 각자
...
- **Orca·진짜 tmux·일반 터미널 셋 다에서 병렬로 동작한다**(§3-5, §4-0). 앞의 둘은 pane 백엔드(선호),
  일반 터미널은 에이전트 팀 백엔드를 쓴다. 어느 환경에서도 "병렬 포기" 분기는 없다.
...
| 결정 | 내용 |
|---|---|
| 팀원 정체 | **자기 서브에이전트를 띄울 수 있는 독립 세션.** 근거: `/dflow-dev` 가 Phase 1~4 를 서브에이전트로 쪼개는데(dev-discipline.md) 단순 서브에이전트는 서브에이전트를 못 띄우므로, 팀원을 서브에이전트로 부르면 `/dflow-dev` 가 깨진다. 이 조건을 만족하는 형태는 둘 — **pane 의 별도 프로세스**(선호) 또는 **에이전트 팀 팀원(`isolation: worktree`)**. 둘 다 2026-09-11 실측으로 확인(§3-1) |
| 백엔드 | **환경 적응형**: Orca → `orca worktree create --agent claude --prompt`, 진짜 tmux → dev-plugin `/team-mode`(로드 실패 수정 후 재사용), 일반 터미널 → **에이전트 팀**(Agent 도구 + `isolation: worktree` 필수). 셋 다 병렬이 되며 포기 분기는 없다 |
| pane 을 선호하는 이유 | 팀원 화면이 사람에게 보이고(좌석표가 비추는 대상), `blocked` 질문을 그 탭에서 사람이 직접 답할 수 있으며, 팀장 세션이 죽어도 팀원이 살아남는다. 에이전트 팀 팀원은 이 셋을 모두 잃는 대신 어느 환경에서나 뜬다(§12) |
...
1. **`cd` 는 Bash 호출 사이에 유지되지 않지만, 세션의 고정 cwd 는 격리로 바꿀 수 있다**(2026-09-11
   실측). 매 Bash 호출이 세션 고정 cwd 로 리셋되는 것은 맞다. 그러나 Agent 도구의
   `isolation: "worktree"` 는 그 **고정 cwd 자체를 팀원 전용 링크드 워크트리로 준다.** 그래서 `cd` 가
   유지되지 않아도 팀원은 항상 자기 워크트리에 있다.

   > **개정 3판의 오류 정정** — 개정 3판 §3-1 은 "여러 in-process 팀원이 각자 다른 워크트리 cwd 를
   > 가질 방법이 없다" 고 단언했으나 이는 틀렸다. 그 근거가 된 실측은 `isolation` 을 주지 않은
   > 에이전트를 관찰한 것이었다(당연히 세션 cwd 를 상속한다).

   2026-09-11 실측 — 팀원 둘을 `isolation: "worktree"` 로 **동시에** 띄운 결과:
   ...
   | 팀장 체크아웃 영향 | 브랜치 `staging` 유지, 워킹트리 깨끗 | 동일 |

   즉 병렬 격리는 별도 OS 프로세스로만 가능한 것이 아니다. **에이전트 팀 팀원도 성립한다.**
2. **팀원은 자기 서브에이전트를 띄울 수 있는 독립 세션이다.** 이것이 유일한 필수 조건이며(머리말의
   제1 제약), pane 의 별도 프로세스와 에이전트 팀 팀원이 둘 다 이를 만족한다. 어느 쪽이든
   AskUserQuestion·Skill·Bash·Agent 를 갖는다(2026-09-11 실측). 팀원 내부의 phase 분할(design/build/
   verify/refactor)은 서브에이전트로 이뤄진다(`/dflow-dev` 가 이미 그렇게 한다).
...
5. **실행 백엔드는 환경별로 갈린다.**
   ...
   - **일반 터미널 (에이전트 팀 백엔드)**: Agent 도구로 팀원을 띄우되 **`isolation: "worktree"` 를
     반드시 준다.** 이 파라미터가 없으면 팀원이 팀장의 cwd 를 그대로 상속해 같은 체크아웃에서
     `git switch -c` 를 치고 서로를 덮어쓴다(§3-3 의 훼손 시나리오). 이건 선택지가 아니라 필수
     조건이며, 빠뜨리면 조용히 깨진다. 팀원 워크트리는 에이전트 종료 시 자동 정리된다(변경이 없으면).
     결과는 `.result` 파일과 **완료 알림** 둘 다로 돌아온다(§4-1 3번).
...
5-A. **에이전트 팀 백엔드에서는 rtk 훅이 일부 git 명령을 차단한다**(2026-09-11 실측, 두 팀원에서
   동일 재현). 워크트리 격리 에이전트 안에서 `git status`·`git branch` 처럼 **rtk 가 가로채 재작성하는
   서브커맨드**는 격리 검증에 걸려 거부된다. 에러 문구는 "a worktree-isolated agent's git operations
   must target its own worktree" 다. 반면 `git switch`·`git rev-parse` 처럼 rtk 가 재작성하지 않는
   서브커맨드는 정상 통과한다. 그래서 증상이 "같은 git 인데 어떤 건 되고 어떤 건 안 되는" 형태로
   보인다.

   - **우회**: 절대경로 `/usr/bin/git` 으로 호출하면 rtk 를 거치지 않아 통과한다(실측 확인).
   - **영향**: `/dflow-dev` 는 `git status`·`git log`·`git diff` 를 대량으로 쓴다. 에이전트 팀
     백엔드로 워커를 돌리려면 이 우회가 워커 경로 전반에 적용돼야 한다.
...
2. 아니고 `$TMUX` 존재(Orca shim 이 아닌 진짜 tmux) → **tmux 백엔드**. `/team-mode` 로드 여부를
   확인하고, **로드 실패면 중단하지 않고 에이전트 팀 백엔드로 내려간다**(그 사실과 이유를 한 줄
   보고한다). dev-plugin 수정 안내(§12)는 함께 출력한다.
   **v1 에서는 `/team-mode` 가 로드돼 있어도 에이전트 팀으로 내려간다.** tmux pane spawn 명령은
   team-mode 인터페이스를 보고 확정해야 하는데(§4-3) dev-plugin 이 아직 로드되지 않아 확정할 수 없기
   때문이다. dev-plugin 수정 뒤 tmux pane spawn 을 순수 가산으로 더한다.
3. 그 외 → **에이전트 팀 백엔드**. 병렬은 그대로 되며 `isolation: "worktree"` 로 팀원을 띄운다(§3-5).
   pane 이 없으므로 `blocked` 응답 경로와 가시성이 달라진다는 점을 시작 시 한 줄 알린다(§7).
...
3. **감시 시작** — 팀장을 깨우는 신호는 **둘**이며 둘 다 띄운다. 팀장은 절대 포그라운드로 기다리지 않는다.
   ...
   - **팀원 완료 감지 — 기상 신호는 백엔드에 따라 다르지만 `.result` 계약은 하나다.**
     - **pane 백엔드(Orca·tmux)**: 팀원이 별도 프로세스라 완료를 알려 오지 않는다. **감시 루프**를
       Bash `run_in_background` 로 띄워(poll.sh 와 같은 방식 — 종료하면 팀장이 깨어난다) 진행 중
       슬롯들의 워크트리에서 `docs/tasks/<TSK>/.result` 가 생기거나 status 가 바뀌는지 20초마다
       본다. 이 감시가 없으면 poll.sh 가 몇 시간 안 끝나는 사이 완료된 슬롯이 보충되지 않는다.
       **재무장 규칙**: 슬롯이 생기거나 회수될 때마다 감시 루프를 다시 띄우는데, 구 루프 종료와 신규
       루프 최초 패스 사이에 도착한 `.result` 가 유실될 수 있다. 따라서 **감시 루프는 기동 즉시 현재
       전체 슬롯 경로를 1회 전수 검사한 뒤 감시에 들어간다.**
     - **에이전트 팀 백엔드**: 팀원이 끝나면 완료 알림이 자동으로 온다. **`.result` 감시도 재무장도
       필요 없다**(위 유실 위험 자체가 없다). 알림을 받으면 그 슬롯의 `.result` 를 읽는다.
     - **TICK(두 백엔드 공통)**: 감시 루프는 30분이 지나면 `TICK` 을 출력하고 끝난다(에이전트 팀은
       `.result` 감시 없이 30분 대기만 한다). 그래서 팀장은 아무 일이 없어도 30분마다 깨어나 승인
       스윕·무응답 점검을 한다(§4-2). 이 기상이 없으면 한가한 구간에 팀장을 깨울 계기가 poll 의
       ready 발견뿐이다.
     - `.result` 파일 계약(§5-보고)은 **두 백엔드에서 동일하다.** 알림은 기상 수단일 뿐이고, `.result`
       는 완료 신호 외에 `blocked` 질문 내용·중단 기록·집계 근거를 함께 나르며 §11 합격 기준 9 가
       이를 검사한다. 백엔드마다 보고 형식을 나누지 않는다.
...
| 감시 루프 `TICK`(30분) | 승인 스윕. 30분 넘게 신호 없는 진행 슬롯이 있으면 `list --scope claimed` 1회로 슬롯 표 갱신, 2시간 넘으면 §7 무응답. 감시 루프 재무장 |
| 팀원 무응답 | §7 |

**승인 스윕 주기(§3-16 대응)**: poll exit 9 에 기대지 않고, 팀장은 **깨어날 때마다** 승인 스윕(§6)을
한 번 돈다 — 시작(§4-1 2번), 팀원 `.result` 도착, 감시 루프 `TICK`(30분), poll 재시작 직전,
마감(§4-4). 스윕 후보는 원격 `origin/agent/*` tip 에서 오므로 팀장 체크아웃의 state.json 유무와
무관하다. 대가는 승인 반영 지연이다: 사람이 승인한 뒤 다음 기상까지 main 반영이 늦는다. TICK 이
있으므로 지연은 최대 30분이다. 이 구간에도
...
**`.result` 기상·폴링**: 팀원 완료는 백엔드별 기상 신호(§4-1 3번 — pane 은 `.result` 감시 루프,
...
진행 슬롯의 신호가 없으면(TICK 때 판정) `dflow.sh list --scope claimed` 로 서버 상태를 슬롯 표에
갱신한다(팀원은 `list` 를 안 부르므로 캐시 경쟁은 팀장 자신뿐). Orca 에서는 `orca terminal read --screen --terminal
...
TSK=<TSK> ID8=<id8> AGENT_ID=<신원>/w<slot> MAIN_CHECKOUT=<팀장 체크아웃 절대경로> BACKEND=<pane|agent-team> MODEL=<opus|sonnet|default>
```
실제로는 위 두 줄을 공백 하나로 이어 **한 줄**로 넘긴다(Orca `--prompt` 자동 제출은 한 줄에서
실측됐다). 워커 프롬프트 경로를 **절대경로**로 주는 이유는 새 워크트리에 스킬이 없을 수 있기
때문이다(§3-15). 모델은 `MODEL_FLAG=--model opus` 처럼 공백이 든 값을 넘기지 않고 `MODEL=` 로
넘기며, 워커가 `{MODEL_FLAG}` 로 바꾼다(`default` 면 빈 값). 에이전트 팀 `blocked` 재개 때만 둘째 줄에
`ANSWER=<담당자 답 한 줄>` 을 붙인다(Agent 도구는 쉘 인자가 아니라 여러 줄이 안전하다).
...
- **에이전트 팀**: Agent 도구로 띄운다. 쉘 인자 인용 문제가 없으므로 포인터가 아니라 **워커 프롬프트
  전문을 그대로 넘겨도 된다**(그래도 다른 백엔드와 문구를 일치시키기 위해 같은 포인터 방식을 쓴다).
  - `isolation: "worktree"` — **필수**(§3-5). 빠뜨리면 팀원이 팀장 cwd 를 상속해 서로를 덮어쓴다.
  - `name` — `w<slot>-<id8>`. 슬롯이 앞에 있어 사람이 알아보고, id8 이 붙어 같은 슬롯의 다음 작업과
    이름이 겹치지 않는다. 진행 중 팀원을 이름으로 조회·중단(`TaskStop`)할 수 있다. 좌석표 식별
    (`AGENT_ID`)은 이름이 아니라 포인터의 `AGENT_ID=` 가 정한다.
  - `subagent_type` — 모든 도구(Skill·Agent·Bash 포함)를 가진 범용 타입(`general-purpose`).
  - `model` — `--model` 값을 전달한다.
...
그 다음 동작은 **백엔드에 따라 갈린다**(포인터가 `BACKEND=` 로 알려 준다).

| 백엔드 | `blocked` 이후 |
|---|---|
| pane(Orca·tmux) | 질문을 화면에 출력한 채 세션을 멈춘다. 탭이 열려 있으므로 사람이 그 탭에서 답하거나 수동 `/dflow-dev {ID8}` 로 이어받는다(§7). 슬롯은 계속 점유한다 |
| 에이전트 팀 | **탭이 없으므로 멈춰 있어도 아무도 못 본다.** 질문을 `.result` 에 남기고 **세션을 종료한다.** 팀장이 알림으로 받아 사람에게 전달하고, 답이 오면 팀장이 같은 워크트리·브랜치로 워커를 다시 띄운다(§7). 슬롯은 회수된다 |

> **왜 에이전트 팀에서는 AskUserQuestion 을 쓰지 않는가** — 팀원은 AskUserQuestion 도구를 갖고 있고
> (2026-09-11 실측) 그것으로 사람에게 직접 물을 수도 있다. 그러나 §5-A 행 D 는 자동 모드에서
> AskUserQuestion 억제를 계약으로 못 박았고([[no-questions-in-auto-loops]]), 슬롯 N개가 각자 질문을
> 띄우면 사람이 어느 팀원의 질문인지 모른 채 N개 창을 받는다. 그래서 질문은 **팀장 한 곳으로 모은다.**
> 억제 계약은 두 백엔드에서 동일하게 유지된다.
...
| 팀장 세션 소실 — **pane** | 각 팀원은 자기 탭에서 계속 돈다(별도 프로세스) | 재기동 시 `dflow.sh list --scope claimed` 와 `git branch -r --list 'origin/agent/*'`, 살아 있는 팀원 탭(Orca `worktree list`)을 대조한다. claimed 인데 팀원 탭도 없는 id8 은 "재개 필요" 로 보고하고 제외 목록에 넣는다(자동 재착수 없음). 그 뒤 §4-1 로 정상 시작 |
| 팀장 세션 소실 — **에이전트 팀** | **팀원이 팀장과 함께 죽는다**(in-process). 진행 중이던 작업은 마지막 커밋·push 까지만 남고 그 뒤 작업은 잃는다 | 서버에는 claimed 로, 원격에는 agent 브랜치가 남아 있다. 재기동 시 위와 같이 대조하되 **살아 있는 팀원은 하나도 없다고 전제한다.** claimed 전부를 "재개 필요" 로 보고하고 제외 목록에 넣는다. 미커밋분 손실을 줄이는 것은 `/dflow-dev` 의 빈번한 커밋 규칙과 `blocked` 전 커밋 규칙이다 |

> **blocked 모델 — 슬롯을 잡는지는 백엔드가 가른다(개정 4판에서 정정).**
>
> 판정 기준은 하나다: **살아 있는 프로세스 둘이 같은 `AGENT_ID` 로 heartbeat 를 보내면 안 된다**
> (§9-1). 좌석표가 한 인물을 두 책상에 그리고 손 든 상태가 새 active 에 덮인다.
>
> - **pane** — blocked 팀원이 탭에 **살아 있다.** 그래서 슬롯을 재배정하면 위 충돌이 난다. 슬롯을
>   계속 잡는다. 비용은 blocked 동안 실효 병렬도가 주는 것이다(팀원 3에 blocked 2면 실효 1).
> - **에이전트 팀** — blocked 팀원이 **종료한다.** 살아 있는 프로세스가 없으므로 충돌이 성립하지
>   않고, 슬롯을 회수해도 안전하다. 실효 병렬도가 줄지 않는다.
>
> 개정 3판은 pane 만 있다고 보고 "blocked 는 항상 슬롯을 점유한다" 로 단정했는데, 그 근거(살아 있는
> 두 프로세스)가 에이전트 팀에서는 성립하지 않는다. 좌석표의 "책상에 손 든 채 남아 있음"(§9-4)은
> pane 에서만 그대로이고, 에이전트 팀에서는 좌석이 비고 질문이 팀장에게 쌓이는 형태가 된다.
...
└── references/
    ...
    ├── backends.md             # §3-5·§4-3 백엔드별(Orca·tmux·에이전트 팀) spawn·정리 명령 정본, 백엔드별 차이표(기상 신호·blocked·슬롯 점유·git 호출)
...
- **`/dflow-dev` 수정이 심링크로 모든 리포에 즉시 적용된다**(§3-14). 방어선은 **순수 가산성** 하나뿐
  ...
  끝낸다.
- **tmux 백엔드는 dev-plugin 수정에 걸려 있다(더는 차단 사유가 아니다).** `hooks/hooks.json` 이
  `PreToolUse`/`PostToolUse` 를 최상위에 두어 플러그인이 "failed to load" 다. `{"hooks": {...}}` 로
...
- **팀장 체크아웃에서 poll exit 9·10 이 팀원 작업에 대해 울리지 않는다**(§3-16). 기상 때마다 도는
  승인 스윕으로 대체했으므로 승인 반영이 최대 30분(감시 루프 TICK) 늦을 수 있다. 같은
...
- **에이전트 팀 워크트리 자동 정리의 이면** — 팀원이 끝나면 워크트리가 자동 정리되는데(변경이 없으면),
  `blocked` 로 종료한 팀원의 워크트리가 정리돼 버리면 재개 시 브랜치에서 워크트리를 새로 만들어야
  한다(§7). 커밋·push 를 먼저 하는 규칙이 이를 안전하게 만든다 — push 되지 않은 변경은 잃는다.
...
- **blocked 가 슬롯을 잡는 비용(pane 한정)**: pane 백엔드에서는 blocked 팀원이 답을 받을 때까지
  슬롯을 점유하므로 그만큼 실효 병렬도가 준다(팀원 3에 blocked 2면 실효 1). AGENT_ID 정합성(§9-1)을

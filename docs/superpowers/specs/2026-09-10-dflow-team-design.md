...
| 팀장 잠금 | 한 체크아웃에 팀장은 하나다. 체크아웃의 git 디렉터리에 잠금 디렉터리를 `mkdir` 로 원자 획득하고, 매 기상 갱신하는 `beat` 가 70분보다 오래되면 죽은 것으로 본다(§4-4) | 같은 체크아웃의 팀장 둘은 슬롯 번호·세대 파일·승인 스윕을 서로 덮어쓴다 |
...
- **부트스트랩 실패 정리**: `.result` 의 branch 가 `-`(브랜치를 만들기 전에 끝남)이면
  ...
  `git worktree remove --force`, Orca 는 `orca worktree rm --worktree path:<경로>` 로 정리한다(backends
  매핑과 같다). 그 밖의 변경이 있으면 보존하고 보고한다. 이유: 브랜치가 없어도 워커가 무언가를 고쳤다면
  그것은 사람이 판단할 산출물이다.
...
1. **전제 검사**: SKILL.md 에 적힌 셸 블록 하나로 돌린다. 블록은 실패한 항목을 모두 출력한 뒤 0 이 아닌
   ...
   - **팀장 잠금**: 잠금은 디렉터리 `$(git rev-parse --git-path dflow-team.lock)` 이며, 나머지 검사가 모두
     ...
     `owner`(`<신원>/<host>/lead` 와 시작 시각의 epoch 초)와 `beat`(epoch 초)를 쓴다. 시작 시각을 epoch 초로
     두는 이유는 마감의 소유 확인을 숫자로 비교하기 위해서다(§4-9). 팀장은 매 기상 `beat` 를 현재
     시각으로 갱신한다(§4-5). `mkdir` 가 실패하면 기존 잠금의 `beat` 를 본다. `beat` 가 없으면 방금 만들어진
     ...
     마감(§4-9)에서 지운다. 이유: 한 체크아웃의 팀장 둘은 슬롯
     번호·세대 파일·승인 스윕을 서로 덮어쓴다. 프로세스 PID 대신 `beat` 로 생존을 보는 이유는 셸 블록이
     팀장 세션 프로세스의 PID 를 믿을 만하게 얻을 수단이 없어서다. 살아 있는 팀장은 늦어도 `TICK`(30분)
...
**기상마다 하는 일**: 잠금 `beat` 갱신(§4-4) → 재구성(§4-2) → 아래 표의 처리 → 승인 스윕(§4-7) → 빈 슬롯이 있고 차단기가
허락하면 대기 큐 맨 앞부터 spawn(§4-8) → 끝나 있는 감시 루프 재기동, 재기동 조건(위)을 만족하면
poll.sh 재기동. 단 `STALE` 기상은 `beat` 갱신만 하고 나머지는 넘긴다. 컨텍스트 압축 뒤 poll 이 떠 있는지 모르면
재기동 조건에 따라 새로 띄운다. poll 이 겹쳐 떠도 exit 0 처리의 대조와 spawn 전 확인(§4-8)이 같은
작업을 두 번 띄우지 않게 막는다.
...
디렉터리(§4-4)를 지운다. 지우기 전에 `owner` 가 자기 `<신원>/<host>/lead` 이고 그 시작 시각(epoch 초)이
이 팀장의 마지막 `team.start` 시각(epoch 초로 바꾼 값) 이하인지 숫자로 비교한다. 이유: 이 팀장이 `beat` 를
70분 넘게 놓쳐 다른 팀장이 잠금을 가져갔다면 그 잠금의 시작 시각은 이 팀장의 `team.start` 보다 늦으며,
그 잠금을 지우면 안 된다. 문자열 대소 비교를 쓰지 않는 이유는 zsh 에서 실패해(§3-21) 정상 마감 뒤에도
잠금이 남기 때문이다.
...
- **수정 목록**(이 목록 밖의 원문은 바꾸지 않는다). 각 항목의 "수동" 은 `--worker` 없는 동작의 변화이며
  ...
  - `/dflow-dev`
    ...
    2. Phase 0-가 1~5번(60~79행): 1번 후보 식별을 `/dflow-merge` 와 같게 로컬 + 원격으로 넓히고, 2~5번
       ...
       push 실패가 체크아웃에 남지 않는다. 반려로 판정된 로컬 후보의 state.json 을 `phase=rejected` 로
  ...
  - `/dflow-merge`(§6-4)
    ...
    2. 후보 식별(1번): 원격 `origin/agent/*` 후보를 더하고, 로컬·원격 후보 모두에 `api_base` 필터를
       건다. 수동: 다른 PC·세션이 push 한 approved 작업도 반영되고, 다른 D'Flow 의 브랜치는 로컬이든
       원격이든 건너뛴다.
    3. 판정 보고(2번·6번): 승인 대기·반려·건너뜀(서버 상태·조회 실패·다른 D'Flow)을 가른다. 수동: 반려와
       조회 실패가 "승인 대기" 에 묻히지 않는다.
    4. 머지(4번): 머지 전 승인 뒤 변경 확인, 충돌 시 `git merge --abort`, `phase=merged` 커밋을 push 전에
       만들고 push 실패 시 `git reset --keep`. 수동: 승인 뒤 브랜치에 올라온 커밋이 main 에 섞이지 않고, 충돌
       상태나 push 안 된 커밋이 체크아웃에 남지 않는다.
    5. 뒷정리(5번): 로컬 브랜치 삭제 실패(없음·checked out) 건너뜀. 수동: 뒷정리가 중단되지 않는다.
    6. 순서(3번): 스택 판정을 브랜치 tip 의 조상 관계가 아니라 state.json `branch_base` 로 한다. 수동:
       `reported` 커밋 뒤에도 스택을 알아봐, 승인되지 않은 선행 위의 후손만 머지하지 않는다.
...
- 1번 후보 식별을 `/dflow-merge` 1번(§6-4)과 같게 한다. 로컬 `docs/tasks/*/state.json` 의
  `phase=reported` 에 더해 원격 `origin/agent/*` 후보를 보며, `api_base` 필터도 `/dflow-merge` 와 같다.
...
- **후보 식별(1번)**: 로컬 `docs/tasks/*/state.json` 의 `phase=reported`(현행 그대로)에 더해 원격
  ...
  - `api_base` 필터: 후보의 state.json `api_base` 가 현재 `DFLOW_API_BASE`(끝 `/` 제거)와 다르면
    로컬이든 원격이든 "건너뜀(다른 D'Flow)" 로 보고한다. 원격 후보는 값이 없어도 건너뛴다. 값이 없는
    로컬 후보(이 수정 전에 만든 state.json)는 수동 실행에서 지금처럼 판정하고, 팀장은 그런 후보가 있으면
    시작하지 않는다(§4-4). 이유: 스테이징 DB 는 운영을 복제하므로(§3-20), 스테이징 `.env` 로 실제
    리포에서 스윕하면 운영에서 승인된 작업을 로컬 후보든 원격 후보든 머지할 수 있다. 값이 없는 옛
    로컬 후보는 출처를 가릴 수 없으므로 사람이 보는 수동 경로에만 남긴다.
...
- **머지(4번)**: 후보마다 다음 순서로 한다.
  1. `git fetch origin && git switch <기본브랜치> && git pull --ff-only origin <기본브랜치>` 뒤 머지 직전
     HEAD 를 기록한다.
  2. **승인 뒤 변경 확인**: `git diff --name-only <증적 head_sha>..<머지 대상>` 가 그 작업의
     `docs/tasks/<TSK>/state.json` 뿐이거나 비어 있으면 머지하고, 다른 파일이 있으면 "건너뜀(승인 뒤 변경)" 으로
     보고하고 다음 후보로 간다. 이유: 원격 후보를 받으므로 승인 뒤 같은 agent 브랜치에 올라온 커밋까지 머지
     대상이 되는데, 사람이 승인한 것은 증적의 head_sha 까지다. tip 이 head_sha 와 같은지만 보면 Phase 5 의
     `reported` 커밋 때문에 늘 다르다. 증적에 head_sha 가 없는 옛 완료 보고는 이 확인을 건너뛰고 지금처럼
     머지하되 보고에 "승인 뒤 변경 확인 불가" 를 붙인다. 이유: 수동 경로가 머지하던 후보를 거부하면 퇴행이다.
...
- **수동 동작의 변화(개선)**:
  ...
  - 승인 뒤 agent 브랜치에 올라온 커밋을 머지하지 않고, `reported` 커밋 뒤에도 스택을 알아본다.
...
| 승인 스윕의 머지 충돌·push 실패 | 영향 없음 | `/dflow-merge` 가 되돌리고 보고한다(§4-7, §6-4). 팀장 체크아웃은 깨끗하게 남는다 |
...
- **머지 충돌의 반복**: 충돌하는 approved 후보는 사람이 머지할 때까지 스윕마다 되돌려지고 다시
  보고된다(§4-7). 체크아웃은 깨끗하게 남지만 보고가 반복된다.

...
6. **(신규) rtk 의 git 차단 대응** — 워커 프롬프트에서 `/usr/bin/git` 강제로 충분한지, 아니면 rtk 훅을
   고쳐야 하는지. 에이전트 팀 리허설에서 판정한다(스펙 §3-5-A·§11).
7. **(신규) 기본 백엔드 선호 정책** — pane 이 가능한 환경에서도 에이전트 팀을 쓸 수 있게 할지
   (`--backend` 플래그), 아니면 항상 pane 을 우선할지. 장시간 무인 운전은 pane 이 안전하다(스펙 §12).

...
| 구분 | 치명 | 높음 | 중간 | 낮음 |
|---|---|---|---|---|
| 공통·연동 | ~~1 (plan.md)~~ 해소 | 1 (상태 모델) | — | — |
| 가상오피스 | 1 (Micro 부하) | 1 (STANDBY 신호) | 3 | 1 |
| 팀장 스킬 | — | ~~2~~ 해소·축소 + 신규 2 해소 | 3 + 신규 2 해소, 1 해소 | 1 |

> heartbeat PAT 인증 경로는 당초 [치명]이었으나 기존 스킬로 해소되어 위 집계에서 제외했다. 남은 잔여(스코프 리터럴)는 아래에 [낮음]으로 기록한다.
>
> 2026-09-11 오후 기준 **팀장 스킬에 남은 미해소 항목은 리허설로만 확인할 수 있는 것들이다** — Orca
> 워크트리 정리(switch 된 상태), 다중 신원 캐시 경합, AGENT_ID 다중 PC 유니크성. 착수를 막지 않는다.

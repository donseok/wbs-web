# 에이전트 스튜디오 분리 — 조작(허브)과 감시(에이전트 스튜디오)를 라우트로 나눈다
...
- **프로젝트 스튜디오** `/p/[projectId]/agents/office` 를 새로 둔다. 이 프로젝트 층 하나를 기존 전역 좌석표 컴포넌트(`SeatmapView`)로 그리고 30초 폴링한다.
- 사이드바는 '에이전트' 하나. 두 페이지 상단에 "위임·승인 | 에이전트 스튜디오" 탭(링크)을 둔다.
- 전역 `/agents` 는 **전체 스튜디오**로 이름을 맞추고 그대로 둔다. 입구는 프로젝트 스튜디오 탭의 "전체 스튜디오" 링크.
...
라우트로 나누면 각자 제 갱신 규칙을 갖고, 스튜디오는 뒤에 얹을 연출(비트맵 층·캐릭터)을 프로젝트 단위로 키울 자리를 얻는다.
...
4. `SeatmapView` 에 `projectId` prop: 재조회에 전달, 빈 상태 문구, "전체 스튜디오" 링크.
5. 탭 컴포넌트 `AgentTabs` — 두 페이지의 `ProjectPageShell` `pinned` 슬롯에 얹는다(컴팩트 뷰포트에서도 남아야 하는 조작부라 `hero` 가 아니다).
6. 라벨: `nav.agents` ko '전체 스튜디오' / en 'All studios'. 전역 페이지 제목 "에이전트 스튜디오 · 전체". 허브 상태 줄의 "전체 좌석표" 링크는 스튜디오 탭으로 옮긴다.
...
- 프로젝트 스튜디오: `SeatmapView` 그대로 — 30초 폴링, 숨긴 탭은 쉼, 실패는 마지막 데이터 유지 + 표시.
...
- `usePathname()` 으로 활성 판정. `<nav aria-label="에이전트 화면">` 안에 링크 둘:
  ...
  - `data-agent-tab="office"` → `/p/${projectId}/agents/office`, 라벨 '에이전트 스튜디오'. 활성 조건 `pathname === base + '/office'`.
...
### 6-3. 스튜디오 페이지 `src/app/(app)/p/[projectId]/agents/office/page.tsx`
...
- `ProjectPageShell hero={<PageHero eyebrow="AGENTS" title={`${name} 에이전트 스튜디오`} description="이 프로젝트 층의 좌석을 30초마다 갱신합니다." />} pinned={<AgentTabs projectId={projectId} />}` 안에 `<SeatmapView initial={seatmap} projectId={projectId} />`.
...
- 헤더 오른쪽(`css.topRight`)에 `projectId` 가 있을 때만 `<Link href="/agents" data-office-all-link>전체 스튜디오</Link>`.
...
- 제목 "에이전트 스튜디오 · 전체". 주석의 입구 설명을 "프로젝트 스튜디오 탭의 전체 스튜디오 링크"로 고친다. 그 외 변경 없음.

### 6-6. 라벨

- `common.ts` `nav.agents` '전체 스튜디오', `common.en.ts` 'All studios'. `usageMenu.ts` 의 `seatmap` fallback '전체 스튜디오'.
- 설정 페이지·명세 패널 링크(→ 허브)는 그대로.

## 7. 성능 예산

- 허브: 조회 7건 그대로, 조립에서 좌석 계산이 빠진다. 서버 처리 시간은 v1 이하.
- 프로젝트 스튜디오 첫 렌더: 프로젝트 조회 1 + 좌석표 조회(주문 → 항목·부모·보고·감시자·프로젝트) — 층 하나라 주문 필터가 `in('project_id',[id])`.
...
- `tests/components/agents-seatmap-view.test.tsx`: `projectId` 있으면 재조회 인자 `('mine','p1')`, 전체 스튜디오 링크 존재, 빈 상태 프로젝트 문구; 없으면 링크 없음.
...
- `tests/domain/agents-access.test.ts`: `nav.agents` '전체 스튜디오' / 'All studios'.

## 9. 롤아웃

브랜치 `feat/agent-office-split`(origin/staging adb6466b). UI 위험 파일은 건드리지 않는다(`Sidebar.tsx` 변경 없음). 스테이징 배포 후 확인 항목: 허브에 층이 없고 탭 둘이 보임; 스튜디오 탭에서 이 프로젝트 층·폴링·전체 스튜디오 링크; 멤버(yoo7032)로 두 탭 모두 열림; 전역 `/agents` 제목·라벨.

...
| 지표 | 중앙값 | 범위 |
|---|---|---|
| 스킬 문서(SKILL.md + 참조)가 누적 입력에서 차지하는 몫 | **21.5%** | 13.1~45.4% |
| SKILL.md 본문만 | 11.7% | 9.2~19.1% |
| dev-discipline.md | 5.6% | 0.9~18.6% |

- 턴 수 69~159, 세션 누적 입력 11M~36M 토큰. 문서가 일찍 실리고 매 턴 캐시로 다시 실리므로 몫이 크다.
...
- 한계: Phase 서브에이전트 transcript 는 넣지 않았다(서브에이전트는 phase-prompt 와 자기 Phase 파일만 읽는다). 09-26 집계로 팀원 메인
  세션은 Task 전체 가중 비용의 약 11.5% 이므로, 스킬 문서 몫은 Task 전체로 보면 약 2.5% 다. 재개·반려 세션은 표본에 없었다.
...
| 「Phase 02~05」·「Phase 종료마다」 | 4 | `orch/phase-common.md` |
...
2. **원문 보존 테스트**(`dflow-dev-worker.test.ts`, fixture `dflow-dev.SKILL.orig.md`)는 `stripWorkerBlocks(routeText('manual'))`
   에 대해 돌린다. 이동만 한 줄은 그대로 통과한다. 중복 정리(§6)로 지우거나 바꾼 줄은 `CHANGED` 목록에 더한다 — 이 목록이
   **무엇을 지웠는지의 감사 기록**이 된다.
...
1. **이동 단계와 정리 단계를 커밋으로 나눈다.** 첫 커밋은 문구를 바꾸지 않고 옮기기만 한다(포인터 추가만 허용). 원문 보존 테스트가
   `CHANGED` 목록 없이 통과해야 한다 — 이것이 "모든 규칙이 수동 경로에서 여전히 읽힌다" 의 기계 검증이다.
...
- §2.3 실측에 대입하면: 팀원 메인 세션에서 스킬 문서 몫(중앙값 21.5%)이 B 경로 기준 약 45% 줄어 **메인 세션 누적 입력의 약
  10%** 가 준다. Task 전체 가중 비용으로는 약 1% 다. 절감 자체보다 **구조 정리(중복 13건·흩어진 분기 9곳·압축 유실·읽기 지시

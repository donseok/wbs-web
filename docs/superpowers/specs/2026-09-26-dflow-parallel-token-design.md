...
### 6.5-1 구현에서 달라진 점(2026-09-26 구현 뒤 정정 — 위 절보다 이 절이 우선한다)

- **heartbeat `wait_pred` 는 훅이 아니라 스킬이 직접 보낸다**: 킷 heartbeat.sh 는 진행 중 phase 만 보내므로 `wait_pred` 를 싣지
  않는다. `/dflow-dev` 가 state.json 을 바꾼 뒤 `dflow.sh heartbeat --phase wait_pred` 로 보낸다(훅 재설치 불필요).
- **claim 출력**: claim 은 JSON 을 출력하지 않던 명령이라 미충족 선행은 `DESIGN_FIRST_UNMET <JSON>` 한 줄, 너무 이른 선행 거부는
  stderr `DESIGN_FIRST_TOO_EARLY` 로 알린다.
- **build-start 404**: 서버 계약이 2.9 미만일 때만 `BUILD_START_UNSUPPORTED`(exit 0). 2.9 이상에서 404 면 exit 7(프로젝트 게이트·PAT
  범위 문제로 선행 관문을 건너뛰지 않게), 버전을 확인하지 못하면 실패. 버전 비교는 새 명령 `dflow.sh contract-ge`(숫자 비교 —
  문자열이면 2.10 < 2.9).
- **좌석**: WAIT 를 그대로 쓰면 결재할 보고가 없는 좌석이 「승인 대기」로 보이고 승인·반려 버튼이 뜬다. 좌석에 `designWait` 표식을
  두어 레인·라벨(「선행 대기」)·결재 버튼(중단만)·승인 대기 타일·허브 필터에서 따로 다룬다.
- **크레딧 표**: `ds` 가 없는 옛 표는 읽을 때 10 으로 채운다(RPC 와 같은 값). 저장 검증의 이웃 간격 10 규칙은 그대로라, ip 가 20
  미만인 옛 표는 사람이 ip 를 옮겨야 저장된다. rollback 은 표의 `ds` 키를 지운다.
- **dflow-poll 은 설계 선행을 하지 않는다**: 상한이 없어 설계를 한 건씩 쌓을 수 있다. 서버가 2.9 여도 선행 미충족은 종전 선행 대기다.
- **원격 스윕에서 `wait_pred` 브랜치를 뺀다**(`/dflow-merge`·sweep-check.sh): 설계만 push 한 브랜치가 매 스윕 후보로 잡히지 않게.
- **설계 선행 상한**: `DFLOW_DESIGN_AHEAD_MAX` 는 설계 완료 대기 작업과 설계 선행으로 도는 팀원을 함께 센다. 너무 이르다고 거부된 작업은
  2시간 또는 그 선행이 끝날 때까지 다시 고르지 않는다. 설계 완료 대기 목록은 워크트리 state.json 이 정본이다(팀장 재기동·압축에도 남는다).
- **재개**: 재개 워커는 기본 브랜치로 detach 된 채 뜨므로 agent 브랜치 끝의 state.json 에서 `wait_pred` 를 읽고 그 브랜치로 옮긴다.
  선행 머지로 lockfile 이 바뀌면 의존성을 다시 설치하고, 준비 빌드를 다시 돈 뒤 기준선을 잰다. 재머지 사유는 머지 커밋 메시지에 남긴다.
- **`## 선행 기준` 은 대개 "읽을 곳 없음"**: 미충족 선행에는 서버 `head_sha` 가 없고(승인된 선행에만 있다) 구현 중인 선행은 원격
  브랜치도 대개 없다. 그래서 재개 때 검토 모드 Design 이 거의 늘 한 번 더 돈다 — 기대 효과가 추정보다 작을 수 있다.
- **재시작 경로**: 고아 스캔 「0번 설계 완료 대기」와 restart.md 판정 표 4-1 행이 `wait_pred` 작업을 선행이 충족·반영될 때만 재개한다.
  0099 재개 요청은 재개 워커의 `wait_pred` heartbeat 가 표식을 지워 한 번만 뜬다. 점유자 신원은 PAT 경로에서 `claimed_by_user_id`
  로 가리므로 슬롯 번호(라벨)가 바뀌어도 build-start 가 통과한다.
- **검증**: 0107·rollback 은 로컬 Postgres 17 임시 스키마에서 실행을 확인했다(전체 Supabase 스키마 위는 스테이징 리허설이 첫 실행).
...
### 6.7 제품 결정 — 2026-09-26 사용자 확정("ds 크레딧 10, 늘 ds 경유, 기본값대로 진행해")

아래 넷 모두 기본값으로 확정했다. 보완안(Design 게이트 뒤 agent 브랜치를 늘 push 해 후속이 선행 design.md 를 읽게 함)은 넣지 않는다.

1. `ds` 크레딧 기본값 10.
2. 새 클라이언트는 선행이 충족된 작업도 늘 `ds` 를 거친다(모든 작업의 단계 표시가 바뀐다).
3. 선행이 `ds` 일 때는 설계 선행을 허용하지 않는다(한 단계 깊이).
4. `ds` 가 없는 옛 크레딧 표는 읽을 때 10 으로 채우고, ip 가 20 미만이면 저장 때 ip 를 옮겨야 한다(이웃 간격 10 규칙 유지).

 ...
 */
async function recordHeavy(
  admin: ReturnType<typeof createAdminClient>, userId: string, holder: string,
  heavy: HeavyReport | { pc: null; orders: [] },
) {
  try {
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (parsed.op === 'renew') {
      ...
      // heavy 가 없거나 틀린 renew(옛 킷·snapshot 실패)도 부른다 — 비워야 옛 값이 lease 가 살아 있는 동안 계속 늘어나지 않는다.
      if (kept.length > 0) await recordHeavy(admin, principal.userId, parsed.holder, parsed.heavy ?? { pc: null, orders: [] })

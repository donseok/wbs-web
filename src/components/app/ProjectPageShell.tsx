 ...
 *
 * 컴팩트 뷰포트(폭<1280 또는 높이<800 — 랩탑·태블릿·가로 폰)에선 `hero` 래퍼를 아예 렌더하지
 * 않는다 — CSS(md:) 폭 기준만으로는 가로 폰(폭 640px+)이 데스크톱으로 판정되고,
 * 숨긴 래퍼가 flex gap 을 이중으로 먹는 문제도 있다. 프로젝트명은 헤더가 보여준다.
 * SSR 은 데스크톱으로 그리므로 md 미만 첫 페인트는 PageHero 의 CSS(hidden md:grid)가 가린다.
 *
 * `pinned` 는 컴팩트에서도 남는 고정 머리다. `hero` 는 "없어도 되는 장식(제목)"이라 걷어도
 * 되지만, 위키 검색 카드처럼 화면의 유일한 조작부를 고정 영역에 얹은 경우 같이 걷히면

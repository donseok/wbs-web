...
export function KanbanBoard({ items, canEdit }: { items: ComputedItem[]; canEdit: boolean }) {
  ...
  const canDrag = canEdit && mode === 'status'
  ...
  function handleDrop(e: DragEvent<HTMLDivElement>, columnKey: string) {
    ...
    setDraggingId(null)
    const target = DROP_TARGET[columnKey]
    if (!id || target === undefined) return
    startTransition(async () => {
      const res = await updateActual(id, target)
      if (!res.ok) {
    ...
    <div className="space-y-4">
      ...
      {canDrag && (
        <div className="flex items-center gap-2 rounded-xl border border-line bg-surface-2 px-3.5 py-2 text-[12px] text-ink-muted">
          ...
          <span className="font-semibold text-pending">시작전</span> 컬럼으로 드래그하면 실적이 자동 반영됩니다.
      ...
      <div className="flex gap-4 overflow-x-auto pb-2">
        {columns.map(col => {
          const isDrop = canDrag && DROP_TARGET[col.key] !== undefined
          const active = isDrop && dragOverKey === col.key && draggingId !== null
          return (
            <div
              key={col.key}
              onDragOver={isDrop ? e => { e.preventDefault(); setDragOverKey(col.key) } : undefined}
              onDragLeave={isDrop ? () => setDragOverKey(k => (k === col.key ? null : k)) : undefined}
              onDrop={isDrop ? e => handleDrop(e, col.key) : undefined}

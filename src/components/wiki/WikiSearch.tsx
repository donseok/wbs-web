...
export function WikiSearch({ projectId, locale, initialQuery, pageHero, adminSlot }: {
  ...
  // 검색 카드는 ProjectPageShell 의 고정 영역(`pinned`)에 얹는다 — 스크롤에서 아예 빠지므로
  ...
  // `hero` 가 아니라 `pinned` 인 이유: 셸은 컴팩트 뷰포트(폭<1280·높이<800)에서 `hero` 를
  // 언마운트한다. 이 카드는 이 화면의 유일한 조작부라 같이 걷히면 검색 자체가 불가능해진다
  // (2026-08-28 운영 실측: 랩탑에서 검색창·칩이 통째로 사라짐). 페이지 제목(pageHero)은

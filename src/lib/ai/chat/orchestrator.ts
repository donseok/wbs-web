...
- 여러 출처는 [S1][S2]처럼 각각 대괄호로 붙인다. [S1, S2] 같은 묶음 표기는 금지.
- 서두 요약 문장을 포함해, 숫자나 날짜가 들어간 모든 문장 끝에 출처를 붙인다. 인용할 수 없는 숫자·날짜는 아예 쓰지 않는다.
- URL이나 Markdown 링크를 만들지 않는다. 출처 ID만 인용한다.
- 일부 도구가 실패하거나 truncated=true면 그 한계를 짧게 밝힌다.`

/**
 * LLM이 [S1, S2]처럼 묶어 쓴 인용을 검증기의 [S1][S2] 계약으로 정규화한다.
 * (실측: gemini-3.5-flash가 규칙 명시에도 묶음 표기를 자주 사용 — 유효한 인용이
 * 형식 때문에 "근거 없음"으로 폐기되는 거짓 음성을 막는다.)
 */
export function normalizeCitationGroups(text: string): string {
  return text.replace(/\[((?:S|F|R)\d+(?:\s*,\s*(?:S|F|R)\d+)+)\]/g, (_, group: string) =>
    group.split(/\s*,\s*/).map(id => `[${id}]`).join(''))
}

/**
 * LLM이 EVIDENCE 안의 fact/record ID([F1]·[R1])를 인용하는 실측 패턴을 각 항목이
 * 선언한 sourceIds([S..])로 결정적 치환한다. 팩이 이미 보유한 결속 정보만 사용하므로
 * 새로운 근거를 만들지 않으며, 알 수 없는 ID는 제거해 해당 주장이 검증에서 걸러지게 둔다.
 */
export function normalizeSynthesizedCitations(text: string, pack: EvidencePack): string {
  const sourceIdsByRef = new Map<string, readonly string[]>()
  for (const fact of pack.facts) sourceIdsByRef.set(fact.id, fact.sourceIds)
  for (const record of pack.records) sourceIdsByRef.set(record.id, record.sourceIds)

  const mapped = normalizeCitationGroups(text).replace(/\[([FR]\d+)\]/g, (_, ref: string) => {
    const sourceIds = sourceIdsByRef.get(ref)
    return sourceIds?.length ? sourceIds.map(id => `[${id}]`).join('') : ''
  })
  // 치환으로 생긴 인접 중복 인용([S1][S1])을 접는다.
  return mapped.replace(/(\[S\d+\])(?:\1)+/g, '$1')
}

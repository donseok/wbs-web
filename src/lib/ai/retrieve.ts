...
export async function retrieveContext(query: string, projectId: string | null, k = 8): Promise<Match[]> {
  ...
  // 의미검색은 부가 기능 — 어떤 실패(service_role 미설정, 마이그레이션 미적용, RPC 오류)든
  // 빈 배열로 강등해 결정형 답변 경로가 끊기지 않게 한다(/api/chat 가 500 나지 않도록).
  try {
    const admin = createAdminClient()
    const { data, error } = await admin.rpc('match_wbs_documents', {
    ...
    return ((data as RawMatch[] | null) ?? []).map(m => ({
      kind: m.kind,
      refId: m.ref_id,
      content: m.content,
    ...
    console.error('[dkbot] 의미검색 사용 불가(서비스롤/연결 문제) → 결정형 답변으로 폴백:', e instanceof Error ? e.message : e)
    return []
  }
}

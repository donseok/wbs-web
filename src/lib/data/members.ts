...
// 멤버 조회 select 정본 — 아래 mapProjectMemberRows 가 기대하는 행 모양과 한 몸이므로,
// 이 select 로 읽은 결과만 저 매퍼에 넣는다(둘을 따로 고치면 조용히 필드가 빈다).
export const PROJECT_MEMBER_SELECT =
  'id, project_id, name, email, role, title, role_label, user_id, created_at, teams(code)'

/**
 * PROJECT_MEMBER_SELECT 결과 → ProjectMember[] (가나다순).
 * getProjectMembers 와 이슈 액션(fetchIssueProjectMembers)이 같은 매핑을 복제하다가
 * 한쪽만 고쳐지는 드리프트를 막기 위해 여기 한 곳에 둔다. 에러 처리 계약은 호출부 몫이다
 * (데이터 헬퍼는 로깅 후 빈 배열, 서버 액션은 { ok: false }).
 */
export function mapProjectMemberRows(data: unknown[] | null): ProjectMember[] {
  // 멤버 명단은 앱 어디서 보든 가나다순 — 멤버 보드·근태 셀렉트·회의 참석자 피커·주간보고가
  // 모두 이 배열을 그대로 렌더하므로 여기서 한 번 정렬하면 화면 전체가 같은 순서를 쓴다.
  const rows = sortByKoreanName((data ?? []) as Record<string, unknown>[], r => r.name as string)
...
export const getProjectMembers = cache(async (projectId: string): Promise<ProjectMember[]> => {
  ...
  const { data, error } = await sb
    ...
    // 정렬은 mapProjectMemberRows 의 sortByKoreanName 이 담당한다. created_at 은 동명이인의
  ...
  return mapProjectMemberRows(data)
})

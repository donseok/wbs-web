...
function mapMinute(r: Row, bodyMd = ''): Minute {
  ...
  return {
    ...
    externalId: (r.external_id as string | null | undefined) ?? null,
  }
}
...
export const getMinuteDetail = cache(async (
  ...
  const { data: r, error } = await sb.from('minutes')
    .select('id, minute_date, team_code, title, body_md, meeting_id, project_id, meeting_occurrence_date, archived_at, created_by, created_by_name, created_at, updated_at, folder_id, external_id, meetings(project_id), projects(name)')

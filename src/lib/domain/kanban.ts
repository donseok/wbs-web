...
const TEAM_DOT: Record<TeamCode, string> = {
  PMO: 'bg-team-pmo', DT: 'bg-team-dt', ERP: 'bg-team-erp', MES: 'bg-team-mes',
}
...
/** 담당자별 — PMO/DT/ERP/MES + 미배정. leaf는 primary 담당팀마다 들어가고, primary가 없으면 미배정. */
export function groupByOwner(items: ComputedItem[]): KanbanColumn[] {
  const leaves = leavesOf(items)
  const buckets: Record<string, ComputedItem[]> = { PMO: [], DT: [], ERP: [], MES: [], 미배정: [] }

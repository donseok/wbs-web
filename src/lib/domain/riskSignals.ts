...
const SPI_TAIL = 3                 // 연속 하락 판정 표본 수
 ...
 * buildTrend 재호출 대신 spiSeries 관례(today 이전·planned≥5 가드·소수 2자리)를 동일식으로 복제.
 * plannedAt 재샘플링은 성능 미해결(fast-follow)이라 절대 하지 않는다. */
function detectDelayTrend(snapshots: SnapshotPoint[], today: string): RiskSignal | null {
  const spiSeries = snapshots
...
export function detectRiskSignals(input: RiskSignalInput): RiskSignalReport {
  ...
  const signals: RiskSignal[] = []
  const trend = detectDelayTrend(snapshots, today)

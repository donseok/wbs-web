export const minutesKo = {
  'nav.minutes': '회의록',
  'min.heroTitle': '회의록',
  'min.heroDesc': '일자·담당별 회의록(.md)을 보관하고 검색합니다.',
  'min.kpi.month': '이달 회의록',
  'min.kpi.monthSub': '이번 달 기준',
  'min.upload': '회의록 업로드',
  'min.export.all': '전체 내려받기',
  'min.export.progress': '내려받는 중…',
  'min.export.failed': '전체 회의록을 내려받지 못했습니다.',
  'min.export.networkError': '네트워크 상태를 확인하고 다시 시도해 주세요.',
  'min.team.all': '전체',
  'min.view.calendar': '달력',
  'min.view.tree': '트리',
  'min.tree.expandAll': '전체 펼치기',
  'min.tree.collapseAll': '전체 접기',
  'min.tree.allPeriod': '전체 기간',
  'min.tree.truncated': '최근 {n}건 기준으로 표시·집계됩니다',
  'min.tree.error': '트리를 불러오지 못했습니다',
  'min.tree.retry': '다시 시도',
  // 탐색기 (스펙 2026-07-23-minutes-explorer-design.md)
  'min.exp.favorites': '즐겨찾기',
  'min.exp.all': '전체',
  'min.exp.folders': '폴더',
  'min.exp.latest': '최근 {d}',
  'min.exp.more': '더 보기 ({n})',
  'min.exp.layout.grid': '그리드',
  'min.exp.layout.list': '리스트',
  'min.exp.favEmpty': '별을 눌러 자주 보는 회의록을 모아두세요',
  'min.exp.favError': '즐겨찾기를 불러오지 못했습니다',
  'min.exp.favToggleError': '즐겨찾기 저장에 실패했습니다',
  'min.exp.starAdd': '즐겨찾기 추가',
  'min.exp.starRemove': '즐겨찾기 해제',
  // 폴더 디렉토리 (스펙 2026-07-23-minutes-folders-design.md)
  'min.fold.unfiled': '미분류',
  'min.fold.new': '새 폴더',
  'min.fold.addSub': '하위 폴더 추가',
  'min.fold.rename': '이름 변경',
  'min.fold.delete': '삭제',
  'min.fold.deleteTitle': '폴더 삭제',
  'min.fold.deleteConfirm': '하위 폴더와 소속 회의록을 상위 폴더로 옮긴 뒤 이 폴더만 삭제합니다. 상위에 같은 이름의 폴더가 있으면 중단됩니다.',
  'min.fold.name': '폴더 이름',
  'min.fold.move': '폴더 이동',
  'min.fold.pickTitle': '폴더 선택',
  'min.fold.menuAria': '폴더 메뉴',
  'min.fold.moved': '폴더로 이동했습니다',
  'min.fold.cancel': '취소',
  'min.fold.error': '폴더 작업에 실패했습니다',
  // 폴더 중심 재편(§6) — D&D 판정 사유. canDropFolder 의 reason 과 1:1 대응
  'min.fold.folderMoved': '폴더를 옮겼습니다',
  'min.fold.drop.notAdmin': '폴더 이동은 관리자만 할 수 있습니다.',
  'min.fold.drop.seedRoot': '팀 기본 폴더는 이동할 수 없습니다.',
  'min.fold.drop.toRoot': '폴더는 담당 팀 폴더 안에만 둘 수 있습니다.',
  'min.fold.drop.cycle': '폴더를 자기 하위 폴더 아래로 옮길 수 없습니다.',
  'min.fold.drop.depth': '폴더는 최대 5단까지 만들 수 있습니다.',
  'min.fold.drop.crossTeam': '다른 담당 팀으로는 옮길 수 없습니다. 회의록을 개별로 옮기세요.',
  'min.fold.drop.dupName': '같은 폴더에 같은 이름이 이미 있습니다.',
  'min.fold.drop.unknownFolder': '폴더를 찾을 수 없습니다.',
  'min.fold.drop.sameParent': '이미 그 폴더 안에 있습니다.',
  'min.search.placeholder': '제목·본문 검색',
  'min.search.truncated': '최근 100건만 표시합니다.',
  'min.empty.title': '회의록이 없습니다',
  'min.empty.desc': '.md 파일을 업로드해 보관함을 시작하세요.',
  'min.form.date': '일자',
  'min.form.team': '담당',
  'min.form.subTeam': '하위 구분',
  'min.form.folder': '폴더',
  'min.form.folderPick': '폴더 선택',
  'min.form.folderRequired': '폴더를 선택하세요.',
  'min.form.folderNoTeam': '담당 팀을 판정할 수 없는 폴더입니다.',
  'min.form.title': '제목',
  'min.form.bodyFile': '본문 (.md)',
  'min.form.attachments': '첨부 파일 (선택)',
  'min.form.files': '회의록 파일',
  'min.form.filesHint': '.md 파일은 본문으로, 나머지는 첨부로 자동 분류됩니다. 한 번에 여러 개 선택 가능.',
  'min.form.roleBody': '본문',
  'min.form.roleAttach': '첨부',
  'min.form.meeting': '회의 연결 (선택)',
  'min.form.meetingNone': '연결 안 함',
  'min.form.project': '프로젝트',
  'min.form.projectNone': '프로젝트 없음',
  'min.form.save': '업로드',
  'min.form.saving': '업로드 중…',
  'min.err.bodyRequired': '회의록 본문이 될 .md 파일을 포함해 주세요.',
  'min.err.bodyExt': '.md 또는 .markdown 파일만 가능합니다.',
  'min.err.bodyFileMax': '본문 파일은 1MB 이하여야 합니다.',
  'min.err.bodyMax': '본문이 100,000자를 넘습니다. 파일을 나눠 업로드하세요.',
  'min.err.attachMax': '첨부는 개당 20MB 이하여야 합니다.',
  'min.err.attachCount': '첨부는 최대 10개까지입니다.',
  'min.err.upload': '파일 업로드에 실패했습니다.',
  'min.err.download': '다운로드 링크 발급에 실패했습니다.',
  'min.err.record': '파일 기록에 실패했습니다.',
  'min.detail.download': '원본 .md',
  'min.detail.downloadBody': '내용 .md',
  'min.detail.attachments': '첨부',
  'min.detail.linkedMeeting': '연결된 회의',
  'min.detail.edit': '수정',
  'min.detail.replaceBody': '새 버전 업로드',
  'min.detail.delete': '보관',
  'min.detail.deleteConfirm': '목록에서는 숨기되 원본·모든 버전·Wiki 근거는 그대로 보존합니다. 보관할까요?',
  'min.archive.banner': '보관된 회의록입니다. 원본 근거 확인을 위해 읽기 전용으로 표시합니다.',
  'min.detail.back': '목록',
  'min.version.title': '원본 버전',
  'min.version.desc': '회의록 원본은 덮어쓰지 않고 버전별로 보관합니다.',
  'min.version.current': '현재 버전',
  'min.version.previous': '이전 버전',
  'min.version.total': '총 {n}개',
  'min.version.download': '원본 파일',
  'min.version.noFile': '업로드된 원본 파일 없이 본문으로 생성된 버전입니다.',
  'min.version.viewing': '열람 중',
  'min.version.viewingBanner': '이전 원본 v{n}을 열람 중입니다. 이 화면에서는 원문을 변경하지 않습니다.',
  'min.version.backCurrent': '최신 버전으로 돌아가기',
  'min.wiki.title': 'Wiki 반영 결과',
  'min.wiki.open': '프로젝트 Wiki 열기',
  'min.wiki.status.unlinked': '프로젝트 미연결',
  'min.wiki.status.queued': '반영 대기',
  'min.wiki.status.processing': '반영 중',
  'min.wiki.status.ready': '반영 완료',
  'min.wiki.status.partial': '일부 반영',
  'min.wiki.status.failed': '반영 지연',
  'min.wiki.desc.unlinked': '프로젝트를 연결하면 회의록에서 확인된 지식이 Wiki에 자동 반영됩니다.',
  'min.wiki.desc.queued': 'Wiki 자동 반영을 준비하고 있습니다.',
  'min.wiki.desc.processing': '회의록 근거를 분석해 Wiki를 업데이트하고 있습니다.',
  'min.wiki.desc.ready': '명시적으로 확인된 내용만 반영했으며, 각 항목에서 원문 근거를 확인할 수 있습니다.',
  'min.wiki.desc.failed': '자동 반영을 완료하지 못했습니다. 시스템이 다시 시도합니다.',
  'min.wiki.count.created': '생성',
  'min.wiki.count.changed': '변경',
  'min.wiki.count.reaffirmed': '재확인',
  'min.wiki.count.conflicted': '충돌',
  'min.wiki.affected': '영향을 받은 Wiki 항목',
  'min.wiki.noChanges': '이번 버전에서 새로 반영된 Wiki 변경이 없습니다.',
  'min.wiki.processedAt': '최근 반영',
  'min.source.missing': '원문 위치가 변경되어 회의록 상단을 열었습니다.',
  'min.timeFix.title': '회의 시간 보정됨',
  'min.timeFix.desc': '녹취 시간대(UTC→KST, +9h)를 자동 보정했습니다',
  'min.chat.doc.title': '이 회의록에 질문',
  'min.chat.archive.title': '회의록 챗봇',
  'min.chat.placeholder': '예: 결정사항만 요약해줘',
  'min.chat.typing': '답변 준비 중',
  'min.chat.send': '보내기',
  'min.chat.empty': '응답이 비어 있어요. 다시 시도해 주세요.',
  'min.chat.error': '응답 생성에 실패했어요. 잠시 후 다시 시도해 주세요.',
  'min.chat.reset': '대화 초기화',
  'min.chat.scope.doc': '이 문서',
  'min.chat.scope.all': '전체 회의록',
  'min.meta.save': '저장',
  'min.meta.title': '회의록 수정',
  // 연동 식별자 표시 + 연결 초기화(§9)
  'min.ext.label': '또박또박 연동',
  'min.ext.other': '외부 연동',
  'min.ext.none': '연동 없음',
  'min.ext.copy': '식별자 복사',
  'min.ext.copied': '복사했습니다',
  'min.ext.copyFailed': '복사에 실패했습니다',
  'min.ext.reset': '연결 초기화',
  'min.ext.resetTitle': '연동 연결을 초기화할까요?',
  'min.ext.resetWarn1': '또박또박과의 연결이 끊깁니다.',
  'min.ext.resetWarn2': '그 상태로 또박또박이 재전송하면 중복 회의록이 생깁니다.',
  'min.ext.resetWarn3': "또박또박의 [D'Flow에서 찾기]로 다시 연결할 수 있습니다.",
  'min.ext.resetConfirm': '초기화',
  'min.ext.resetDone': '연결을 초기화했습니다',
  'min.insight.title': '핵심 요약',
  'min.insight.kind.decision': '결정',
  'min.insight.kind.action': '액션',
  'min.insight.kind.deadline': '기한',
  'min.insight.kind.risk': '리스크',
  'min.insight.preparing': 'AI 요약 준비 중…',
  'min.insight.unavailable': 'AI 요약을 만들지 못했습니다',
  'min.insight.retry': '다시 시도',
  'min.insight.none': 'AI가 뽑은 핵심 항목이 없습니다',
  'min.insight.attention': '많이 주목한 구간',
  'min.insight.collapse': '접기',
  'min.insight.expand': '펼치기',
  'min.hl.add': '하이라이트',
  'min.hl.remove': '하이라이트 해제',
  'min.hl.people': '하이라이트한 사람',
  'min.hl.failed': '하이라이트를 저장하지 못했습니다',
  'min.issue.create': '이슈로 등록',
  'min.issue.createAnother': '별도 이슈 등록',
  'min.issue.linked': '연결된 이슈',
  'min.issue.open': '이슈 #{n} 열기',
  'min.issue.more': '외 {n}건',
  'min.issue.created': '이슈가 등록되었습니다',
  'min.issue.createdDesc': '회의록 원문과 연결했습니다.',
  'min.issue.projectTitle': '이슈 프로젝트 선택',
  'min.issue.projectDesc': '이 회의록은 프로젝트가 지정되지 않았습니다. 이슈를 등록할 프로젝트를 선택하세요.',
  'min.issue.projectRequired': '프로젝트를 선택하세요.',
  'min.issue.continue': '계속',
  'min.issue.membersFailed': '프로젝트 담당자 목록을 불러오지 못했습니다.',
  'min.issue.versionMissing': '현재 회의록 원본 버전을 찾을 수 없어 이슈를 등록할 수 없습니다.',
  'min.issue.sourceLabel': '선택한 회의록 블록',
  'min.toc.title': '목차',
  'min.focus.on': '집중 모드',
  'min.focus.off': '집중 모드 해제',
  'min.fs.group': '본문 글자크기',
  'min.fs.decrease': '글자 작게',
  'min.fs.increase': '글자 크게',
  'min.fs.reset': '기본 크기로',
  'min.fs.current': '현재 글자크기',
  'min.share.readonly': '읽기 전용 공유 문서',
  'min.share.button': '공유',
  'min.share.title': '외부 링크 공유',
  'min.share.desc': '켜면 링크가 있는 누구나 로그인 없이 본문과 목차를 볼 수 있습니다.',
  'min.share.on': '링크 공유 켜짐',
  'min.share.off': '링크 공유 꺼짐',
  'min.share.copy': '링크 복사',
  'min.share.copied': '링크를 복사했습니다.',
  'min.share.copyFailed': '복사에 실패했습니다. 주소를 직접 선택해 복사하세요.',
  'min.share.regen': '재발급',
  'min.share.regenConfirm': '기존 링크가 즉시 무효화됩니다. 재발급할까요?',
  'min.share.failed': '공유 설정 처리에 실패했습니다.',
}

export const minutesEn: Record<keyof typeof minutesKo, string> = {
  'nav.minutes': 'Minutes',
  'min.heroTitle': 'Minutes',
  'min.heroDesc': 'Archive and search meeting minutes (.md) by date and team.',
  'min.kpi.month': 'This month',
  'min.kpi.monthSub': 'this month',
  'min.upload': 'Upload minutes',
  'min.export.all': 'Download all',
  'min.export.progress': 'Downloading…',
  'min.export.failed': 'Failed to download all minutes.',
  'min.export.networkError': 'Check your network connection and try again.',
  'min.team.all': 'All',
  'min.view.calendar': 'Calendar',
  'min.view.tree': 'Tree',
  'min.tree.expandAll': 'Expand all',
  'min.tree.collapseAll': 'Collapse all',
  'min.tree.allPeriod': 'All time',
  'min.tree.truncated': 'Showing and counting the most recent {n} only',
  'min.tree.error': 'Failed to load the tree',
  'min.tree.retry': 'Retry',
  'min.exp.favorites': 'Favorites',
  'min.exp.all': 'All',
  'min.exp.folders': 'Folders',
  'min.exp.latest': 'Latest {d}',
  'min.exp.more': 'Show more ({n})',
  'min.exp.layout.grid': 'Grid',
  'min.exp.layout.list': 'List',
  'min.exp.favEmpty': 'Star minutes to collect them here',
  'min.exp.favError': 'Failed to load favorites',
  'min.exp.favToggleError': 'Failed to save favorite',
  'min.exp.starAdd': 'Add to favorites',
  'min.exp.starRemove': 'Remove from favorites',
  'min.fold.unfiled': 'Unfiled',
  'min.fold.new': 'New folder',
  'min.fold.addSub': 'Add subfolder',
  'min.fold.rename': 'Rename',
  'min.fold.delete': 'Delete',
  'min.fold.deleteTitle': 'Delete folder',
  'min.fold.deleteConfirm': 'Subfolders and their minutes move up to the parent folder, then this folder alone is deleted. Stops if the parent already has a folder with the same name.',
  'min.fold.name': 'Folder name',
  'min.fold.move': 'Move to folder',
  'min.fold.pickTitle': 'Choose folder',
  'min.fold.menuAria': 'Folder menu',
  'min.fold.moved': 'Moved to folder',
  'min.fold.cancel': 'Cancel',
  'min.fold.error': 'Folder operation failed',
  'min.fold.folderMoved': 'Folder moved',
  'min.fold.drop.notAdmin': 'Only administrators can move folders.',
  'min.fold.drop.seedRoot': 'Team default folders cannot be moved.',
  'min.fold.drop.toRoot': 'Folders must stay inside a team folder.',
  'min.fold.drop.cycle': 'A folder cannot be moved into its own subfolder.',
  'min.fold.drop.depth': 'Folders can be nested up to 5 levels.',
  'min.fold.drop.crossTeam': 'Cannot move across teams. Move the minutes individually.',
  'min.fold.drop.dupName': 'A folder with the same name already exists here.',
  'min.fold.drop.unknownFolder': 'Folder not found.',
  'min.fold.drop.sameParent': 'It is already in that folder.',
  'min.search.placeholder': 'Search title/body',
  'min.search.truncated': 'Showing latest 100 only.',
  'min.empty.title': 'No minutes yet',
  'min.empty.desc': 'Upload a .md file to start the archive.',
  'min.form.date': 'Date',
  'min.form.team': 'Team',
  'min.form.subTeam': 'Subcategory',
  'min.form.folder': 'Folder',
  'min.form.folderPick': 'Choose folder',
  'min.form.folderRequired': 'Select a folder.',
  'min.form.folderNoTeam': 'Cannot determine the team for this folder.',
  'min.form.title': 'Title',
  'min.form.bodyFile': 'Body (.md)',
  'min.form.attachments': 'Attachments (optional)',
  'min.form.files': 'Minutes files',
  'min.form.filesHint': 'The .md file becomes the body; everything else is attached. Multi-select supported.',
  'min.form.roleBody': 'Body',
  'min.form.roleAttach': 'Attach',
  'min.form.meeting': 'Link meeting (optional)',
  'min.form.meetingNone': 'No link',
  'min.form.project': 'Project',
  'min.form.projectNone': 'No project',
  'min.form.save': 'Upload',
  'min.form.saving': 'Uploading…',
  'min.err.bodyRequired': 'Include a .md file to be the minutes body.',
  'min.err.bodyExt': 'Only .md or .markdown files are allowed.',
  'min.err.bodyFileMax': 'Body file must be 1MB or less.',
  'min.err.bodyMax': 'Body exceeds 100,000 characters. Split the file.',
  'min.err.attachMax': 'Each attachment must be 20MB or less.',
  'min.err.attachCount': 'Up to 10 attachments.',
  'min.err.upload': 'File upload failed.',
  'min.err.download': 'Failed to issue a download link.',
  'min.err.record': 'Failed to record file.',
  'min.detail.download': 'Original .md',
  'min.detail.downloadBody': 'Content .md',
  'min.detail.attachments': 'Attachments',
  'min.detail.linkedMeeting': 'Linked meeting',
  'min.detail.edit': 'Edit',
  'min.detail.replaceBody': 'Upload new version',
  'min.detail.delete': 'Archive',
  'min.detail.deleteConfirm': 'Hide this from lists while preserving the original, every version, and Wiki evidence?',
  'min.archive.banner': 'This minute is archived and shown read-only so its source evidence remains auditable.',
  'min.detail.back': 'Back',
  'min.version.title': 'Source versions',
  'min.version.desc': 'Minutes source files are preserved by version instead of being overwritten.',
  'min.version.current': 'Current',
  'min.version.previous': 'Previous versions',
  'min.version.total': '{n} total',
  'min.version.download': 'Source file',
  'min.version.noFile': 'This version was created from text without an uploaded source file.',
  'min.version.viewing': 'Viewing',
  'min.version.viewingBanner': 'Viewing source version v{n}. Historical source is read-only.',
  'min.version.backCurrent': 'Back to current',
  'min.wiki.title': 'Wiki impact',
  'min.wiki.open': 'Open project Wiki',
  'min.wiki.status.unlinked': 'No project',
  'min.wiki.status.queued': 'Queued',
  'min.wiki.status.processing': 'Processing',
  'min.wiki.status.ready': 'Updated',
  'min.wiki.status.partial': 'Partially updated',
  'min.wiki.status.failed': 'Delayed',
  'min.wiki.desc.unlinked': 'Link a project to automatically add confirmed knowledge from these minutes to its Wiki.',
  'min.wiki.desc.queued': 'Preparing the automatic Wiki update.',
  'min.wiki.desc.processing': 'Analyzing evidence in the minutes and updating the Wiki.',
  'min.wiki.desc.ready': 'Only explicit statements were applied, with source evidence available from each item.',
  'min.wiki.desc.failed': 'The automatic update did not finish. The system will retry.',
  'min.wiki.count.created': 'Created',
  'min.wiki.count.changed': 'Changed',
  'min.wiki.count.reaffirmed': 'Reaffirmed',
  'min.wiki.count.conflicted': 'Conflicts',
  'min.wiki.affected': 'Affected Wiki items',
  'min.wiki.noChanges': 'This version did not produce a new Wiki change.',
  'min.wiki.processedAt': 'Last updated',
  'min.source.missing': 'The source moved or was removed, so the document opened at the top.',
  'min.timeFix.title': 'Meeting time corrected',
  'min.timeFix.desc': 'Auto-corrected recording timezone (UTC→KST, +9h)',
  'min.chat.doc.title': 'Ask this document',
  'min.chat.archive.title': 'Minutes chatbot',
  'min.chat.placeholder': 'e.g. Summarize only the decisions',
  'min.chat.typing': 'Preparing a reply',
  'min.chat.send': 'Send',
  'min.chat.empty': 'Empty response. Please retry.',
  'min.chat.error': 'Failed to generate a response. Try again shortly.',
  'min.chat.reset': 'Clear conversation',
  'min.chat.scope.doc': 'This doc',
  'min.chat.scope.all': 'All minutes',
  'min.meta.save': 'Save',
  'min.meta.title': 'Edit minutes',
  'min.ext.label': 'Ddobak link',
  'min.ext.other': 'External link',
  'min.ext.none': 'Not linked',
  'min.ext.copy': 'Copy identifier',
  'min.ext.copied': 'Copied',
  'min.ext.copyFailed': 'Copy failed',
  'min.ext.reset': 'Reset link',
  'min.ext.resetTitle': 'Reset the external link?',
  'min.ext.resetWarn1': 'The link to Ddobak will be broken.',
  'min.ext.resetWarn2': 'If Ddobak re-sends afterwards, a duplicate minute is created.',
  'min.ext.resetWarn3': "You can relink from Ddobak's [Find in D'Flow].",
  'min.ext.resetConfirm': 'Reset',
  'min.ext.resetDone': 'Link reset',
  'min.insight.title': 'Key takeaways',
  'min.insight.kind.decision': 'Decision',
  'min.insight.kind.action': 'Action',
  'min.insight.kind.deadline': 'Deadline',
  'min.insight.kind.risk': 'Risk',
  'min.insight.preparing': 'Preparing AI summary…',
  'min.insight.unavailable': "Couldn't generate the AI summary",
  'min.insight.retry': 'Retry',
  'min.insight.none': 'No key items found',
  'min.insight.attention': 'Most highlighted',
  'min.insight.collapse': 'Collapse',
  'min.insight.expand': 'Expand',
  'min.hl.add': 'Highlight',
  'min.hl.remove': 'Remove highlight',
  'min.hl.people': 'Highlighted by',
  'min.hl.failed': "Couldn't save the highlight",
  'min.issue.create': 'Create issue',
  'min.issue.createAnother': 'Create another issue',
  'min.issue.linked': 'Linked issues',
  'min.issue.open': 'Open issue #{n}',
  'min.issue.more': '+{n} more',
  'min.issue.created': 'Issue created',
  'min.issue.createdDesc': 'Linked it to the minutes source.',
  'min.issue.projectTitle': 'Choose issue project',
  'min.issue.projectDesc': 'These minutes have no project. Choose the project where the issue should be created.',
  'min.issue.projectRequired': 'Choose a project.',
  'min.issue.continue': 'Continue',
  'min.issue.membersFailed': 'Failed to load project assignees.',
  'min.issue.versionMissing': 'The current source version is unavailable, so an issue cannot be created.',
  'min.issue.sourceLabel': 'Selected minutes block',
  'min.toc.title': 'Contents',
  'min.focus.on': 'Focus mode',
  'min.focus.off': 'Exit focus',
  'min.fs.group': 'Body text size',
  'min.fs.decrease': 'Decrease text size',
  'min.fs.increase': 'Increase text size',
  'min.fs.reset': 'Reset to default size',
  'min.fs.current': 'Current text size',
  'min.share.readonly': 'Read-only shared document',
  'min.share.button': 'Share',
  'min.share.title': 'Share via link',
  'min.share.desc': 'When on, anyone with the link can view the body and outline without signing in.',
  'min.share.on': 'Link sharing on',
  'min.share.off': 'Link sharing off',
  'min.share.copy': 'Copy link',
  'min.share.copied': 'Link copied.',
  'min.share.copyFailed': 'Copy failed. Select the URL and copy it manually.',
  'min.share.regen': 'Regenerate',
  'min.share.regenConfirm': 'The current link stops working immediately. Regenerate?',
  'min.share.failed': 'Failed to update sharing settings.',
}

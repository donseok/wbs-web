...
describe('StageCreditSlider', () => {
  ...
  it('표는 하나뿐이고 설정이 없으면 코드 기본값으로 그린다', async () => {
    ...
    expect(['as', 'ds', 'ip', 'rw', 'im'].map(k => input(k).value)).toEqual(['0', '10', '30', '50', '80'])
  ...
  it('저장은 검증된 표로 액션을 부르고, 성공하면 저장됨 표시 + 새로고침', async () => {
    await mount({ initial: { default: { as: 0, ds: 10, ip: 30, rw: 50, im: 80, xx: 100 } } })
    await type(input('rw'), '60')
    await act(async () => { saveBtn()!.click() })
    expect(updateStageCredits).toHaveBeenCalledWith('p1', { default: { as: 0, ds: 10, ip: 30, rw: 60, im: 80, xx: 100 } })
    expect(container.querySelector('[data-credit-saved]')).not.toBeNull()
    expect(refresh).toHaveBeenCalledTimes(1)
  })

  it('ds 가 없는 옛 표(0107 이전)는 ds 를 기본값 10 으로 그리고, 저장하면 ds 를 담는다', async () => {
    await mount({ initial: { default: { as: 0, ip: 40, rw: 50, im: 80, xx: 100 } } as never })
    expect(input('ds').value).toBe('10')
    expect(handle('ds').style.left).toBe('10%')
    await type(input('im'), '85')
    await act(async () => { saveBtn()!.click() })
    expect(updateStageCredits).toHaveBeenCalledWith('p1', { default: { as: 0, ds: 10, ip: 40, rw: 50, im: 85, xx: 100 } })
  })
  ...
  it('미리보기는 지금 값으로 사건 흐름을 보여 주고, 행을 누르면 현재 위치가 옮겨간다', async () => {
    ...
    expect(actual(1)).toContain('10')   // claim(설계 선행) → ds
    expect(actual(2)).toContain('30')   // build_start → ip
    expect(actual(5)).toContain('100')  // 승인 → xx
    // 계획 진척 60 이 기본이라 실적 10 은 지연, 100 은 완료다.
    expect(container.querySelector('[data-credit-pv-status="1"]')?.textContent).toBe('settings.creditPvDelayed')
    expect(container.querySelector('[data-credit-pv-status="5"]')?.textContent).toBe('settings.creditPvDone')
    ...
    await act(async () => { (rows[3] as HTMLElement).click() })

...
describe('splitLeafOwners — 복수 담당 말단 분리', () => {
  it('복수 담당 말단 activity 아래에 팀당 1개 sub-act를 생성하고 원본 행은 그대로 둔다', () => {
    const src = [
      ...
      imp({
        tempId: 't2', parentTempId: 't1', name: '데이터 플랫폼 요건 정의', biz: 'DT', deliverable: '요건정의서',
        owners: [
    ...
    for (const s of subs) {
      ...
      expect(s.biz).toBe('DT') // biz·산출물 승계
  ...
  it('단일 담당·무담당 말단, 자식 있는 복수 담당 상위, 말단 phase 는 분리하지 않는다', () => {
    const src = [
      ...
      imp({ tempId: 't2', parentTempId: 't1', name: '단일 담당', owners: [{ team: 'DT', kind: 'primary' }] }),
      ...
      imp({
        ...
        owners: [{ team: 'PMO', kind: 'primary' }, { team: 'DT', kind: 'support' }],
      }),
    ]
    const out = splitLeafOwners(src)
  ...
  it('자식 없는 복수 담당 task 도 담당별 activity 로 분리된다', () => {
    ...
    expect(subs.map(s => s.name)).toEqual(['1-1. 중간보고 (PMO 주관)', '1-1. 중간보고 (DT 지원)'])
...
describe('buildWbsAoa — sub-act 접기(라운드트립 보호)', () => {
  it('activity 하위 sub-act 는 행으로 내보내지 않고 부모 실적%에 롤업값을 싣는다', () => {
    const sub = (id: string, team: 'DT' | 'ERP', pct: number) =>
    ...
    const parent = comp({
      ...
      owners: [{ team: 'DT', kind: 'primary' }, { team: 'ERP', kind: 'primary' }],
      children: [sub('s1', 'DT', 50), sub('s2', 'ERP', 30)],
    })
    ...
    expect(actRow[6 + 1]).toBe('●') // DT 담당 표기 보존(G6~J9 중 H7)

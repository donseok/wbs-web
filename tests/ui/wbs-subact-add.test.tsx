...
describe('RowDetailPanel — SUB-ACT 추가 어포던스', () => {
  ...
  const teamOptions = () => buttons().filter(b => /^(PMO|ERP|MES|가공)$/.test((b.textContent ?? '').trim()))
  ...
  it('실적%가 있는 리프 ACT에서 폼을 열면 롤업 전환 경고가 뜬다', async () => {
    ...
    expect(teamOptions().map(b => (b.textContent ?? '').trim())).toEqual(['PMO', 'ERP', 'MES', '가공'])
  })
  ...
  it('남은 팀이 하나뿐이면 폼을 열 때 자동 선택되어 팀 클릭 없이 바로 추가된다', async () => {
    await mount(item({ id: 'a1', name: 'A', children: [child('PMO'), child('ERP'), child('MES')] }))
    await act(async () => byText(/wbs\.addSubAct/)[0].click())
